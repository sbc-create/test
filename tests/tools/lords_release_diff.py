...
def сверить(до, после, *, ожидается_новый_релиз, сайт):
    ...
    if ожидается_новый_релиз:
        ...
        # Соседняя витрина вправе получить свой релиз: обновление каталога идёт
        # по всем трём. Не вправе она получить ЧУЖОЙ шаблон — это и проверено
        # выше, полем template_digest. Требовать от соседа неизменного номера
        # релиза значило бы объявлять дефектом штатную работу таймера.
        pass
    return беды
