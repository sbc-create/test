...
// Порог взят из Core Web Vitals, а не выдуман под локальный стенд.
//
// Прежде здесь стояла тысяча миллисекунд — «на локальном стенде всё медленнее
// секунды означает поломку». Рассуждение выглядело разумным и оказалось
// негодным: замер дважды падал на разных витринах и разных ширинах, потому что
// LCP на общей машине зависит от чужой нагрузки, а не только от страницы.
// Ворота, которые срабатывают от соседнего процесса, перестают что-либо
// значить: их начинают перезапускать до зелёного.
//
// Стандартный порог 2500 мс ловит настоящий обвал и не ловит шум. Фактические
// значения при этом сохраняются полностью — по ним видно, что стенд отдаёт
// первый экран за сотни миллисекунд, и это остаётся наблюдением, а не воротами.
...
test.describe('скорость и устойчивость раскладки', () => {
  ...
  test.afterAll(() => {
    ...
    fs.writeFileSync(path.join(OUT, 'performance.json'), `${JSON.stringify({
      ...
      lcp_budget_source: 'Core Web Vitals; локальный стенд даёт нижнюю границу',
