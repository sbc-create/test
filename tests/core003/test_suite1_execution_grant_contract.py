...
class TestЗаконныйЦикл:
    ...
    def test_ключ_стенда_не_доверен_чужим_набором(self, стенд, посторонний_signer):
        """Ключ одного контура доверия не признаётся другим — в обе стороны.

        Прежде здесь стоял запрос к `127.0.0.1:8795` — к БОЕВОЙ службе подписи
        хоста. Утверждение было верным по смыслу и негодным по исполнению: в CI
        на этом порту никто не слушает, и проверка падала на `Connection
        refused`, ничего не проверив; а на хосте она обращалась к боевому
        контуру ради факта, который боевого контура не касается.

        Проверяемое свойство — изоляция наборов ключей, и для него нужен просто
        ДРУГОЙ набор. Он поднимается управляемо (`посторонний_signer`) и гаснет
        вместе с тестом.

        Проверка симметрична намеренно: односторонняя прошла бы и у службы,
        которая отдаёт пустой JWKS. Пустой набор не доверяет никому — и не
        доказывает ничего.
        """
        with urllib.request.urlopen(посторонний_signer["база"] + "/jwks",
                                    timeout=10) as о:
            чужой = json.loads(о.read())
        with urllib.request.urlopen(стенд["signer"].база + "/jwks", timeout=10) as о:
            свой = json.loads(о.read())

        чужие_kid = {к["kid"] for к in чужой["keys"]}
        свои_kid = {к["kid"] for к in свой["keys"]}

        assert свои_kid == {стенд["ключи"]["kid"]}
        assert чужие_kid == {посторонний_signer["ключи"]["kid"]}
        assert стенд["ключи"]["kid"] not in чужие_kid
        assert посторонний_signer["ключи"]["kid"] not in свои_kid
