...
class ТестовыйSigner:
    ...
    def __init__(self, каталог_ключей: Path, changeset_db: Path,
        ...
        окр = dict(os.environ,
                   ...
                   PYTHONUNBUFFERED="1")
        self.путь_журнала = каталог_ключей.parent / f"signer-{self.порт}.log"
        self.журнал = self.путь_журнала.open("w")
        self.процесс = subprocess.Popen(
            [ПИТОН, "-m", "factory.site_engine.approval.service"],
            cwd=str(РЕЛИЗ), env=окр, stdout=self.журнал,
            stderr=subprocess.STDOUT)
        self._дождаться_готовности()

    def _дождаться_готовности(self, срок: float = 30.0) -> None:
        """Готовность, а не пауза наугад.

        Прежний цикл ждал восемь секунд и сообщал «служба не поднялась», не
        различая три разных случая: служба ещё стартует, служба уже умерла и
        служба жива, но не отвечает. На медленном раннере первый случай
        выглядел как отказ, а во втором и третьем причина оставалась в логе,
        которого никто не читал.

        Теперь ранняя смерть процесса обрывает ожидание сразу и выносит наружу
        то, что служба успела написать: это и есть ответ на вопрос «почему».
        """
        предел = time.monotonic() + срок
        последняя: Exception | None = None
        while time.monotonic() < предел:
            код = self.процесс.poll()
            if код is not None:
                raise RuntimeError(
                    f"служба подписи завершилась с кодом {код} до готовности:\n"
                    + self._хвост_журнала())
            try:
                with urllib.request.urlopen(
                        f"http://127.0.0.1:{self.порт}/healthz", timeout=2):
                    return
            except Exception as ош:  # noqa: BLE001 - ждём именно любой отказ
                последняя = ош
                time.sleep(0.1)
        raise RuntimeError(
            f"служба подписи не ответила на /healthz за {срок} с "
            f"(последняя ошибка: {последняя}):\n" + self._хвост_журнала())

    def _хвост_журнала(self) -> str:
        self.журнал.flush()
        if not self.путь_журнала.is_file():
            return "<журнала нет>"
        return self.путь_журнала.read_text(encoding="utf-8", errors="replace")[-2000:]
    ...
    def закрыть(self) -> None:
        """Гашение обязательно и выполняется до конца.

        `terminate` без последующего `kill` оставлял бы процесс жить, если тот
        не реагирует на TERM: порт остался бы занят, а следующий прогон принял
        бы чужую службу за свою.
        """
...
@pytest.fixture()
def посторонний_signer(стенд, tmp_path):
    """Вторая служба подписи со СВОИМ набором ключей — чужой контур доверия.

    Нужна для одного утверждения: ключ стенда не признаётся набором, который
    его не выдавал. Прежде это проверялось обращением к службе на
    `127.0.0.1:8795` — то есть к БОЕВОЙ службе хоста. Проверка была верна по
    смыслу и негодна по исполнению: в CI на этом порту никто не слушает, и она
    падала на `Connection refused`, а на хосте тест подключался к боевому
    контуру, чтобы узнать то, что к боевому контуру отношения не имеет.

    Свойство «ключ одного контура не доверен другому» не требует, чтобы вторым
    контуром был именно production: оно требует ДРУГОГО набора ключей. Здесь он
    поднимается управляемо и гасится вместе с тестом.
    """
    каталог = tmp_path / "посторонний"
    сведения = подготовить_ключи(каталог)
    служба = ТестовыйSigner(каталог, tmp_path / "посторонний.sqlite3",
                            стенд["registry_base"])
    try:
        yield {"база": служба.база, "ключи": сведения}
    finally:
        служба.закрыть()
