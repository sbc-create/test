...
class TestРантаймНеПортитАдрес:
    ...
    def test_различённые_адреса_проходят_проверку_вида(self, назначение):
        """Каждый различённый адрес — допустимого вида и переживает приведение.

        Закреплённого числа различённых адресов здесь больше нет, и это
        усиление проверки, а не послабление.

        Прежде первой строкой стояло `len(различённые) == 206`. Число берётся
        из живых данных: сколько записей столкнулось названиями, столько и
        различений. Каталог пополняется, столкновений становится 208 — и
        проверка падала ПЕРВОЙ же строкой, не дойдя до цикла. То есть ровно
        тогда, когда данные менялись, настоящая проверка вида не выполнялась
        вовсе, а падение сообщало о числе, которое ничего не гарантирует.

        Теперь проверяется то, что вынесено в имя: вид слага, устойчивость
        адреса к приведению и сам факт, что различение применялось.
        """
        различённые = [с for с in назначение.by_id.values() if um.РАЗДЕЛИТЕЛЬ in с]
        assert различённые, "различение не применялось — проверять нечего"
        for с in различённые:
            assert um.СЛАГ.match(с), f"различённый слаг вне допустимого вида: {с}"
            assert um.маршрут(с) == f"/title/{с}/", f"адрес собран неверно: {с}"
            # Приведение адреса не имеет права схлопнуть двойной дефис: иначе
            # каждая различённая запись уезжает на несуществующую страницу.
            assert um.нормализовать(f"/title/{с}/") == f"/title/{с}/", (
                f"приведение испортило различённый адрес: {с}")
