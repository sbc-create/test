...
def _база(куда: Path) -> Path:
    """Контур фикстуры с живыми часами: рантайм читает его реальным временем.

    Фикстура заморожена на `ФИКСТУРА.СЕЙЧАС` — так и надо модульным тестам,
    которые передают эти часы явно. Рантайм часов не принимает: в бою он
    обязан смотреть на настоящее время, и у материалов есть SLA свежести в
    48 часов. Поэтому замороженный контур через двое суток после этой даты
    выглядит для рантайма протухшим, и `/__contract` честно отвечает
    `news: 0` — проверка же падала так, будто рантайм перестал видеть
    поверхность. Воспроизводится на исходном HEAD b9a154c.

    Здесь контур приводится к состоянию живого: материалы обновлены «сейчас»,
    будущие серии сдвинуты в будущее. Ничего не выдумывается — сдвигается
    только время, и ровно у тех полей, по которым рантайм принимает решение.
    """
    файл = куда / "readmodel.sqlite3"
    соед = sqlite3.connect(файл)
    ФИКСТУРА.построить(соед)
    сейчас = dt.datetime.now(dt.timezone.utc)
    соед.execute("UPDATE editorial_post SET published_at=?, updated_at=?",
                 (сейчас.isoformat(), сейчас.isoformat()))
    for н, (ид,) in enumerate(
            соед.execute("SELECT entity_id FROM entity WHERE next_episode_at"
                         " IS NOT NULL ORDER BY entity_id").fetchall(), 1):
        соед.execute("UPDATE entity SET next_episode_at=? WHERE entity_id=?",
                     ((сейчас + dt.timedelta(days=н)).isoformat(), ид))
    соед.commit()
    соед.close()
    return файл
...
class TestДанныеСобственныхСтраниц:
    """Разделы наполняются контуром, а не разбором чужой разметки.

    Заглушка приложения не отдаёт ни одной плитки каталога и ни одной ленты
    главной — в ней только голова, шапка, пустой `main` и подвал. Если раздел
    всё же наполнен, значит данные пришли из контура чтения. Именно это и
    требуется закрепить: прежний разбор полезной нагрузки регулярными
    выражениями сломался молча, и страницы вышли наружу пустыми.
    """

    def test_заглушка_действительно_пуста(self, витрина):
        # Проверка самой проверки: если бы заглушка отдавала плитки, тесты
        # ниже зеленели бы и при вернувшемся разборе чужой разметки.
        assert "portal-catalog-tile" not in ЗАГЛУШКА
        assert "Новые серии" not in ЗАГЛУШКА

    def test_новинки_наполнены_из_контура(self, витрина):
        код, тело = взять(витрина, "/new/")
        assert код == 200
        assert 'data-sf-own="1"' in тело
        # Фикстура даёт четыре события появления серии.
        assert тело.count('class="portal-catalog-tile"') >= 4
        assert "Новые серии" in тело

    def test_топ_рисует_карточки(self, витрина):
        """Регрессия: словарь карточки разошёлся, и Топ остался без плиток."""
        код, тело = взять(витрина, "/top/")
        assert код == 200
        assert тело.count('class="portal-catalog-tile"') >= 1
        assert 'class="sf-rank">#1<' in тело

    def test_подборки_показывают_оценку_с_провайдером(self, витрина):
        _, тело = взять(витрина, "/collections/")
        assert "Высокие оценки" in тело
        assert "portal-catalog-rating-value" in тело
        # Оценка без названия провайдера запрещена: общее безымянное число
        # сводит разные шкалы в одно.
        assert "IMDb" in тело

    def test_одна_система_карточек_на_странице(self, витрина):
        """Две системы карточек рядом — расхождение с оригиналом."""
        for путь in ("/new/", "/collections/", "/top/"):
            _, тело = взять(витрина, путь)
            main = тело.split("<main", 1)[1].split("</main>")[0]
            assert 'class="sf-rc"' not in main, путь

    def test_пустая_поверхность_не_рисует_ни_полосы_ни_рамки(self, витрина):
        """«Пока пусто» в рамке читается как работающий раздел без данных."""
        _, тело = взять(витрина, "/new/")
        main = тело.split("<main", 1)[1].split("</main>")[0]
        # У фикстуры нет поверхности «сейчас выходят» в подтверждённом виде
        # ровно тогда, когда её нет; полоса без плиток не рисуется никогда.
        полосы = re.findall(r'<div class="portal-section-bar">([^<]+)</div>', main)
        for полоса in полосы:
            кусок = main.split(f'>{полоса}</div>', 1)[1]
            следующая = кусок.find('class="portal-section-bar"')
            кусок = кусок if следующая < 0 else кусок[:следующая]
            assert "portal-catalog-tile" in кусок, f"полоса без плиток: {полоса}"

    def test_страница_называет_возраст_контура(self, витрина):
        """Пустая полка без возраста данных читается как поломка шаблона."""
        _, тело = взять(витрина, "/new/")
        assert ("Контур пополнялся последний раз" in тело
                or "Момент последнего пополнения" in тело)

    def test_страница_без_данных_объясняет_себя(self, витрина):
        """Раздел без единой наполненной поверхности не молчит."""
        _, тело = взять(витрина, "/schedule/")
        assert взять(витрина, "/schedule/")[0] == 200
        # Либо есть плитки, либо названы поверхности, которых не хватило.
        assert ('class="portal-catalog-tile"' in тело
                or "строится на поверхностях контура" in тело)


class TestПовторныйОтветСовпадает:
    """Один и тот же запрос дважды — один и тот же ответ.

    Сравнение «до и после» и приёмка кадрами держатся на этом: страница,
    которая меняется сама по себе, делает любое сравнение бессмысленным.
    Проверяется именно ответ целиком, а не число карточек: перестановка двух
    плиток местами числа не меняет.
    """

    ПУТИ = ("/top/", "/new/", "/collections/", "/schedule/")

    def test_собственные_страницы_совпадают_побайтно(self, витрина):
        for путь in self.ПУТИ:
            первый = взять(витрина, путь)
            второй = взять(витрина, путь)
            assert первый[0] == второй[0] == 200, путь
            assert первый[1] == второй[1], f"ответ изменился между заходами: {путь}"

    def test_объявление_контракта_совпадает(self, витрина):
        # Числа поверхностей — тоже ответ: пляшущее число невозможно принять.
        первый = json.loads(взять(витрина, "/__contract")[1])
        второй = json.loads(взять(витрина, "/__contract")[1])
