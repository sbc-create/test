...
class TestКарточкаПоHTTP:
    ...
    def test_блок_приходит_в_конце_body_и_помечен_к_переносу(self, страница):
        """Чужой ребёнок в контейнере React ломает гидратацию.

        Блок кладётся в конец body — там React его переживает — и переносится
        на место скриптом после `load`. Переносится собственный узел, ничего
        чужого при этом не удаляется.
        """
        assert 'data-sf-move="1"' in страница
...
class TestНавигация:
    ...
    def test_в_разметке_сервера_нет_чужих_узлов(self, витрина):
        _, тело = взять(витрина, "/")
        assert "data-sf-nav=" not in self._шапка(тело), (
            "пункт в разметке сервера — это расхождение гидратации")

    def test_скрипт_ждёт_конца_гидратации(self, витрина):
        _, тело = взять(витрина, "/")
        assert 'data-sf-nav-script="1"' in тело
        # Ожидание `load`, а не немедленный запуск: скрипт стоит перед
        # </body> и без ожидания выполняется раньше гидратации.
        assert "addEventListener('load'" in тело
        assert "readyState==='complete'" in тело

    def test_скрипт_ничего_не_удаляет(self, витрина):
        """Узлы React удалять нельзя: он потом не найдёт их родителя.

        Отсюда была вторая ошибка на публичной странице —
        `TypeError: Cannot read properties of null (reading 'removeChild')`.
        """
        _, тело = взять(витрина, "/")
        скрипт = тело[тело.index('data-sf-nav-script'):]
        скрипт = скрипт[:скрипт.index("</script>")]
        for запрет in ("removeChild", "remove()", "innerHTML", "replaceChild"):
            assert запрет not in скрипт, f"скрипт разрушает чужой DOM: {запрет}"

    def test_маршруты_объявлены_скриптом(self, витрина):
        _, тело = взять(витрина, "/")
        for адрес in self.ЗАПРОШЕННЫЕ:
            assert адрес in тело, f"маршрут {адрес} не попал в навигацию"

    def test_порядок_из_профиля_домена(self, витрина):
        _, тело = взять(витрина, "/")
        скрипт = тело[тело.index('data-sf-nav-script'):]
        адреса = re.findall(r'\["(/[a-z/]+)",\s*"', скрипт[:скрипт.index("</script>")])
        # editorial-guide ведёт подборками, а не расписанием.
        assert адреса[0] == "/collections/"

    def test_полоса_разделов_для_узкой_ширины(self, витрина):
        """Нижняя панель витрины рассчитана на пять пунктов, не на восемь."""
        _, тело = взять(витрина, "/")
        assert "sf-mobnav" in тело
        assert "@media(max-width:700px)" in тело
