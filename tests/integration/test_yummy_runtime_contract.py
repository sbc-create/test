...
class TestНавигация:
    ...
    def test_стиль_с_меткой_не_отключает_вставку(self):
        """Признак «пункты уже есть» — ссылка, а не голая метка.

        Стиль, прячущий пункты на узкой ширине, содержит тот же
        `data-sf-nav="1"`. Пока признаком была метка, стиль попадал в страницу
        раньше вставки, и вставка молча выключалась целиком: меню исчезало,
        а ошибок не было ни одной.
        """
        источник = (ХОСТ / "yummy-frontend.py").read_text(encoding="utf-8")
        assert 'НАВ_ПРИЗНАК = b\'class="portal-nav-link" data-sf-nav="1"\'' in источник
        assert "if self.НАВ_ПРИЗНАК not in тело:" in источник
