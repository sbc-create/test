...
ПРИЁМКА = КОРЕНЬ / "automation" / "host" / "yummy-indexing-verify.sh"


def _приёмка() -> str:
    return ПРИЁМКА.read_text(encoding="utf-8")


class TestСценарийПриёмки:
    """Один и тот же сценарий снимает baseline и доказывает результат.

    Отдельная «проверялка результата» рядом с «проверялкой baseline»
    разошлась бы с ней молча, и расхождение заметили бы позже всех.
    """

    def test_сценарий_существует(self):
        assert ПРИЁМКА.is_file()

    def test_не_мутирует(self):
        """Приёмка обязана быть read-only: её запускают и до, и после."""
        т = _приёмка()
        for опасное in ("systemctl restart", "systemctl stop", "install -m",
                        "docker compose up", "rm -", "mv ", "cp -"):
            assert опасное not in т, f"приёмка выполняет мутацию: {опасное}"

    def test_покрывает_требования_владельца(self):
        т = _приёмка()
        for требование in ("Basic Auth", "X-Robots-Tag", "meta", "Disallow",
                           "sitemap", "canonical", "404"):
            assert требование in т, f"не проверяется требование: {требование}"

    def test_проверяет_соседние_площадки(self):
        т = _приёмка()
        assert "yummyani.org" in т and "yummyani.biz" in т

    def test_нет_молчаливого_пропуска_в_sitemap(self):
        """Пропущенная проверка выглядит как пройденная — это запрещено."""
        assert "проверить не на чем" in _приёмка()

    def test_код_возврата_отражает_итог(self):
        assert '[ "${FAIL_COUNT}" -eq 0 ]' in _приёмка(), (
            "без ненулевого кода возврата провал приёмки останется незамеченным")
