...
class TestБарьерВстроенВКанарейку:
    """Проверка стоит в двух местах, и второе важнее первого.

    Между отрисовкой и объявлением результата проходят часы. Заявка, законная
    в начале, к концу может оказаться вытесненной — и именно там раньше
    происходила подмена production вытесненной ревизией.
    """

    def _текст(self):
        from pathlib import Path
        return (Path(__file__).resolve().parents[2]
                / "automation" / "deploy" / "lords-deployctl").read_text(encoding="utf-8")

    def test_канарейка_спрашивает_барьер_дважды(self):
        текст = self._текст()
        assert текст.count('_барьер("') == 2
        assert '_барьер("pre-render")' in текст
        assert '_барьер("pre-switch")' in текст

    def test_вытеснение_до_отрисовки_не_трогает_витрину(self):
        текст = self._текст()
        assert "заявка вытеснена до отрисовки: витрина не тронута" in текст

    def test_вытеснение_во_время_отрисовки_не_принимает_результат(self):
        текст = self._текст()
        assert "заявка вытеснена во время отрисовки: результат не принят" in текст

    def test_поколение_и_отпечаток_приходят_аргументами(self):
        текст = self._текст()
