...
class TestЭкранГотовности:
    ...
    def test_разрешённый_источник_и_запреты_названы(self, app):
        """Раздел показывает границу разрешения, а не только его наличие.

        Раньше здесь проверялось, что причина отсутствия оценок названа. После
        решения владельца от 2026-09-06 отсутствия нет — есть точная граница:
        разрешён фид, прямые обращения к сайтам запрещены. Проверять надо её.
        """
        html = app.handle("GET", "/admin/readiness", cookies=войти(app)).html
        ...
        assert "запрещ" in html, "запрет на прямое обращение обязан быть виден"
