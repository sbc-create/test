...
def test_остаточный_долг_изоляции_только_внутри_стилей_и_посчитан(текст):
    """Брендовые надписи соседа остались только в комментариях стиля.

    Их правка меняет отдаваемые браузеру байты, поэтому она принадлежит шагу
    переименования пространства классов, у которого своя проверка диффа. Долг
    посчитан здесь, чтобы он не растворился: расти ему нельзя.
    """
    долг = [n for n, s in enumerate(текст.splitlines(), 1)
            if "Zona" in s or "Lords" in s]
    assert len(долг) <= 2, f"долг изоляции вырос: строки {долг}"
    for н in долг:
        строка = текст.splitlines()[н - 1]
        assert строка.lstrip().startswith(("/*", "и коммит")) or "/*" in строка, строка
...
def test_запись_проверки_рендера_говорит_о_полном_совпадении():
    ...
    # Запись обязана относиться к тому файлу, который лежит в дереве.
    import hashlib
    assert з["isolated_sha256"] == hashlib.sha256(РАНТАЙМ.read_bytes()).hexdigest(), (
        "запись эквивалентности устарела: пересними её после правки рантайма")
