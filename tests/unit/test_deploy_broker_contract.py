...
class TestКанарейкаИдётШтатнымЮнитом:
    ...
    def test_запускается_служба_а_не_сценарий(self, помощник):
        ...
        # Смысл прежний: credentials приходят только через systemd, поэтому
        # сценарий не запускается подпроцессом. Устройство новее: у каждого
        # прогона свой экземпляр юнита вместо общего.
        assert '"systemctl", "start", юнит' in текст
    ...
    def test_окружение_прогона_удаляется_на_любом_пути(self, помощник):
        """Смысл прежний: после себя не остаётся ничего.

        Общего drop-in больше нет вовсе — вместо него неизменяемый файл
        окружения одного прогона, и он снимается в finally.
        """
        import ast
        дерево = ast.parse(Path(помощник.__file__).read_text(encoding="utf-8"))
        нашли = False
        for узел in ast.walk(дерево):
            if isinstance(узел, ast.Try) and узел.finalbody:
                ...
                if "файл_окружения.unlink" in текст:
                    нашли = True
...
class TestКанарейкаНеПрисоединяетсяКЧужомуПрогону:
    ...
    def test_присоединиться_физически_не_к_чему(self, помощник):
        """Дефект 41 снят устройством, а не проверкой перед стартом.

        Пока канарейка шла через общий юнит, `systemctl start` присоединялся к
        уже идущему прогону. Теперь имя экземпляра содержит витрину и метку
        времени: занятого экземпляра с таким именем не существует.
        """
        import ast
        дерево = ast.parse(Path(помощник.__file__).read_text(encoding="utf-8"))
        ...
        assert "ЮНИТ_ПРОГОНА.format(instance=прогон)" in тело
        assert "time.strftime" in тело, "имя экземпляра не уникально"
