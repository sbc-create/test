...
def test_генератор_не_трогает_пакет_витрины_в_репозитории(стенд, tmp_path):
    """Набор стенда пишется в корень стенда, а не в репозиторий.

    Проверка написана, когда наборов содержимого ещё не было и правка пакета
    отсюда столкнулась бы с поставкой TEMPLATES. Поставка пришла (handoff 032
    закрыт), и `content_package_ref` у живой витрины теперь заполнен — но
    правило осталось прежним: генератор стенда в репозиторий не пишет.

    Поэтому сравнивается содержимое пакета до и после запуска генератора, а не
    его конкретное значение: значение принадлежит TEMPLATES и меняется без нас.
    """
    путь = РЕПО / "sites" / "site-a" / "package.yaml"
    до = путь.read_bytes()
    fixture_catalog.записать(tmp_path, РЕПО, "site-a", 5)
    assert путь.read_bytes() == до, "генератор стенда изменил пакет живой витрины"
    # Набор лежит в стенде, а не рядом с пакетом витрины.
    assert (tmp_path / "sites" / "site-a" / "content" / "catalog.json").is_file()
