...
class TestProfileProperties:
    ...
    def test_real_inventories_hold_only_what_the_owner_supplied(self):
        ...
        Собственные витрины стоят в списке отдельным блоком и по другой
        причине: без записи guard отклоняет запрос к домену, и приёмка
        выкаченного релиза остаётся словом вместо измерения. Это не расширение
        периметра интеграций — перечислены собственные домены, только читающие
        методы, ни одного wildcard. Каждый внесён названным заданием владельца
        и обязан быть назван здесь: список растёт только вместе с этим тестом,
        ...
        assert unattended.network_hosts() == {
            ...
            # Задание ZONA-SLIDER-CROP-DATE (2026-09-21): приёмка витрины Zona.
            "zonafilm.space",
            # Задание SITE_RELEASE_FULL_CYCLE (2026-09-22): домен нового контура
            # передан владельцем в исходных данных, делегация подтверждена
            # серверами зоны .cc и четырьмя публичными резолверами.
            "zonafilm.cc",
        }
