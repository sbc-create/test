...
class TestProfileProperties:
    ...
    def test_real_inventories_hold_only_what_the_owner_supplied(self):
        ...
        расширение по инициативе агента прямо запрещено.

        Сетевой allowlist не пуст, и каждая группа в нём названа заданием
        владельца. Проверяется точный состав, а не вхождение: незамеченная
        лишняя строка здесь — это открытый наружу канал. Поэтому список ниже
        расширяется только вместе со ссылкой на задание, которым хост внесён.
        """
        assert unattended.inventory_hosts() == set()
        assert unattended.inventory_zones() == set()
        assert unattended.network_hosts() == {
            # Задание «полная автоматизация Яндекс.Метрики и Вебмастера»
            # (2026-08-23): API и официальная документация.
            "api-metrika.yandex.net",
            "api.webmaster.yandex.net",
            "yandex.ru",
            "yandex.com",
            # Задание «центральный Secret Hub» (2026-08-25, D88): read-only
            # проверка выданного токена перед сохранением.
            "public-api.cdnvideohub.com",
            # Задание «ночной автономный SEO-цикл — шесть LIVE-сайтов»
            # (2026-08-29): чтение собственных витрин, первоисточники правил и
            # подтверждение фактической загрузки счётчика. Блок жил в ветке
            # claude/seo-operator-live-11 и в main не попадал.
            "yummyani.site",
            "yummyani.org",
            "yummyani.biz",
            "lordfilm47.space",
            ...
            "schema.org",
            "mc.yandex.ru",
            # Задание SEO-PREINDEX-READINESS-LOCKED-01 (2026-09-15): ещё три
            # витрины того же портфеля, названные владельцем в тексте задания.
            # amd.online в список намеренно не внесён — чужой сайт.
            "zonafilm.space",
            "animedia.icu",
            "animedia.space",
        }

    def test_third_party_reference_host_never_enters_the_network_allowlist(self):
        """amd.online назван в задании как ориентир, а не как наша витрина.

        Соблазн завести его рядом с собственными доменами существует ровно до
        первого запроса: обходить сайт стороннего владельца оператору нечем и
        незачем.
        """
        assert "amd.online" not in unattended.network_hosts()
