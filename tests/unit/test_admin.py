...
def test_панель_не_показывает_недоступных_действий(app):
    """Проверяются формы, а не слова.

    Раньше здесь стояло «слова „Настройки“ на странице нет». После появления
    отдельного раздела настроек, открытого на чтение всем, ссылка в меню есть
    у любого вошедшего — и проверка по слову стала ловить меню вместо кнопки.
    Недоступное действие — это отправляемая форма, её и проверяем.
    """
    cookies, _ = войти(app, READONLY)
    r = app.handle("GET", f"/admin/sites/{SITE}", cookies=cookies)
    assert "Задание" not in r.html
    assert f'action="/admin/sites/{SITE}/settings"' not in r.html
    assert f'action="/admin/sites/{SITE}/jobs"' not in r.html
