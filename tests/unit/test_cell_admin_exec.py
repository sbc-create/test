...
def test_несовпадение_digest_отменяет_активацию(репозиторий_сайта):
    ...
    Проверяется на сухом прогоне: подтверждённая активация закрыта целиком
    (следующий тест), и отказ по digest наступал бы там уже после отказа по
    самому пути — то есть не проверялся бы вовсе.
    """
    head = _git(репозиторий_сайта, "rev-parse", "HEAD")
    with pytest.raises(admin_exec.ExecutorRefused) as ош:
        admin_exec.активировать(ЗАРЕГИСТРИРОВАННЫЙ, commit=head, dry_run=True,
                                expect_digest="sha256:" + "0" * 64)
    assert "CI" in str(ош.value)


def test_подтверждённая_активация_закрыта(репозиторий_сайта):
    """Этот путь исполнял сценарий репозитория от root и не спрашивал GitHub.

    Пока он оставался проходимым, выкладка без доказанного происхождения
    требовала одного лишнего флага, а не обхода проверки.
    """
    head = _git(репозиторий_сайта, "rev-parse", "HEAD")
    with pytest.raises(admin_exec.ExecutorRefused) as ош:
        admin_exec.активировать(ЗАРЕГИСТРИРОВАННЫЙ, commit=head, dry_run=False)
    assert "закрыта" in str(ош.value)
    assert "очередь" in str(ош.value)
