...
class TestЛенты:
    ...
    def test_пост_рисуется_с_источником_и_датой(self, база):
        # Момент берётся из фикстуры, а не из календаря. Без этого тест жил до
        # первого дня, когда новости фикстуры протухнут, и падал сам по себе:
        # соседний тест как раз и доказывает, что к 20 сентября лента пуста.
        # Дата в разметке сверяется с фикстурной, так что подстановка «сейчас»
        # ничего не ослабляет — она лишь возвращает тесту его точку отсчёта.
        разметка = ВИД.пост(СВЯЗЬ.посты(база, "news", сейчас=ФИКСТУРА.СЕЙЧАС)[0])
