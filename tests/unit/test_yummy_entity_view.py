...
class TestЛенты:
    ...
    #:
    #: Значение берётся у фикстуры, а не переписывается рядом. Скопированная
    #: константа расходится с источником молча: фикстуру сдвинут, а копия
    #: останется, и тест начнёт проверять несуществующий момент.
    СЕЙЧАС = ФИКСТУРА.СЕЙЧАС
    ...
    def test_черновик_не_публикуется(self, база):
        # Часы фикстуры обязательны: без них список пустеет по SLA свежести,
        # и проверка зеленела бы даже с выключенным отбором по статусу —
        # доказывая не то, что черновик скрыт, а то, что фикстура устарела.
        база.execute("UPDATE editorial_post SET status='draft'")
        база.commit()
        assert СВЯЗЬ.посты(база, "news", сейчас=self.СЕЙЧАС) == []
    ...
    def test_пост_рисуется_с_источником_и_датой(self, база):
        разметка = ВИД.пост(СВЯЗЬ.посты(база, "news", сейчас=self.СЕЙЧАС)[0])
        assert "fixture" in разметка and "2026-09-10" in разметка

    def test_поверхность_с_SLA_читается_часами_фикстуры(self, база):
        """Часы реального времени на замороженной фикстуре — ложное падение.

        У материалов есть SLA свежести: не обновлялся 48 часов — не
        показывается. Фикстура заморожена на `ФИКСТУРА.СЕЙЧАС`, поэтому
        вызов без явных часов через двое суток после этой даты возвращает
        пустой список — и следующий тест падал на `[0]` по IndexError.
        Падало не представление и не контур: истекло время.

        Проверка закрепляет оба исхода, чтобы причина была видна в наборе, а
        не восстанавливалась по трассировке.
        """
        далеко = "2099-01-01T00:00:00+00:00"
        assert СВЯЗЬ.посты(база, "news", сейчас=далеко) == []
        assert len(СВЯЗЬ.посты(база, "news", сейчас=self.СЕЙЧАС)) == 3
...
class TestПервыйЭкран:
    ...
    def test_редакционная_открывается_без_поиска(self):
        """У редакционной витрины нет формы поиска, но ряд разделов есть.

        Голова и шапка копируются у приложения без скриптов, поэтому
        выпадающий список «Аниме» на собственной странице не открывается, а
        три из четырёх разделов живут именно в нём. Без ряда с такой
        страницы некуда уйти.
        """
        э = self.экран("yummyani.biz")
        assert "sf-tabs" in э and "sf-find" not in э

    def test_три_композиции_различны(self):
        """Различие осталось: форма поиска и порядок разделов."""
