...
class TestРеестр:
    ...
    def test_ни_один_источник_не_разрешён_по_умолчанию(self, реестр):
        решение = реестр.resolve(REPO)
        assert решение.authorized == (), "разрешение источника — решение владельца"
...
class TestМаршрутИЭкран:
    def test_состояние_источников_видно_через_api(self):
        ...
        assert ответ.body["authorized"] == []
        assert ответ.body["known"]
        assert ответ.body["blocker"], "причина отсутствия оценок обязана быть названа"

    def test_в_поставке_нет_разрешённых_источников(self, реестр):
        """Проверяется разобранный реестр, а не текст файла.

        Поиск строки по тексту ловил бы объяснение в комментарии — там слово
        «granted» стоит как раз в описании того, как разрешение выдаётся.
        """
        решение = реестр.resolve(REPO)
        assert решение.authorized == ()
        assert all(и["authorization"]["status"] != "granted" for и in решение.known)
        assert решение.blocker
