...
class TestЗамерСдвигаПривязанКДвижку:
    """`LORDS-CLS-FIREFOX-CANNOT-MEASURE-43`: приёмка Firefox падала всегда.

    Запись `layout-shift` реализована только в Chromium. Проверено на самих
    движках: `PerformanceObserver.supportedEntryTypes` в Firefox содержит
    `largest-contentful-paint`, но не содержит `layout-shift`. Наблюдатель там
    не подключается, `window.__cls` остаётся нулём, и самопроверка наблюдателя
    честно падала по таймауту 10 с.

    Из-за этого канарейка lords-02 объявила провал приёмки и откатилась —
    хотя провалом был не шаблон, а попытка измерить в движке то, чего он не
    умеет. Chromium ту же приёмку проходил.

    Ноль вместо измерения не подставляется: неизмеренное называется
    неизмеренным с причиной. Это то же правило, по которому недоступный
    источник сообщает метрику как неизмеренную, а не как 0.
    """

    СПЕК = Path(__file__).resolve().parents[2] / "tests" / "e2e-release" / "live-acceptance.spec.js"

    def test_замер_сдвига_под_проверкой_возможностей(self):
        текст = self.СПЕК.read_text(encoding="utf-8")
        assert "supportedEntryTypes" in текст, (
            "замер сдвига не спрашивает движок о поддержке: Firefox снова будет "
            "проваливать приёмку целиком")
        assert "наблюдаемСдвиг" in текст

    def test_неизмеренный_сдвиг_не_равен_нулю(self):
        текст = self.СПЕК.read_text(encoding="utf-8")
        assert "cls_unmeasured_reason" in текст, (
            "неизмеренный сдвиг попадёт в отчёт нулём и будет прочитан как "
            "«сдвига нет»")

    def test_в_chromium_проверка_остаётся_строгой(self):
        """Ослаблять Chromium нельзя: там сдвиг измерим и обязан проверяться."""
        текст = self.СПЕК.read_text(encoding="utf-8")
        assert "toBeLessThanOrEqual(0.1)" in текст, "утверждение о бюджете сдвига исчезло"
        assert "наблюдатель сдвига не сработал" in текст, "самопроверка наблюдателя исчезла"


class TestОднаЗакреплённаяОснастка:
    """`LORDS-THREE-PINS-DRIFT-44`: закреплённый путь жил в трёх местах.

    Он был вписан руками в drop-in службы, в константу ОСНАСТКА помощника и в
    умолчание --repo публичной проверки. Три пина расходились независимо, и за
    одну ночь это стоило трёх разных отказов:

      * помощник исполнялся из релиза 31014bb4 и полтора часа падал на
        TypeError, исправленном тремя коммитами выше;
      * исправление хранения не действовало, потому что служба читала релиз
        41031da0;
      * приёмка канарейки исполняла спеку из /home/claude/wt-release-03, куда
        исправление теста Firefox не попадало вовсе.

    Общая черта у всех трёх — исправление лежало в git и не исполнялось ни
    разу. Указатель `lords-tooling/current` делает «что сейчас выложено» одним
    местом, а его смену — атомарной.
    """

    def test_помощник_не_содержит_вписанного_релиза(self):
        текст = (ИСТОЧНИК / "lords-deployctl").read_text(encoding="utf-8")
        начало = текст.index("def _действующая_оснастка")
        конец = текст.index("ОСНАСТКА = _действующая_оснастка()")
        тело = текст[начало:конец]
        assert "lords-tooling/current" in тело
        assert "releases/7982913" not in текст, "вписанный релиз вернулся в помощник"

    def test_отсутствие_указателя_это_отказ(self):
        """Без указателя выкладка не идёт: путь наугад выложит неизвестное."""
        текст = (ИСТОЧНИК / "lords-deployctl").read_text(encoding="utf-8")
        assert "оснастка не опубликована" in текст

    def test_приёмка_берёт_спеку_из_оснастки(self):
        текст = (ИСТОЧНИК / "lords-public-check.py").read_text(encoding="utf-8")
        assert 'default="/srv/site-factory/lords-tooling/current"' in текст
        # Упоминание в комментарии законно — там записана причина правки.
        # Проверяется именно умолчание аргумента.
        import re as _re
        умолчания = _re.findall(r'--repo["\']\s*,\s*default=(["\'][^"\']+["\'])', текст)
        assert умолчания and "wt-release-03" not in умолчания[0], (
            f"приёмка снова смотрит в чужое дерево: {умолчания}")

    def test_публикатор_ведёт_указатель(self):
        текст = (Path(__file__).resolve().parents[2] / "automation" / "host"
                 / "lords-tooling-release.py").read_text(encoding="utf-8")
        assert 'БАЗА / "current"' in текст
        assert "os.replace(временный, указатель)" in текст, "смена указателя не атомарна"


class TestВисячаяСсылкаНеТочкаОтката:
    """Инструмент восстановления сам считал висячую ссылку исправной.

    Первая версия возвращала УЖЕ_ЕСТЬ по одному факту существования
    символической ссылки — и потому объявляла целыми ровно те витрины, где
    хранение удалило цель. На 2026-09-10 так были «исправны» lords-01 и
    lords-03: ссылки указывали на bfdbdbe8ebc5 и a7b5da502eca, удалённые
    хранением.
    """

    ИНСТРУМЕНТ = (Path(__file__).resolve().parents[2] / "automation" / "host"
                  / "lords-backfill-previous.py")

    def test_целость_цели_проверяется(self):
        текст = self.ИНСТРУМЕНТ.read_text(encoding="utf-8")
        assert "цель_ссылки.is_dir()" in текст, (
            "висячая ссылка снова будет принята за точку отката")

    def test_висячая_ссылка_называется_в_отчёте(self):
        текст = self.ИНСТРУМЕНТ.read_text(encoding="utf-8")
        assert '"dangling"' in текст, "висячая ссылка исчезнет из отчёта молча"
