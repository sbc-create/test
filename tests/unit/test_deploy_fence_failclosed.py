...
class TestЗаморозкаАртефактаНаСемейство:
    """`LORDS-FAMILY-ARTIFACT-DRIFT-41`: построитель всегда брал HEAD.

    Порядок выкладки — lords-02, затем lords-01, затем lords-03 — тем же
    замороженным артефактом. Но артефакт собирался из `rev-parse HEAD`, а между
    витринами семейства HEAD успевает уйти вперёд: за время отрисовки канарейки
    в это дерево легло несколько коммитов. Вторая витрина получила бы другой
    шаблон, чем первая, и «тот же артефакт» стало бы неправдой.

    Кэш по имени файла тут не спасает: имя считается от ревизии, а ревизия
    другая. Поэтому ревизия обязана задаваться явно.
    """

    def test_построитель_принимает_ревизию_явно(self):
        текст = (ИСТОЧНИК / "lords-deploy-request.py").read_text(encoding="utf-8")
        assert '"--revision"' in текст, (
            "нельзя закрепить ревизию: витрины семейства разъедутся по шаблонам")

    def test_ревизия_проверяется_на_полную_форму(self):
        текст = (ИСТОЧНИК / "lords-deploy-request.py").read_text(encoding="utf-8")
        assert "ХЕКС40" in текст or "{40}" in текст, (
            "сокращённая ревизия принимается: доказательством она не является")

    def test_артефакт_переиспользуется_а_не_пересобирается(self):
        """Один и тот же артефакт между доменами семейства не пересобирается."""
        текст = (ИСТОЧНИК / "lords-deploy-request.py").read_text(encoding="utf-8")
        начало = текст.index("def собрать_артефакт")
        тело = текст[начало:текст.index("\ndef ", начало + 10)]
        assert "архив.is_file()" in тело and "return архив" in тело
