...
class TestScaffoldDeclaresItsOwnTheme:
    """Новый шаблон с новой темой обязан создаваться, а не отвергаться.

    Дефект нашёлся на первой же попытке завести шаблон с собственной палитрой.
    Порядок в `scaffold()` таков: сначала `validate_manifest`, потом записи. А
    валидатор требует, чтобы тема была уже объявлена в blueprint — и потому
    отвергает ровно тот случай, ради которого scaffold написан: третья запись
    в его собственном описании гласит «themes — если тема новая».

    Получалась замкнутая пара: тему нельзя объявить, не создав шаблон, и
    нельзя создать шаблон, не объявив тему. Снаружи это выглядело как отказ без
    единой правки — команда печатала предупреждение и ничего не делала.

    Проверка требования при этом остаётся нужной: шаблон, ссылающийся на тему,
    которой никто не объявит, действительно сломает валидацию пакета. Поэтому
    послабление ровно одно — тема, которую объявит эта же операция.
    """

    def manifest_with_new_theme(self) -> dict:
        manifest = copy.deepcopy(scaffold_mod.example_manifest("themed-example"))
        manifest["theme"]["name"] = "brand_new_theme"
        return manifest

    def test_шаблон_с_новой_темой_создаётся(self, sandbox):
        result = scaffold_mod.scaffold(self.manifest_with_new_theme(), root=sandbox)
        assert result.ok, [str(p) for p in result.problems]
        created = sandbox / contract.PROFILE_DIR / "themed-example.yaml"
        assert created.is_file(), "профиль не создан"

    def test_новая_тема_объявляется_в_blueprint(self, sandbox):
        scaffold_mod.scaffold(self.manifest_with_new_theme(), root=sandbox)
        blueprint = yaml.safe_load((sandbox / contract.BLUEPRINT_FILE).read_text(encoding="utf-8"))
        assert "brand_new_theme" in (blueprint.get("themes") or []), (
            "тема не попала в blueprint — пакет сайта с ней будет отклонён валидацией")

    def test_новая_тема_попадает_в_схему_пакета(self, sandbox):
        scaffold_mod.scaffold(self.manifest_with_new_theme(), root=sandbox)
        schema = json.loads(
            (sandbox / "schemas" / contract.PACKAGE_SCHEMA_FILE).read_text(encoding="utf-8"))
        text = json.dumps(schema, ensure_ascii=False)
        assert "brand_new_theme" in text, "тема не попала в перечисление схемы пакета"

    def test_проверка_существующего_шаблона_не_ослаблена(self, sandbox):
        # Послабление касается только создания. Уже существующий шаблон,
        # ссылающийся на тему, которой никто не объявил, обязан отвергаться —
        # иначе проверка перестала бы что-либо охранять.
        manifest = self.manifest_with_new_theme()
        problems = contract.validate_manifest(manifest, root=sandbox, where="манифест")
        assert any("не объявлена" in p.message for p in problems), (
            "проверка объявленности темы исчезла вовсе")

    def test_сухой_прогон_ничего_не_пишет(self, sandbox):
        before = sorted(p.name for p in (sandbox / contract.PROFILE_DIR).glob("*.yaml"))
        result = scaffold_mod.scaffold(self.manifest_with_new_theme(), root=sandbox, dry_run=True)
        assert result.ok, [str(p) for p in result.problems]
        after = sorted(p.name for p in (sandbox / contract.PROFILE_DIR).glob("*.yaml"))
        assert before == after
