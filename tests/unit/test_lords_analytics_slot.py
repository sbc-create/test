...
class TestSafeDefaults:
    ...
    def test_module_still_parses(self):
        # Разбор падает на сломанном модуле, и зелёный результат кое-что
        # значит. Но пустой файл разбирается тоже, а именно так выглядит
        # потерянный при правке модуль.
        дерево = ast.parse(SOURCE)
        assert дерево.body, "модуль разобрался, но пуст"
