...
@pytest.fixture(scope="module")
def замер() -> dict:
    файл = ДОК / "40-probe-fill" / "PROBE_fill_animedia_icu.json"
...
def test_ширина_плитки_карусели_считается_от_окна_прокрутки(стиль):
    ...
    Проверяется свойство, а не число. Прежде здесь стояла точная формула
    `calc((100cqw - 198px)/7)`: она закрепляла и зазор, и «ровно семь
    карточек». После ANIMEDIA-TEMPLATE-FIX-01 лента занимает ширину основного
    содержимого, а число видимых карточек подстраивается под место — семь на
    широком экране, пять ниже 1280. Дефект, который тест держит закрытым,
    остался прежним: `100%` внутри ленты считается от ВИДИМОЙ части, а не от
    прокручиваемой, и плитки уезжают за край.
    """
    import re as _re
    assert "container-type:inline-size" in стиль
    формулы = _re.findall(r"calc\(\(100cqw - \d+px\)/(\d+)\)", стиль)
    assert формулы, "ширина плитки больше не считается от окна прокрутки"
    # Число видимых карточек подстраивается: одной ступени мало.
    assert len(set(формулы)) >= 2, формулы
    assert not _re.search(r"calc\(\(100% - \d+px\)/\d+\)", стиль)
...
def test_карточка_одинакова_на_всех_маршрутах(замер):
    ...
    """
    # Страницы произведения и серии сюда не входят: их нижний блок у оригинала
    # не сетка постеров, а строка с миниатюрой, и его геометрия проверяется
    # отдельно — ниже.
    ожидания = {768: 118, 1440: 163, 1920: 163}
    for маршрут in ("catalog", "search", "collection_detail"):
        for ш, эт in ожидания.items():
...
def test_объём_главной_сопоставим_с_оригиналом(замер, эталон):
    """Главная показывала две ленты при двенадцати, обеспеченных данными.

    Это и есть «пустая витрина»: страница выходила втрое короче оригинала не
    из-за отступов, а из-за того, что ленты не рисовались. Порог мягкий —
    часть блоков оригинала (дневники, рецензии, комментарии) требует данных и
    аутентификации, которых у витрины нет, и выдумывать их нельзя.
    """
    for ш, vp in ((1440, "1440x900"), (390, "390x844")):
        эт = эталон["home"]["viewports"][vp]["metrics"]["bodyH"]
        наш = замер[("home", ш)]["body_h"]
        assert наш >= эт * 0.6, f"{ш}: главная {наш} при эталонных {эт}"


def test_на_главной_нет_повторяющихся_карточек(замер):
    for ш in (390, 1440):
        c = замер[("home", ш)]
        assert c["cards_count"] >= 24, ш


def test_нижний_блок_строкой_совпал_с_эталоном():
    """Измерено на эталоне 1440: три колонки, карточка 436x120, миниатюра ~100.

    Прежде здесь была семиколоночная сетка постеров: не «другой размер», а
    другая композиция.
    """
    з = json.loads((ДОК / "29-card-final" / "CARD_AUDIT.json").read_text(encoding="utf-8"))
    по_ширине = {}
    for c in з["cells"]:
        if c["route"] != "title":
            continue
        for блок, с in c["blocks"].items():
            if "related-row" in с["variants"]:
                по_ширине[c["width"]] = с["sample"][0]
    assert set(по_ширине) == {390, 768, 1440}, по_ширине
    широкая = по_ширине[1440]
    assert abs(широкая["w"] - 436) <= 4, широкая
    assert abs(широкая["h"] - 120) <= 4, широкая
    # На телефоне карточка растягивается в одну колонку — тоже как у эталона.
    assert по_ширине[390]["w"] > 300
...
def test_запись_проверки_слайдера_зелёная():
    файл = ДОК / "31-slider-final" / "SLIDER_CHECK.json"
