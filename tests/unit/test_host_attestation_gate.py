...
def test_обязательность_нельзя_снять_в_самом_свидетельстве():
    """Поле `required` пишет тот, кто выпускает документ, — решают не по нему.

    Иначе провалившуюся проверку достаточно объявить необязательной, и ворота
    её пропустят: свидетельство само разрешило бы себе не быть полным.
    Обязательность берётся из контракта.
    """
    документ = свидетельство(провалить={"ledger.backup": "FAIL"})
    for п in документ["checks"]:
        if п["check_id"] == "ledger.backup":
            п["required"] = False
    документ["verdict"] = "PASS"
    ...
    with pytest.raises(G.РелизЗаблокирован) as ош:
        G.проверить(документ, candidate_sha=SHA, сейчас_utc=МОМЕНТ)
    assert ош.value.код == "HOST_ATTESTATION_CHECK_FAILED"
    assert "ledger.backup=FAIL" in ош.value.detail
