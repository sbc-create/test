...
#: проверка ищет не наличие обработчиков вообще, а появление чужого. Пуст
#: намеренно: снятие не загрузившегося постера — делегированный слушатель в
#: `/assets/app.js` по классу тега, а не инлайновый `onerror` в разметке (см.
#: tests/unit/test_json_ld_cannot_break_out.py — инлайновый атрибут события
#: рядом с данными поставщика неотличим от признака пробитой экранировки).
СВОИ_ОБРАБОТЧИКИ: set[str] = set()
