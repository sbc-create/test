...
#: Канонический модуль сообщества: выпуск и байты, закреплённые всеми тремя
#: витринами семейства в их `pins.lock.json`.
СООБЩЕСТВО_ВЫПУСК = "20260923T190000Z-community-2-2-12"
СООБЩЕСТВО_SHA = "91ba82d37d467abb3cd06505d7c98eb99b32b7fb5eb0ceeeb3ed4fcded662fae"
СООБЩЕСТВО_ПУТИ = (
    Path("/srv/lords/.frontend/releases") / СООБЩЕСТВО_ВЫПУСК / "community.py",
)


def канонический_модуль_сообщества() -> Path | None:
    """Путь к каноническому `community.py` или None.

    Модуль принадлежит другому выпуску и в шаблон не копируется: закрепление
    описывает происхождение чужого кода, а копия завела бы вторую его версию.
    Байты сверяются с закреплённой суммой — проверка на подменённом модуле
    доказывала бы не тот контракт.
    """
    import hashlib
    for путь in СООБЩЕСТВО_ПУТИ:
        if not путь.is_file():
            continue
        if hashlib.sha256(путь.read_bytes()).hexdigest() == СООБЩЕСТВО_SHA:
            return путь
    return None
