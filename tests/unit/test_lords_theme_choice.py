...
def sheet(name: str) -> str:
    """Таблица стилей профиля с объявленной второй палитрой.

    Профиль дополняется `tokens_alt` намеренно, и это не подгонка под
    проверку, а следствие слияния двух линий.

    В ветке шаблонов правила светлой и тёмной палитры выводились из основной и
    выводились всегда. Производственная линия ввела механизм явной второй
    палитры: правила появляются только там, где витрина её объявила, а
    переключатель — только там, где правила есть. Кнопка, которая ничего не
    меняет, хуже её отсутствия; таблица стилей с правилами, которые нечем
    включить, — тот же мёртвый вес, только невидимый.

    Дизайн производственной линии позже и полнее, он и принят. Ни один профиль
    направления сейчас второй палитры не объявляет, поэтому проверки контраста
    обеих палитр не на чем было бы выполнять — а требование к контрасту от
    этого никуда не делось. Профиль ниже даёт им предмет.
    """
    профиль = dict(plan_mod.load_profiles()[name])
    тема = dict(профиль.get("theme") or {})
    if "tokens_alt" not in тема:
        тема["tokens_alt"] = {"bg": "#f4f6f8", "surface": "#ffffff",
                              "text": "#141414", "muted": "#5a5a5a",
                              "border": "#d8d8d8"}
        тема["alt_scheme"] = "light"
    профиль["theme"] = тема
    return theme_mod.stylesheet(профиль)
...
class TestОбеПалитрыОбъявлены:
    ...
    def test_системная_настройка_применяется_только_по_просьбе(self):
        """Системная тема — выбор зрителя, а не умолчание витрины.

        Прежде эта проверка требовала, чтобы системная ветка не перебивала
        явный выбор, и условие `:not([data-theme=…])` её удовлетворяло. Но оно
        же означало, что системная настройка перебивает **палитру профиля** —
        то есть действует на всякой странице, где зритель ничего не выбирал.

        Следствие измерено в браузере: витрины семейства `lords_dark`
        отрисовывались со светлым полотном яркости 0,919, ровно как
        `lords_light`. Два семейства, объявленные разными, выглядели одинаково
        при любой системной настройке, и требование о явном различии не
        выполнялось никогда.

        Теперь системная палитра привязана к состоянию `data-theme="system"` —
        то есть применяется, когда зритель попросил. Умолчание витрины — её
        собственная палитра, и это её опознавательный знак.
        """
        css = sheet("lords-general")
        for scheme in ("dark", "light"):
            media = re.search(
                r"@media \(prefers-color-scheme: " + scheme + r"\)\s*\{(.+?)\n\}", css, re.S)
            assert media, f"нет ветки системной {scheme}-темы"
            assert '[data-theme="system"]' in media.group(1), (
                f"системная ветка {scheme} применяется без просьбы зрителя")
            assert 'not([data-theme=' not in media.group(1), (
                "системная ветка снова перебивает палитру профиля")
