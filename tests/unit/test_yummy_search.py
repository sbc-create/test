...
class TestПоискПоКаталогу:
    ...
    def test_ведущий_разделитель_не_сдвигает_разбор(self, ФРОНТ):
        """Пустой кусок в начале сдвигал разделители относительно слов.

        «,Жил-был-воин» собиралось обратно как «Жил,был» — строкой, которой
        нет ни в одном названии, и вариант уходил наверх впустую.
        """
        assert ФРОНТ.варианты_запроса(",Жил-был-воин")[0] == "Жил-был"
        assert ФРОНТ.варианты_запроса("Жил-был-воин,")[0] == "Жил-был"

    def test_строка_из_одних_разделителей_ничего_не_даёт(self, ФРОНТ):
        assert ФРОНТ.варианты_запроса("---") == []
        assert ФРОНТ.варианты_запроса(" , ; ") == []
