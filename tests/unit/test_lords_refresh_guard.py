...
def test_старый_шаблон_без_модулей_поиска_не_роняет_обновление():
    """Витрина со старым закреплённым шаблоном обязана продолжать обновляться.

    Копирование модулей поиска без проверки уронило прогон на двух витринах из
    трёх: `cp: cannot stat .../search.py` при `set -e` завершает сценарий, и
    каталог перестаёт обновляться вовсе. У витрины со старым шаблоном
    серверного поиска и не было — она просто остаётся без него.
    """
    текст = СЦЕНАРИЙ.read_text(encoding="utf-8")
    assert "search_ready=1" in текст, "копирование модулей поиска снова безусловно"
    assert '[ -f "${TEMPLATE_ROOT}/factory/lords/${m}" ] || search_ready=0' in текст
    assert 'if [ "$search_ready" = "1" ]; then' in текст
    # Молчать об этом нельзя: витрина без поиска должна быть видна в журнале.
    assert "нет модулей поиска" in текст


def test_рантайм_обновляется_до_ворот_сборки():
    """Исправление рантайма не должно ждать нового тайтла от поставщика.

    Блок обновления serve.py стоял после сборки, и ворота уводили выполнение
    мимо него словами «вход не изменился — сборка пропущена». Витрина с
    неизменившимся каталогом не получала исправление вовсе: измерено на
    lords-02, где серверный поиск отвечал 503 при разложенном рядом указателе,
    потому что процесс работал на прежнем файле.
    """
    текст = СЦЕНАРИЙ.read_text(encoding="utf-8")
    рантайм = текст.find("РАНТАЙМ ОБНОВЛЯЕТСЯ ДО ВОРОТ")
    ворота = текст.find("ВОРОТА ПЕРЕД СБОРКОЙ")
    assert рантайм > 0 and ворота > 0, "разделы не найдены"
    assert рантайм < ворота, (
        "обновление рантайма снова стоит после ворот — на тихом цикле оно не "
        "доедет до витрины")
    # И до пропуска сборки: `continue` ворот не должен обгонять обновление.
    пропуск = текст.find("вход не изменился — сборка пропущена")
    assert рантайм < пропуск
