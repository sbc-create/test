...
def test_кириллица_из_строки_адреса_разбирается_как_utf8(витрина):
    """WSGI отдаёт QUERY_STRING побайтовым разбором latin-1.

    «матрица» приезжает как «Ð¼Ð°ÑÑÐ¸ÑÐ°», поиск не находит ничего и выглядит
    сломанным при исправном указателе. Измерено на боевой витрине: ответ 200,
    count 0, испорченная кириллица в поле query.
    """
    модуль, _ = витрина
    испорченная = "матрица".encode("utf-8").decode("latin-1")
    статус, _, тело = _вызов(модуль, "/api/search", f"q={испорченная}")
    данные = json.loads(тело)
    assert статус.startswith("200")
    assert данные["query"] == "матрица", f"запрос не восстановлен: {данные['query']!r}"
    assert данные["count"] == 1


def test_процентное_кодирование_тоже_работает(витрина):
    модуль, _ = витрина
    статус, _, тело = _вызов(модуль, "/api/search",
                             "q=%D0%BC%D0%B0%D1%82%D1%80%D0%B8%D1%86%D0%B0")
    данные = json.loads(тело)
    assert данные["query"] == "матрица"
    assert данные["count"] == 1


def test_латиница_не_ломается_обратным_перекодированием(витрина):
    модуль, _ = витрина
    _, _, тело = _вызов(модуль, "/api/search", "q=matrix")
    assert json.loads(тело)["query"] == "matrix"
