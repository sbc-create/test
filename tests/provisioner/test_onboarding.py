...
def _provisioner(о, одобряющий=одобрить):
    return O.Provisioner(соед_наборов=о["соед"], связи=о["связи"],
                         ...
                         # Требование журнала снимается тем же рычагом, что в
                         # tests/changeset: здесь проверяется онбординг, а не
                         # доставка в журнал. Канонический журнал в эфемерном
                         # испытании недоступен, и без этого каждое применение
                         # отклонялось бы как AUDIT_LEDGER_UNAVAILABLE.
