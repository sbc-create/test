...
def test_полоса_точек_умеет_сжиматься(главная):
    """Дефект 320px: восемь точек по 44px давали строку 394px в героe 296px.

    Первая точка оказывалась при x=-37 и не нажималась вовсе: две из восьми
    были недостижимы. Документ при этом не переполнялся — строку обрезал
    родитель, и обычная проверка горизонтальной прокрутки дефект не видела.

    Лечится не брейкпойнтом, а разрешением точкам сжиматься. Здесь проверяется
    именно это свойство: жёсткая нижняя граница ширины вернула бы дефект.
    """
    m = re.search(r"\.zhero__dots\{([^}]*)\}", главная, re.S)
    assert m, "правило .zhero__dots не найдено"
    полоса = m.group(1).replace("\n", "")
    assert "max-width:calc(100% - 16px)" in полоса, (
        "полоса точек не ограничена шириной героя")
    d = re.search(r"\.zhero__dot\{([^}]*)\}", главная, re.S)
    assert d, "правило .zhero__dot не найдено"
    точка = d.group(1).replace("\n", "")
    assert "min-width:0" in точка, "точка не умеет сжиматься — строка снова вылезет"
    assert "flex:0 1 44px" in точка, "нет основы 44px, от которой идёт сжатие"
    assert "min-height:44px" in точка, "высота цели обязана остаться 44px"
