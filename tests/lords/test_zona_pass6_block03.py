...
def test_home_has_distinct_sections_not_new_duplicate(зона):
    """Главная состоит из разных секций и не повторяет раздел новинок.

    Проверка раньше перечисляла пять конкретных заголовков полок. Главную
    позже перестроили: вместо пяти однотипных полок появились первый экран,
    недельный снимок оценок, поступления, разделы, жанры и подборки. Смысл
    проверки при этом не изменился — секции обязаны быть разными и не должны
    дублировать `/new/`, — поэтому она переписана на устройство главной, а не
    на подписи, которые перестраиваются вместе с оформлением.
    """
    о = запросить(зона, "/")
    assert о.статус == 200

    классы = re.findall(r'<section class="([^"]+)"', о.тело)
    assert len(классы) >= 5, f"главная схлопнулась до {классы}"
    assert len(set(классы)) == len(классы), f"секция повторяется: {классы}"

    # Название недельного блока обязано опираться на недельный снимок, а не
    # называться «недельным» при пересчёте на каждом запросе.
    assert "Высокие оценки недели" in о.тело
    assert re.search(r'data-week-id="\d{4}-W\d{2}"', о.тело)
    assert re.search(r'data-weekly-digest="[0-9a-f]{6,}"', о.тело)

    assert "Новое в каталоге" in о.тело
    # Полка «Весь раздел» не должна уводить убранный дубль на /new/.
    assert 'data-shelf="new-all"' not in о.тело


def test_home_cards_omit_plot_snippets(зона):
    """Описание сюжета не попадает в карточку.

    Раньше сюжет искался во всём документе. На главной появился первый экран,
    где развёрнутое описание уместно и предусмотрено, поэтому проверка сужена
    до тел карточек — именно там компактность и нарушалась.
    """
    о = запросить(зона, "/")
    карточки = re.findall(r'data-testid="title-card".*?</a>', о.тело, re.S)
    assert карточки, "на главной нет ни одной карточки"
    for карточка in карточки:
        assert "Long plot synopsis" not in карточка
