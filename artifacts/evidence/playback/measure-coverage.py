...
for path in sorted(CACHE.glob("lords-*.json")):
    ...
    for t in items:
        c = classify(t, play)
        причины[c] += 1
        тип = "сериал" if t.get("is_series") else ("фильм" if t.get("is_series") is False else "не указан")
        тип_причина[(тип, c)] += 1
        g = t.get("year")
        бак = ("без года" if not isinstance(g, int) else
               "до 2000" if g < 2000 else "2000-2015" if g < 2016 else "2016+")
        год_причина[(бак, c)] += 1
        pb = t.get("playback") or {}
        агрегаторы[pb.get("aggregator") or "нет"] += 1
        if c not in примеры and not c.startswith("DESCRIPTOR_OK"):
            примеры[c] = {"name": t.get("name"), "id": t.get("external_id"),
                          "ext": t.get("external_ids"), "year": t.get("year"),
                          "type": t.get("type"), "series": t.get("is_series")}
    итог[site] = {"всего": n, "причины": dict(причины), "агрегаторы": dict(агрегаторы),
                  "примеры": примеры,
...
for site, s in итог.items():
    ...
    print(f"     из них проба «играет»:      {c.get('DESCRIPTOR_OK_PLAYABLE',0):6}  {c.get('DESCRIPTOR_OK_PLAYABLE',0)/n*100:5.1f}%")
    print(f"     из них не проверялись:      {c.get('DESCRIPTOR_OK_UNPROBED',0):6}  {c.get('DESCRIPTOR_OK_UNPROBED',0)/n*100:5.1f}%")
    ...
    if s["примеры"]:
        ...
        for k, ex in list(s["примеры"].items())[:5]:
            print(f"     {k}: {ex['name']!r} тип={ex['type']} сериал={ex['series']} год={ex['year']} ids={ex['ext']}")

Path("/tmp/coverage.json").write_text(json.dumps(итог, ensure_ascii=False, indent=2), encoding="utf-8")
