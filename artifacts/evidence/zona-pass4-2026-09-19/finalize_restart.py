...
EVIDENCE = Path("/home/claude/wt-zona-finalization-01/artifacts/evidence/zona-pass4-2026-09-19")
...
def fetch(path: str) -> tuple[int, dict, str]:
    req = urllib.request.Request(
        f"https://{DOMAIN}{path}",
        headers={"User-Agent": "zona-pass4-finalize", "Cache-Control": "no-cache"},
    )
...
def main() -> int:
    ...
    with open(LOCK, "a+", encoding="utf-8") as lockf:
        ...
        stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ") + f"-{SITE}-pass4"
