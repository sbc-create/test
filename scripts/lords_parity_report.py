...
#: Страницы, на которых карточек нет по устройству страницы. Отсутствие
#: постеров на них — не дефект кандидата, но и не доказанное сходство.
СТРАНИЦЫ_БЕЗ_КАРТОЧЕК = ("not-found",)
...
def строки(замер: dict) -> list[dict]:
    ...
    for страница, св in замер["pages"].items():
        for ш, з in sorted(св["viewports"].items(), key=lambda x: int(x[0])):
            ...
            elif страница in СТРАНИЦЫ_БЕЗ_КАРТОЧЕК:
                # Постеров нет и быть не должно. Но «должно» здесь — наше
                # суждение: снимка страницы 404 у референса не существует, и
                # применима ли к ней проверка пропорции, установить нечем.
                # Объявить ABSENT_IN_REFERENCE без доказательства нельзя.
                итог.append({"страница": страница, "вьюпорт": ширина,
                             "поле": "пропорция: постер",
                             "эталон": "нет снимка страницы у референса",
                             "наше": "карточек нет",
