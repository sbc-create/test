...
class Потребитель:
    ...
    def _применить(self, событие: dict[str, Any]) -> None:
        ...
        if тип in (РЕГИСТРАЦИЯ, ОБНОВЛЕНИЕ, АКТИВАЦИЯ):
            # Событие сообщает ФАКТ изменения, а не полное состояние.
            # Полную запись берём у Registry: собирать её из полезной
            # нагрузки события значит держать вторую схему сайта.
            запись = self.клиент._получить(f"/api/v1/sites/{сид}")[0]
            сайт = Сайт.из_записи(запись)
            активен = (сайт.environment == "production"
                       and сайт.lifecycle_state == "ACTIVE")
            with self.проекция.соед:
                self.проекция._записать_сайт(сайт, версия, активен=активен)
            return
