...
class ЗаписьСекретаИзТеста(RuntimeError):
    """Набор тестов попытался записать значение секрета в боевой путь."""


def _под_тестом() -> bool:
    """Идёт ли прогон тестов.

    Проверяются оба признака, а не один: ``PYTEST_CURRENT_TEST`` появляется
    только на время выполнения теста и отсутствует во время сбора, когда код
    модуля уже исполняется. Модуль ``pytest`` в ``sys.modules`` покрывает и сбор.
    """
    return "pytest" in sys.modules or bool(os.environ.get("PYTEST_CURRENT_TEST"))


def _запретить_боевой_путь(path: Path) -> None:
    """Запрет на запись секрета из тестов куда-либо, кроме временного каталога.

    История отказа, ради которой это существует. 2026-08-31 в 18:15:01 прогон
    ``pytest tests/unit`` от root переписал боевые файлы credential стенда Yami
    маркерами из набора тестов. Витрины продолжали отвечать 200 — контейнеры
    держали прежнее значение в памяти, — но фоновая синхронизация с этого
    момента получала от поставщика отказ авторизации, и каталог перестал
    обновляться на десять с половиной часов.

    Почему проверка здесь, а не только в ``conftest.py``. Приспособление в
    conftest действует, пока conftest загружен: запуск отдельного файла с
    другим ``rootdir``, свой ``-p no:cacheprovider``, чужая обвязка — и защиты
    нет. Это единственная функция, через которую значение секрета попадает на
    диск, и запрет в ней обойти нельзя, не переписав её.

    Почему не проверяется root. Отказ 31 августа случился от root, но право
    записи — следствие, а не причина: тест не должен писать в боевой каталог ни
    от какой учётной записи. От обычного пользователя та же попытка
    заканчивалась отказом файловой системы и выглядела случайностью среды, а не
    запретом — то есть маскировала ту же ошибку.

    Проверка **fail-closed**: любая неопределённость — отказ. Не удалось
    выяснить временный каталог, не удалось привести путь к каноническому виду —
    записи не будет. Ошибиться в сторону «не записали» здесь дёшево: тест
    упадёт с внятным сообщением. Ошибиться в другую сторону уже стоило суток
    неработающей синхронизации.
    """
    if not _под_тестом():
        return
    try:
        цель = Path(path).resolve()
        корни = [Path(tempfile.gettempdir()).resolve()]
        свой = os.environ.get("PYTEST_TMPDIR") or os.environ.get("TMPDIR")
        if свой:
            корни.append(Path(свой).resolve())
    except Exception as ошибка:  # noqa: BLE001 — fail-closed: любой сбой проверки запрещает запись
        raise ЗаписьСекретаИзТеста(
            "не удалось проверить путь записи секрета в тестовом прогоне "
            f"({ошибка!r}); запись отклонена"
        ) from ошибка
    if any(цель == корень or корень in цель.parents for корень in корни):
        return
    raise ЗаписьСекретаИзТеста(
        f"запись значения секрета в {цель} из тестового прогона отклонена: "
        "разрешён только временный каталог прогона. Подмените `directory` "
        "потребителя на `tmp_path`, как это сделано в "
        "tests/unit/test_secret_hub_reconcile.py."
    )
