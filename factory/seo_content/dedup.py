...
#: Доля вложения словесных четвёрок. Пороги выбраны измерением, а не на глаз:
#: на девяти тысячах пар принятых текстов золотого набора вложение нигде не
#: поднялось выше 0,375, а разбавленная копия — текст, в который вставили по
#: лишнему слову через каждые девять, чтобы разорвать цепочку совпадений и
#: опустить пятиграммы под их порог, — даёт 0,70. Пороги поставлены в этот
#: разрыв.
ПОРОГ_ВЛОЖЕНИЯ_БЛОК = 0.60
ПОРОГ_ВЛОЖЕНИЯ_РАЗБОР = 0.45
#: Совпадение символьной последовательности целиком.
ПОРОГ_ПОСЛЕДОВАТЕЛЬНОСТИ = 0.90
#: Ниже этого числа четвёрок вложение не считается. Короткий текст, почти
#: целиком состоящий из длинного названия, после снятия имени превращается в
#: две-три общие четвёрки, и любая доля от такой величины — шум.
МИН_ЧЕТВЁРОК = 8
...
def word_ngrams(текст: str, n: int = 4) -> set[tuple[str, ...]]:
    с = слова(текст)
    if len(с) < n:
        return {tuple(с)} if с else set()
    return {tuple(с[i:i + n]) for i in range(len(с) - n + 1)}


def containment(а: set, б: set) -> float:
    """Какая доля меньшего множества содержится в большем.

    Отличается от Жаккара тем, что не наказывает за добавленный объём.
    Именно это и нужно против разбавления: вставить лишнее слово через
    каждые девять — дешёвый способ разорвать цепочку совпадающих слов и
    опустить близость пятиграмм чуть ниже порога, оставив текст тем же.
    Доля вложения при этом почти не меняется.
    """
    if not а or not б:
        return 0.0
    return len(а & б) / min(len(а), len(б))
...
class NearDuplicateDetector:
    ...
    def check(self, текст: str, *, site_id: str, entity_id: str,
        ...
        # Четвёрки, целиком состоящие из слов разрешённого названия, из
        # сравнения исключаются: задание прямо выводит названия из-под
        # правила о совпадающих цепочках.
        слова_названий = {с for т in allowed_titles for с in слова(т or "")}

        def без_названий(набор: set) -> set:
            if not слова_названий:
                return набор
            return {г for г in набор if not set(г) <= слова_названий}
        ...
        for запись in self.корпус:
            ...
            # Вложение словесных четвёрок — против разбавления. Проверяется
            # раньше пятиграмм: разбавленная копия садится ровно под их
            # порог, а вложение остаётся высоким.
            мои_четвёрки = без_названий(мой["word4"])
            его_четвёрки = без_названий(и["word4"])
            достаточно = (len(мои_четвёрки) >= МИН_ЧЕТВЁРОК
                          and len(его_четвёрки) >= МИН_ЧЕТВЁРОК)
            вложение = (containment(мои_четвёрки, его_четвёрки)
                        if достаточно else 0.0)
            # Сравнение последовательностей квадратично по длине и считается
            # только там, где дешёвая мера уже показала близость. Иначе на
            # корпусе в двести текстов оно занимает больше времени, чем весь
            # остальной разбор вместе взятый.
            последовательность = (sequence_ratio(текст, запись.text)
                                  if вложение >= 0.3 else 0.0)
            if not своя_сущность and достаточно and (
                    вложение >= ПОРОГ_ВЛОЖЕНИЯ_БЛОК
                    or последовательность >= ПОРОГ_ПОСЛЕДОВАТЕЛЬНОСТИ):
                отчёт.long_run_matches += 1
                отчёт.findings.append(DuplicateFinding(
                    "word_containment", запись.entry_id, запись.site_id,
                    запись.entity_id, max(вложение, последовательность),
                    f"вложение словесных четвёрок {вложение:.2f}, совпадение "
                    f"последовательности {последовательность:.2f}: текст "
                    f"выведен из существующего, а не написан заново",
                    "BLOCK"))
                continue
            ...
            if вложение >= ПОРОГ_ВЛОЖЕНИЯ_РАЗБОР and not своя_сущность:
                отчёт.review_required += 1
                отчёт.findings.append(DuplicateFinding(
                    "word_containment", запись.entry_id, запись.site_id,
                    запись.entity_id, вложение,
                    f"вложение словесных четвёрок {вложение:.2f} ≥ "
                    f"{ПОРОГ_ВЛОЖЕНИЯ_РАЗБОР}", "REVIEW"))
                continue
