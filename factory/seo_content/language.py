...
РЕКЛАМА = (
    ...
    # Мягкие призывы: обращение к читателю вместо сведений о работе.
    r"начн[иет]\w*\s+просмотр\s+(?:прямо\s+)?(?:сейчас|сегодня)",
    r"убедит[ес]\w*\s+сам\w+", r"оцените\s+сами",
    r"переход[иь]\w*\s+(?:к\s+просмотру|по\s+ссылке)",
    r"добав[ьи]\w*\s+в\s+закладки", r"поделит[еь]\w*\s+с\s+друзьями",
    r"вас\s+жд[её]т", r"спешите\s+(?:увидеть|посмотреть)",
)
