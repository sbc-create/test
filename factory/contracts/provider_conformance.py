...
Первое — расхождение связи: увидеть его можно только сверкой одного с другим.

Второе оказалось **не дефектом**, и это важная оговорка о пределах проверки.
`site-engine` выключен по умолчанию: без флага маршрутов нет, и отвечает он
`404`, а не `403`, намеренно — существование маршрута тоже сведения. Отличить
«не реализовано» от «намеренно выключено» по коду ответа невозможно, поэтому
такая находка называется `MISMATCH` и требует прочтения человеком, а не
считается провалом контракта автоматически.
