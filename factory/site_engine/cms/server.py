...
class Обработчик(BaseHTTPRequestHandler):
    ...
    def _задания(self, лицо, параметры):
        ...
        периодические = [
            [э(j.get("name", "")), э(j.get("service", "")), э(j.get("site_id", "—")),
             э(j.get("raw", ""))]
            for j in self._данные("jobs", лицо, {"per_page": "60"})
        ]
        return "Задания", (
            форма
            + "<h1>Поданные команды</h1>"
            + таблица(["Команда", "Вид", "Состояние", "Автор", "Сайт", "Обновлена"], строки)
            + "<h1>Периодические работы</h1>"
            + '<div class="note">Читается у systemd, а не выдумывается: '
              'расписание, которое показывают, но не проверяют, однажды расходится '
              'с тем, что на самом деле выполняется.</div>'
            + таблица(["Таймер", "Служба", "Область", "Расписание"], периодические)
        )
