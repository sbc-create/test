...
def _слить_детали(записи: list[dict], каталог_деталей: Path) -> tuple[list[dict], int]:
    ...
    Цена измерена, а не оценена: полная выгрузка `lords-01` (53 257 записей,
    12 022 файла detail) занимает 12,7 с против 7,5 с без слияния. Платится она
    один раз на смену входов — выгрузка кэшируется по отпечатку каталога и
    состоянию кэша detail, и потребитель, листающий страницы, за неё не платит.
    Поэтому в ключе кэша обязателен и кэш detail: без него первая же выгрузка
    после дополнения отдавала бы вчерашний ответ без новых сведений.
    """
    ...
    for запись in записи:
        ...
        if not isinstance(деталь, dict):
            итог.append(запись)
            continue
        # Переносится только описательное, и это не осторожность, а
        # обязанность. Ответ detail несёт и `name`, и `type`, и `tags`, и
        # `is_series` — то есть ровно то, из чего считаются адрес страницы и
        # вид произведения. Слить их «как есть» значило бы менять маршруты
        # двенадцати тысячам записей и разводить коллизии адресов из-за того,
        # что второй ответ источника назвал фильм чуть иначе.
        #
        # Пустое поле detail при этом не затирает заполненное поле списка.
        слияние = dict(запись)
        for поле in ОПИСАТЕЛЬНЫЕ_ПОЛЯ:
