...
def snapshot_route_of(
        routes: Iterable[dict[str, Any]]) -> Callable[[str, str], str]:
    """Функция адреса для снимка маршрутов, замкнутая на таблицу витрины.

    `route_snapshot.build` спрашивает адрес по паре (название, идентификатор),
    потому что витрины Lords выводят его из названия. Здесь название не
    участвует вовсе: адрес объявлен витриной, и брать его откуда-то ещё —
    значит завести второй источник адреса рядом с тем, который витрина уже
    объявила. Два источника расходятся молча, и расхождение выглядит как
    обычная страница, приписанная не тому произведению.

    Поэтому слаг ищется по идентификатору в самой таблице, а путь собирается
    единственной функцией `route_of` — той же, которой пользуется связь.

    Берутся только канонические маршруты. Неканонический — это псевдоним:
    второй адрес того же произведения. Он существует, и снимок о нём молчит
    не потому, что его нет, а потому, что снимок отвечает на вопрос «какой
    адрес у произведения», а не «по каким адресам оно ещё открывается».
    Псевдонимы перечисляются отдельно и явно.

    Идентификатор, которого нет в таблице, даёт пустой адрес. `build`
    отклоняет такую запись с причиной — это витрина её не публикует, и
    придумывать ей адрес было бы приписыванием страницы, которой нет.
    """
    по_id: dict[str, str] = {}
    for маршрут in routes:
        if not маршрут.get("canonical"):
            continue
        идентификатор = str(маршрут.get("providerTitleId") or "")
        слаг = str(маршрут.get("slug") or "")
        if идентификатор and слаг:
            по_id.setdefault(идентификатор, слаг)

    def адрес(name: str, external_id: str) -> str:  # noqa: ARG001 - см. выше
        слаг = по_id.get(str(external_id), "")
        return route_of(слаг) if слаг else ""
