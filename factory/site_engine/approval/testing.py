...
def эфемерный_signer(каталог: Path, monkeypatch=None, *,
    ...
    # Хранилище наборов тоже обязано быть эфемерным.
    #
    # Служба читает каноническое состояние сама — в этом весь смысл, и менять
    # его нельзя. Но «каноническое» определяется переменной CHANGESET_DB, и
    # если её никто не задал, служба уходит в рабочее хранилище машины. Тест
    # при этом создаёт свой набор в другом месте, служба его не находит и
    # отвечает 404 — на машине с хранилищем — или 500 там, где хранилища нет
    # вовсе. Обе ошибки говорят не о том, что проверялось, и обе прячут
    # настоящий результат проверки.
    #
    # Поэтому эфемерность здесь доводится до конца: вызывающий, который задал
    # CHANGESET_DB сам, остаётся при своём, а всем остальным достаётся
    # отдельный файл рядом с ключами. Ни при каком исходе испытание не пишет
    # и не читает рабочее хранилище.
    if not str(os.environ.get("CHANGESET_DB", "")).strip():
        установить("CHANGESET_DB", str(Path(каталог).parent / "changeset.sqlite3"))
