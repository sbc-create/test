...
class Состояние:
    ...
    def проверить_ключ_действующим(self) -> None:
        """Ключ всё ещё вправе подписывать — проверяется на КАЖДОЙ подписи.

        Проверка при старте службы отвечает только на вопрос «можно ли было
        начинать». Отзыв происходит потом: владелец помечает ключ REVOKED в
        наборе проверки, и с этого момента подписанное им недействительно.
        Но работающая служба держала `pem` и `kid` с момента запуска и
        продолжала подписывать, пока её не перезапустят. Отзыв, вступающий в
        силу «когда-нибудь при перезапуске», отзывом не является: именно в
        промежуток между ним и рестартом и нужна защита.

        Набор перечитывается с диска: он и есть то место, где владелец
        объявляет отзыв, а кэш в памяти о его решении не знает.
        """
        набор = K.НаборКлючей.из_json(C.получить(НАБОР))
        запись = набор.ключи.get(self.kid)
        if запись is None:
            raise Отказ("SIGNING_KEY_NOT_IN_KEYSET",
                        f"ключ {self.kid} исчез из набора проверки: "
                        f"подписанное им никто не сможет проверить", 503)
        if запись.состояние == "REVOKED":
            # Тот же код, что и при проверке подписи: владельцу важно отличать
            # отозванный ключ от подделки, и с какой стороны обнаружен отзыв —
            # деталь реализации, а не разные события.
            raise Отказ("APPROVAL_KEY_REVOKED",
                        f"ключ {self.kid} отозван; подписывать им нельзя", 403)
        if запись.состояние != "ACTIVE":
            raise Отказ("SIGNING_KEY_NOT_ACTIVE",
                        f"ключ {self.kid} в состоянии {запись.состояние}", 503)
...
class Обработчик(BaseHTTPRequestHandler):
    ...
    def _одобрение(self, кто: str, тело: dict) -> tuple[int, dict]:
        ...
        # Отзыв проверяется ДО обращения к набору изменений. Держатель
        # отозванного ключа не должен по коду ответа узнавать, существует ли
        # набор: 404 на несуществующий и 409 на существующий — это ответ на
        # вопрос, который он уже не вправе задавать.
        self.состояние.проверить_ключ_действующим()
