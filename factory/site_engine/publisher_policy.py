...
def проблемы_профиля(profile: dict, root: Path | str | None = None) -> list[str]:
    ...
    Профиль без семейства не требует файла политики: гейт применяют и к
    каталогам, где его нет вовсе (сборка профиля в тесте, проверка черновика).
    Требовать файл там, где проверять нечего, значит ронять гейт по причине,
    не имеющей отношения к проверяемому профилю. Профиль, объявивший семейство,
    без файла политики по-прежнему не проходит: сверять его не с чем.
    """
    проблемы: list[str] = []
