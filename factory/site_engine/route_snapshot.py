...
Зачем он нужен. Очередь SEO ключуется адресом страницы, каталог — устойчивым
идентификатором произведения. Это два пространства имён для двух разных вещей,
и прямого совпадения между ними нет: измерено — из 167 адресов очереди витрин
lords в 159 885 записях каталога не нашлось ни одного. Соединяет их снимок
маршрутов, и производит его ядро.

Как получается адрес. Витрина строит его сама, функцией `slugify` от названия.
...
def build(entries: list[dict[str, Any]], *, site_id: str, route_of,
    ...
    for запись in entries:
        ...
        if not адрес or адрес.strip("/") in ("", "title", "anime"):
