...
#: номером: «Эпизод 13» — это утверждение записи о самой себе. Догадка по одному
#: слову дала бы ложные срабатывания на названиях вроде «Эпизод неудачи».
#:
#: Живёт здесь, а не в слое API: это правило о видах, и им пользуются и выдача
#: ключей связи, и пересборка очереди разбора. Копия в каждом из них разошлась
#: бы на первом же уточнении.
ЭПИЗОД_В_НАЗВАНИИ = re.compile(r"(?i)\b(эпизод|серия|episode|ep\.?)\s*№?\s*(\d{1,3})\b")

#: Виды, для которых «эпизод N» в названии — противоречие. Для сериала и самого
#: эпизода это нормально.
ВИДЫ_БЕЗ_ЭПИЗОДОВ = frozenset({"MOVIE", "DOCUMENTARY", "SHORT"})
