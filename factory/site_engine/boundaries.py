...
#:
#: Два последних — не поведение, а запись наблюдения, и разница здесь
#: существенная. Правило запрещает ядру *вести себя* по-разному для разных
#: сайтов; `audit/bootstrap.py` содержит genesis-запись о конкретном случившемся
#: наблюдении, а `provisioner/capabilities.py` — матрицу доказательств со
#: ссылками на реальные конфигурации и сертификаты. Переписать эти строки без
#: имён значило бы солгать о том, что именно измерено. Ветвления по семейству
#: сайта в них нет: такие таблицы вынесены в
#: `config/site-engine/site-families.json`.
DOMAIN_AWARE = ("adapters/", "boundaries.py", "gate.py",
                "audit/bootstrap.py", "provisioner/capabilities.py")
...
def check(root: Path | str = ".") -> BoundaryResult:
    ...
    for path, source in sources.items():
        ...
        for imported in _imports(source):
            ...
            # Исключение — файл, который лежит в том же пакете, откуда берёт.
            #
            # Иначе правило запрещало бы ровно то, чего само требует: пакет
            # собирает публичный интерфейс в своём `__init__`, и сделать это
            # можно только импортом собственного подмодуля. Исключение узкое —
            # обязательно лежать *внутри* того же пакета: `provisioner/
            # orchestrator.py`, тянущий `provisioner.providers.base`, ему не
            # удовлетворяет и остаётся нарушением.
            целевой_пакет = "/".join(parts[:-1])
            свой_пакет = bool(целевой_пакет) and rel.startswith(f"{целевой_пакет}/")
            if len(parts) > 2 and not свой_пакет:
                result.fail(
