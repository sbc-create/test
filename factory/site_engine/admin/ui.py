...
def page(title: str, body: str, *, session_label: str = "", csrf: str = "") -> str:
    # Пять зон центра управления видны только в общем контуре: внутри контура
    # витрины они означали бы, что местный администратор смотрит на чужой флот.
    #
    # Экран, до которого нет ссылки, существует только для того, кто помнит его
    # адрес: возможности, добавленные без ссылки, на практике не используются.
    массив = (
        f'<a href="{_путь()}/fleet">Флот</a>'
        f'<a href="{_путь()}/templates">Шаблоны</a>'
        f'<a href="{_путь()}/analytics">Аналитика и SEO</a>'
        if _путь() == "/admin" else ""
    )
    nav = ""
    if session_label:
...
#: Что показывает экран аналитики и SEO. Порядок — от того, что спрашивают
#: первым, к тому, что уточняют потом.
ПОКАЗАТЕЛИ_АНАЛИТИКИ = (
    ("visitors", "Пользователи"),
    ("visits", "Визиты"),
    ("pageviews", "Просмотры"),
    ("playerStarts", "Запуски плеера"),
    ("playableShare", "Доля с видео"),
    ("errors4xx", "Ошибки 4xx"),
    ("errors5xx", "Ошибки 5xx"),
    ("emptySearchQueries", "Поиск без результата"),
    ("freshnessSeconds", "Свежесть каталога, с"),
)

ПОКАЗАТЕЛИ_SEO = (
    ("seoIndexingEnabled", "Индексация разрешена"),
    ("indexedPages", "Проиндексировано"),
    ("sitemapState", "Карта сайта"),
    ("robotsState", "robots.txt"),
    ("canonicalState", "Канонические адреса"),
    ("seoVisibility", "Видимость"),
    ("iks", "ИКС"),
    ("coreWebVitals", "Core Web Vitals"),
)


def analytics(записи: list, *, flash: dict | None, session_label: str, csrf: str) -> str:
    """Аналитика и SEO по всем витринам в одном окне.

    Числа и их отсутствие показаны одинаково подробно. Источник, не выдавший
    учётных данных, виден как `не подключено` с причиной — а не как ноль
    посетителей: ноль означал бы, что сайт никто не открыл, и по нему приняли
    бы решение.
    """
    def таблица(показатели, заголовок, пояснение):
        шапка = "".join(f"<th>{_e(подпись)}</th>" for _, подпись in показатели)
        строки = "".join(
            f'<tr><td><strong>{_e(str((з.get("fields", {}).get("name") or {}).get("value") or з["siteId"]))}</strong>'
            f'<br><span class="mut">{_e(з["siteId"])}</span></td>'
            + "".join(f'<td>{_показатель((з.get("fields") or {}).get(имя))}</td>'
                      for имя, _ in показатели)
            + "</tr>"
            for з in записи
        )
        return (f'<div class="card"><h2>{_e(заголовок)}</h2>'
                f'<p class="hint">{_e(пояснение)}</p>'
                '<div class="scroll-x"><table><thead><tr><th>Витрина</th>'
                + шапка + "</tr></thead><tbody>"
                + (строки or чтопусто(len(показатели) + 1, "Витрин нет."))
                + "</tbody></table></div></div>")

    return page(
        "Аналитика и SEO",
        _flash(flash)
        + таблица(ПОКАЗАТЕЛИ_АНАЛИТИКИ, "Аналитика",
                  "Ошибки и свежесть измеряются на этой машине. Посещаемость и "
                  "запуски плеера приходят из внешнего счётчика; пока учётных "
                  "данных нет, здесь стоит «не подключено» с причиной, а не ноль.")
        + таблица(ПОКАЗАТЕЛИ_SEO, "SEO",
                  "Разрешение на индексацию читается из пакета витрины. "
                  "Проиндексированные страницы и видимость требуют доступа к "
                  "поисковой консоли — без него показывается состояние, а не число."),
        session_label=session_label,
        csrf=csrf,
    )
