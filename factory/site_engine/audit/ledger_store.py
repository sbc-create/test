...
ПРАВА_СЛУЖБ: dict[str, set[str]] = {
    ...
    # Две отдельные личности вместо одной общей. Общая означала бы, что по
    # записи в журнале нельзя сказать, какой процесс её сделал, — а это ровно
    # то, ради чего журнал вообще опознаёт производителя.
    #
    # Обеим достаточно OBSERVE: обе публикуют исходящий ящик и ни одна не
    # задаёт authority, то есть фактически всегда пишет наблюдение. EXECUTE
    # был выдан им «на всякий случай» и не понадобился ни разу.
    "audit-bridge":     {"OBSERVE"},
    "changeset-worker": {"OBSERVE"},
...
def append(соед: sqlite3.Connection, событие: dict[str, Any], *,
           ...
           authority: str, известные_сайты=None,
           проекция: Any = None) -> dict[str, Any]:
    """Добавить событие. Личность и полномочия приходят ОТ СЕРВЕРА.

    ``проекция`` — необязательный обработчик ``(соед, тело, ledger_seq)``,
    вызываемый ВНУТРИ той же транзакции перед фиксацией. Он существует затем,
    чтобы предметная проекция обновлялась вместе с событием, а не отдельной
    записью следом: две транзакции означают окно, в котором событие уже есть,
    а состояние ещё прежнее. Исключение из обработчика откатывает всё —
    поэтому проверка вроде CAS, выполненная в нём, опирается на ограничения
    долговечного хранилища, а не на блокировку внутри процесса.

    При идемпотентном повторе обработчик НЕ вызывается: повтор обязан
    повторять, а не применять изменение второй раз.
    ...
    try:
        ...
        if проекция is not None:
            # Внутри той же транзакции: событие и производное от него
            # состояние либо появляются вместе, либо не появляются вовсе.
            проекция(соед, тело, seq)
        соед.execute("COMMIT")
    except Exception:
