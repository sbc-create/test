"""Журнал редакторских действий (прежний `factory/site_engine/audit.py`).

Модуль переехал сюда не ради порядка, а потому что стоял на пути. Рядом был
создан ПАКЕТ с тем же именем `audit`, и Python при импорте выбирает пакет —
файл переставал существовать для всех, кто его импортировал. Ошибка выглядела
как «cannot import name AuditLog» и ломала сбор целого набора тестов.

Имена ре-экспортируются из `__init__` пакета, поэтому прежние импорты
`from factory.site_engine.audit import AuditLog` продолжают работать.
"""
