...
и то же. `HTTPS://Example.COM/title/x/`, `https://example.com/title/x` и
`https://example.com//title/x/?utm_source=mail` — это один адрес, и считать их
...
def normalize(url: str, *, profiles: dict[str, str],
    ...
    `profiles` — отображение имени узла на витрину. Оно приходит настройкой, а
    не зашито: сорок третья витрина добавляется строкой в объявлении, а не
    правкой этого модуля.
    """
    try:
...
def _kind_of(путь: str, правила: dict[str, Any]) -> tuple[str, str]:
    ...
    if not части:
        return "root", ""
    префиксы = правила.get("titlePrefixes") or ("title", "anime")
