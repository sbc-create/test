...
СТАДИЯ_ПО_РЕСУРСУ = {
    ...
    ("template", "counter_tag"): TEMPLATE_READY,
}
...
def вычислить(*, site_id: str, запись_реестра: dict | None,
    ...
    """Стадия выводится из ДОЛГОВЕЧНОГО состояния.

    Считать её по шагам текущего запуска — соблазнительно и неверно: второй
    проход, где всё уже сделано и потому ничего не делается, показал бы
    стадию НИЖЕ первого. Готовность описывает сайт, а не последний прогон.
    """
