...
# Внутри пакета ходим напрямую, а не через собственную обёртку: обёртка
# импортирует пакет, и на инициализации это замыкается в кольцо.
from factory.site_engine.provisioning.site_request import SiteRequestStore, Заявка
