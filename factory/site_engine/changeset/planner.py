...
#: Поля, значение которых по контракту является JSON-указателем на поле
#: внутри ресурса, а не путём в файловой системе. Указатель начинается с
#: косой черты, и общая проверка приняла бы его за абсолютный путь.
#:
#: Исключение намеренно узкое: оно действует только для ключа `path` внутри
#: списка `operations` и только для значений, которые ЦЕЛИКОМ соответствуют
#: строгой форме указателя. Всё остальное — переход вверх по дереву, точки,
#: пробелы, двоеточия, обратная косая — по-прежнему отвергается, поэтому
#: `/etc/shadow` и `../secret` не проходят и здесь.
УКАЗАТЕЛЬ = re.compile(r"^/[A-Za-z0-9_-]{1,64}$")
УКАЗАТЕЛЬНЫЕ_ПОЛЯ = ("requested_change.operations[",)


def _указатель_ли(путь: str, значение: str) -> bool:
    """Является ли значение объявленным JSON-указателем, а не путём в ФС."""
    if not путь.endswith(".path"):
        return False
    if not any(м in путь for м in УКАЗАТЕЛЬНЫЕ_ПОЛЯ):
        return False
    return bool(УКАЗАТЕЛЬ.fullmatch(значение))
...
def проверить_содержимое(значение: Any, *, глубина: int = 0,
    ...
    если_указатель = _указатель_ли(путь, значение)
    for образец, что in ОПАСНЫЕ_ОБРАЗЦЫ:
        if образец.search(значение):
            # Единственное исключение: объявленный контрактом JSON-указатель
            # на поле ресурса. Он выглядит как абсолютный путь и им не
            # является; остальные правила к нему применяются полностью.
            if если_указатель and что == "абсолютный путь":
                continue
