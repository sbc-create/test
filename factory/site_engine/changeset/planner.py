...
def классифицировать_риск(набор: dict[str, Any], план: dict[str, Any],
    ...
    Правила проверяются от тяжёлого к лёгкому, и первое совпавшее решает.
    Неизвестный род ресурса относится к R3, а не к R0: класс присваивается по
    тому, что про изменение ИЗВЕСТНО, и отсутствие сведений — не довод считать
    его безобидным.

    Окружение здесь не участвует намеренно. Правка в тестовом контуре не
    делает изменение содержимого «песочным»: R1 — это род ресурса, который
    существует только как черновик. Где именно изменение применяется и
    сколько целей задето, измеряет `оценить_влияние`, и смешивать эти две
    величины в одном поле — значит потерять обе.
    """
    род = str(набор.get("resource_type") or "")
    операция = str(набор.get("operation_type") or "")

    if операция in M.НЕОБРАТИМЫЕ_ОПЕРАЦИИ or род in M.РЕСУРСЫ_R4:
        return M.RISK_R4
    if (набор.get("requested_change") or {}).get("force"):
        # Принуждение — это отказ от проверок, которые иначе остановили бы
        # изменение. Род ресурса при этом уже не важен.
        return M.RISK_R4
    if род in M.РЕСУРСЫ_R3:
        return M.RISK_R3
    if len(набор.get("target_site_ids") or ()) >= M.ПОРОГ_МАССОВОСТИ:
        # Массовость поднимает класс, но не опускает: изменение содержимого
        # на девяти витринах — R3, изменение DNS на девяти осталось R4 выше.
        return M.RISK_R3
    if род in M.РЕСУРСЫ_R2:
        return M.RISK_R2
    if род in M.РЕСУРСЫ_R1:
        return M.RISK_R1
    if род in M.РЕСУРСЫ_R0:
        return M.RISK_R0
    return M.RISK_R3


def оценить_влияние(набор: dict[str, Any], окружения: set[str]) -> str:
    """Ширина охвата: сколько и где. Прежняя шкала, под своим именем."""
    if окружения & {"production"}:
        return M.ВЛИЯНИЕ_ВЫСОКОЕ
    if окружения - {"test"}:
        return M.ВЛИЯНИЕ_СРЕДНЕЕ
    if len(набор.get("target_site_ids") or ()) > 1:
        return M.ВЛИЯНИЕ_СРЕДНЕЕ
    return M.ВЛИЯНИЕ_НИЗКОЕ
...
def спланировать(набор: dict[str, Any], *, реестр: RegistryClient | None = None,
    ...
    влияние = оценить_влияние(набор, окружения)
