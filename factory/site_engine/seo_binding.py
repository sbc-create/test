...
#: Пространства имён внешних идентификаторов. Список закрыт: идентификатор из
#: неизвестного пространства не отбрасывается, но и разрешением на
#: воспроизведение не становится.
ID_NAMESPACES: tuple[str, ...] = (
    "kp", "mali", "mdl", "imdb", "cvh", "myanimelist", "shikimori", "tmdb",
)

#: Идентификаторы, разрешённые как основание воспроизведения
#: (`CORE_TO_OWNER-014`). `imdb` и `cvh` в список не входят.
PLAYBACK_AUTHORISED: frozenset[str] = frozenset({"kp", "mali", "mdl"})
...
class RouteBinding:
    ...
    def may_promise_playback(self) -> bool:
        ...
        return (self.binding_state is BindingState.BOUND
