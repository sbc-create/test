...
DEFAULT_LIMITS: dict[str, Limit] = {
    ...
    #: Чтение считается отдельно от записи и щедрее. Один предел на то и другое
    #: сделан для автоматики, а человек, проходящий мастер заведения витрины,
    #: успевает за минуту открыть десяток страниц — и упирался в предел на
    #: последнем шаге, получая пустую страницу вместо формы. Ограничивать надо
    #: то, что меняет состояние; чтение не меняет ничего.
    "actor_read": Limit(capacity=120, per_seconds=60.0),
    "site": Limit(capacity=30, per_seconds=60.0),
    "operation": Limit(capacity=20, per_seconds=60.0),
    #: То же разделение, что и для действующего лица. Двадцать обращений в
    #: минуту на операцию рассчитаны на изменяющие вызовы; экран, читающий
    #: заявку и план на каждом шаге, исчерпывал их к восьмому шагу — и
    #: показывал пустую страницу вместо формы.
    "operation_read": Limit(capacity=60, per_seconds=60.0),
