...
def настройка_есть(root: Path) -> bool:
    """Есть ли файл источников связей в этом корне состояния.

    Отдельный вопрос, потому что ответы «настройки нет» и «настройка описывает
    ноль витрин» — разные, а выглядят одинаково: пустым перечнем. Первый —
    незавершённая установка, второй — осознанное решение. Схлопывать первый во
    второй значит выдавать незнание за измеренный ноль.
    """
    return (root / SOURCES_REF).exists()
...
def каталог_витрин(root: Path) -> dict[str, Any]:
    ...
    if not есть:
        итог["reason"] = (
            f"{SOURCES_REF} нет в корне состояния: перечень пуст не потому, "
            "что витрин со связями нет, а потому, что их нечем описать")
    return итог
