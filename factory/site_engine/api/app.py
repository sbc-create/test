...
class SiteEngineApi:
    ...
    # Путь к обслуживаемому bundle. Прежде здесь стояла 1.0.1, тогда как на
    # диске лежали 1.1.0–1.3.0: служба отдавала план, отставший от реализации
    # на три версии, и расхождение обнаруживалось только попыткой вызвать то,
    # чего в плане нет.
    # Путь — настройка развёртывания, а не код. Переопределяется
    # `CONTROL_PLANE_BUNDLE_DIR`; значение по умолчанию прежнее, поэтому
    # работающая служба ведёт себя точно так же. Без этого кандидатский
    # набор нельзя было бы обслужить иначе как подменой файлов под живой
    # службой — то есть проверить его было бы нечем, кроме веры.
    _BUNDLE = os.environ.get("CONTROL_PLANE_BUNDLE_DIR") \
        or "/srv/site-factory/control-plane-contracts/1.3.1"
