...
class SiteEngineApi:
    ...
    # Путь к обслуживаемому bundle НЕ прибит строкой. Прежде здесь стояла
    # 1.0.1, тогда как на диске лежали 1.1.0–1.3.0: служба отдавала план,
    # отставший от реализации на три версии, и расхождение обнаруживалось
    # только попыткой вызвать то, чего в плане нет. Теперь версия объявлена
    # в одном месте, а какой набор отдаётся на самом деле — сообщается.
    @property
    def _BUNDLE(self) -> str:
        from factory.site_engine import contracts_version as CV
        путь = CV.обслуживаемый().get("path")
        return str(путь or (CV.КАТАЛОГ / CV.ТЕКУЩИЙ))
