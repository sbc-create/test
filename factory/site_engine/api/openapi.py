...
ЗАПИСЬ: dict[str, dict[str, Any]] = {
    ...
    "/api/v1/site-requests": {
        "method": "get",
        "summary": "Заявки на новые витрины",
        "scope": "read",
        "idempotent": True,
        "body": None,
        "success": ("200", "черновики заявок с текущим шагом"),
        "errors": {"400": "негодные параметры"},
    },
    "/api/v1/site-requests/{requestId}/approve": {
        "method": "post",
        "summary": "Подтвердить план заявки по его отпечатку",
        "scope": "sites:create",
        "idempotent": True,
        "body": {
            "type": "object",
            "required": ["planHash"],
            "properties": {"planHash": {"type": "string"}},
        },
        "success": ("200", "заявка подтверждена"),
        "errors": {"409": "подтверждается не тот план: ответы изменились после показа"},
    },
    "/api/v1/site-requests/{requestId}/provision": {
        "method": "post",
        "summary": "Выложить канарейку по подтверждённому плану",
        "scope": "sites:create",
        "idempotent": True,
        "body": {"type": "object", "properties": {}},
        "success": ("200", "канарейка выложена под NOINDEX"),
        "errors": {"409": "нет подтверждения, план изменился или домен занят"},
    },
    "/api/v1/site-requests/{requestId}/verification": {
        "method": "get",
        "summary": "Проверки канарейки поимённо",
        "scope": "read",
        "idempotent": True,
        "body": None,
        "success": ("200", "список проверок с исходом каждой"),
        "errors": {"409": "канарейка ещё не выложена"},
    },
    "/api/v1/site-requests/{requestId}/publish": {
        "method": "post",
        "summary": "Перевести канарейку в боевой контур",
        "scope": "sites:create",
        "idempotent": False,
        "body": {"type": "object", "properties": {}},
        "success": ("200", "не достигается: требуется разрешение владельца"),
        "errors": {"409": "OWNER_AUTHORIZATION_REQUIRED"},
    },
    "/api/v1/site-requests/{requestId}/rollback": {
        "method": "post",
        "summary": "Снять канарейку и освободить домен",
        "scope": "sites:create",
        "idempotent": True,
        "body": {"type": "object", "properties": {}},
        "success": ("200", "всё созданное снято"),
        "errors": {"409": "выкладки не было"},
    },
    "/api/v1/site-requests/{requestId}/plan": {
        "method": "get",
        "summary": "Детерминированный сухой прогон заявки",
        "scope": "read",
        "idempotent": True,
        "body": None,
        "success": ("200", "план без мутаций: ресурсы, замки, контракты, требования, откат"),
        "errors": {"404": "заявки нет"},
    },
    "/api/v1/join-keys/{siteId}": {
        "method": "get",
        "summary": "Связь адреса страницы витрины с записью каталога",
        "scope": "read",
        "idempotent": True,
        "body": None,
        "success": (
            "200",
            "адрес, внутренний идентификатор, вид, состояние показа по записи; "
            "запись без адреса названа, а не выброшена",
        ),
    ...
    "/api/v1/templates": {
        "method": "get",
        "summary": "Реестр шаблонов: версии, отпечатки, способности, откат",
        "scope": "read",
        "idempotent": True,
        "body": None,
        "success": ("200", "зарегистрированные пакеты и отдельно отвергнутые с причиной"),
        "errors": {"400": "негодные параметры"},
    },
    "/api/v1/fleet": {
        "method": "get",
        "summary": "Весь флот витрин одной выборкой",
        "scope": "read",
        "idempotent": True,
        "body": None,
        "success": ("200", "строка на витрину: значение, состояние, источник, время"),
        "errors": {"400": "негодные параметры"},
    },
    "/api/v1/fleet/{siteId}": {
        "method": "get",
        "summary": "Единая запись одной витрины",
        "scope": "read",
        "idempotent": True,
        "body": None,
        "success": ("200", "все известные поля витрины с происхождением каждого"),
        "errors": {"403": "чужая витрина", "404": "витрины нет в реестре"},
    },
    "/api/v1/scorecard": {
        "method": "get",
    ...
    "/api/v1/alerts": {
        ...
        "idempotent": True,
        "body": None,
        "success": ("200", "код, значимость, смысл и путь к инструкции"),
        "errors": {"400": "негодные параметры"},
    },
    "/api/v1/state-inventory": {
        "method": "get",
        "summary": "Опись хранилищ состояния службы",
        "scope": "audit:read",
        "idempotent": True,
        "body": None,
        "success": ("200", "хранилища, их наличие и число файлов"),
        "errors": {"403": "нет права audit:read"},
    },
    "/api/v1/state-backup": {
        "method": "post",
        "summary": "Снять копию состояния и проверить её разворачиванием",
        "scope": "audit:read",
        "idempotent": False,
        "body": {"type": "object", "properties": {"verify": {"type": "boolean"}}},
        "success": ("200", "копия снята; verified говорит, проверена ли она"),
        "errors": {"403": "нет права audit:read"},
    },
    "/api/v1/releases": {
        "method": "get",
        "summary": "Записи выпусков и точка отката",
        "scope": "read",
        "idempotent": True,
        "body": None,
        "success": ("200", "выпуски новыми вперёд; недоступный источник назван"),
        "errors": {"400": "негодные параметры"},
    },
    "/api/v1/incidents": {
        "method": "get",
        "summary": "Происшествия с состоянием и влиянием",
        "scope": "read",
        "idempotent": True,
        "body": None,
        "success": ("200", "происшествия; открытые отделены от закрытых"),
        "errors": {"400": "негодные параметры"},
    },
    "/api/v1/settings/{siteId}": {
        "method": "get",
        "summary": "Схема, значения и границы настроек витрины",
        "scope": "read",
        "idempotent": True,
        "body": None,
        "success": ("200", "поля со схемой, отклонённые поля с причиной, ссылки на секреты"),
        "errors": {"400": "негодный идентификатор", "404": "витрины нет"},
    },
    "/api/v1/settings/{siteId}/rollback": {
        "method": "post",
        "summary": "Вернуть значения последнего изменения настроек",
        "scope": "config:write",
        "idempotent": False,
        "body": {"type": "object", "properties": {"dryRun": {"type": "boolean"}}},
        "success": ("200", "прежние значения возвращены"),
        "errors": {"409": "откатывать нечего или конфигурацию правили после"},
    },
