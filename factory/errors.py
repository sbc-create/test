...
FAILURE_STATES = (
    ...
    "BLOCKED_TARGET",
    # Введён заданием о переносимой ячейке: сайт выделен в собственный
    # репозиторий, и публиковать его отсюда больше нельзя. Это не
    # BLOCKED_AUTHORIZATION (разрешение владельца ни при чём), не
    # BLOCKED_TARGET (направление как раз известно — это репозиторий сайта) и
    # тем более не ошибка среды: отказ постоянный и по замыслу.
    "BLOCKED_SITE_EXTRACTED",
...
NON_RETRYABLE = {
    ...
    "BLOCKED_TARGET",
    # Выделение сайта необратимо в рамках задания: повтор через старый путь
    # обязан отказывать столько же раз, сколько его запустят.
    "BLOCKED_SITE_EXTRACTED",
}
...
class SiteExtracted(FactoryError):
    """Сайт публикуется из собственного репозитория, а не отсюда.

    Отказ выдаётся ДО первой мутации: смысл выделения в том, что у сайта один
    источник изменений. Два пути публикации к одному домену — это не
    избыточность, а гонка, в которой побеждает тот, кто выложил последним.
    """

    status = "BLOCKED_SITE_EXTRACTED"
