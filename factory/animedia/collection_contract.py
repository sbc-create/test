...
#
# Точка форка (ADR-0009): скопировано из канонического экземпляра флота.
#   исходная цифра sha256: 1de55d9d4ee44d5f586bf97b6513d8f18aabc0c946b9efca40603b0ec194d948
#   коммит источника:      d01c13704adfe945eb8d2efde540a7114c27db71
# Расхождение после этой точки — сознательная работа контура, а не случайность:
# по этим двум значениям видно, от какой версии он ушёл.
ТОЧКА_ФОРКА = {
    "source_sha256": "1de55d9d4ee44d5f586bf97b6513d8f18aabc0c946b9efca40603b0ec194d948",
    "source_commit": "d01c13704adfe945eb8d2efde540a7114c27db71",
    "adr": "adr/0009-animedia-collection-contract-fork.md",
}
...
class Снимок:
    ...
    #: Окно «недавно». Тридцать дней — не круглое число ради красоты: за него
    #: в каталог приходит несколько десятков записей, то есть несколько
    #: страниц подборки, и при этом слово «недавно» остаётся правдой.
    ОКНО_НЕДАВНИХ_ДНЕЙ = 30

    #: Часовой пояс витрины. Календарный год считается по нему, а не по UTC:
    #: тридцать первого декабря в двадцать три часа по Москве год на витрине
    #: ещё прежний, и подборка «Этого года» обязана это учитывать.
    ЗОНА_ВИТРИНЫ = timezone(timedelta(hours=3))

    def сейчас(self) -> datetime:
        return datetime.now(self.ЗОНА_ВИТРИНЫ)

    def недавние(self, дней: int | None = None) -> list[dict]:
        """Записи, добавленные в каталог за последние `дней` суток.

        Раньше «Недавно добавленные» отдавали `по_дате()` — ВЕСЬ каталог,
        отсортированный по дате. Счётчик подборки показывал 7446 записей,
        ровно столько же, сколько во всём каталоге, и подборка называлась
        недавними поступлениями, ничем от каталога не отличаясь. Ограничение
        по дате и есть то, что делает её подборкой.

        `_по_дате` отсортирован «сначала новое», поэтому обход прерывается на
        первой записи старше порога: перебирать все семь тысяч ради пяти
        десятков незачем. Записи без даты добавления лежат в конце этого
        порядка и в окно не попадают — придумывать им дату нельзя.
        """
        порог = self.сейчас() - timedelta(
            days=int(дней or self.ОКНО_НЕДАВНИХ_ДНЕЙ))
        готово: list[dict] = []
        for з in self._по_дате:
    ...
    def календарный_год(self) -> list[dict]:
        """Записи ТЕКУЩЕГО календарного года витрины.

        Было `года(максимальный_год)` — самый свежий год, НАЙДЕННЫЙ в данных.
        Пока каталог пополняется, эти два числа совпадают и подмена незаметна;
        первого января они разойдутся, и подборка «Этого года» покажет прошлый
        год, ничего об этом не сказав. А если источник однажды пришлёт запись
        будущего года, она разойдётся сразу.
        """
        return self.года(self.сейчас().year)
...
#:
#: ВНИМАНИЕ ПРИ ЛЮБОМ КЭШИРОВАНИИ `разрешить`. Две выборки зависят не только
#: от снимка, но и от ТЕКУЩЕГО МОМЕНТА: `recent` (скользящее окно) и
#: `current_year` (календарный год витрины). Ключ кэша, состоящий из снимка и
#: ключа коллекции, заморозит их: окно перестанет скользить, а в ночь на
#: первое января «Этого года» покажет прошлый год — до следующей доставки
#: каталога. Соседняя витрина на этом уже попалась. Момент обязан входить в
#: ключ; `checks/collection_rules.py` подменяет часы и требует, чтобы ответ
#: изменился.
ВЫБОРКИ: dict[str, Callable[[Снимок, dict], list[dict]]] = {
    "recent": lambda с, ф: с.недавние(ф.get("days")),
    "recent_of_kind": lambda с, ф: с.по_виду(str(ф.get("kind") or "")),
    "top_rated": lambda с, ф: с.по_оценке(),
    "current_year": lambda с, ф: с.календарный_год(),
...
def _общие(семейство: str, кино: str, сериал: str) -> list[Спецификация]:
    ...
    return [
        _спец("recently_added", семейство, "Недавно добавленные",
              "Записи, появившиеся в каталоге за последние 30 дней, "
              "от самой свежей.",
              "catalog", {"days": Снимок.ОКНО_НЕДАВНИХ_ДНЕЙ},
        ...
        _спец("current_season", семейство, "Этого года",
              "Аниме текущего календарного года, от самого свежего "
              "поступления.",
              "catalog", {"year": "calendar"},
              {"field": "published_at", "order": "desc"},
...
def разрешить(ключ: str, снимок: Снимок, семейство: str, *,
    ...
    if not спец.доступна:
        ...
        имя_выборки = (
            ...
            # «calendar» — текущий календарный год витрины. Прежнее «max»
            # означало самый свежий год, найденный в данных: до первого января
            # это то же число, после — прошлый год под именем «Этого года».
            # Прежнее написание тоже ведёт СЮДА, а не мимо: у диспетчера
            # последняя ветка — «recent», поэтому нераспознанный `year`
            # молча превратил бы «Этого года» в тридцатидневное окно —
            # подборку с чужим содержимым под своим названием.
            "current_year" if спец.filter_spec.get("year") in ("calendar", "max") else
