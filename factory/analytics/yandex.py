...
class YandexAnalyticsProvider:
    ...
    def ensure_metrica_counter(self, domain: str, name: str) -> CounterState:
        ...
        # `code_options` при создании не передаётся. Метрика принимает это поле
        # только на изменении счётчика: POST с ним отвечает
        # `HTTP 400 Could not read JSON … path: counter.code_options.visor`.
        # Проверено боевым запросом 2026-08-27 на всех трёх доменах Lords — это
        # был первый случай, когда ветка создания вообще исполнилась: три
        # счётчика Yummy были найдены готовыми (`reused`), и ошибка пролежала
        # в коде незамеченной.
        #
        # Инвариант «запись сессий выключена» от этого не страдает: сразу после
        # создания `apply` вызывает `ensure_webvisor_disabled`, а тот использует
        # PUT — путь, которым Метрика этим полем и управляет. Счётчик успевает
        # побыть с включённой записью ровно до следующего запроса и до первого
        # визита: домена в нём ещё нет ни у кого.
        #
