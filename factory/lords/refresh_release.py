...
def переключить(
    ...
    *,
    ...
    # Точка отката записывается ссылкой, а не подразумевается каталогом.
    #
    # Прежде `previous` не создавал никто, а `lords-refresh-guard` берёт
    # `previous_release` и `rollback_target` именно из неё — и потому писал в
    # rollback.json `null` с примечанием «стенд ещё не выкатывался». На
    # 2026-09-09 так было у всех трёх боевых витрин сразу: предыдущий релиз
    # лежал на диске, а записанной точки отката не было ни у одной. Наличие
    # каталога точкой отката не является.
    #
    # Пишется только когда откатываться действительно есть куда: первое
    # переключение ссылку не создаёт, потому что врать о возможности отката
    # хуже, чем честно её не иметь.
    if было is not None:
        временная_прежняя = рантайм / ".previous.new"
        if временная_прежняя.exists() or временная_прежняя.is_symlink():
            временная_прежняя.unlink()
        временная_прежняя.symlink_to(было)
        os.replace(временная_прежняя, рантайм / "previous")
    запись = {
