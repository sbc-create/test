...
class Credentials:
    ...
    @classmethod
    def from_credentials_dir(cls, directory: Path | str | None = None,
                             env: dict | None = None) -> Credentials:
        """Значения из каталога systemd credentials, а не из окружения.

        systemd кладёт файлы в tmpfs, доступную только процессу юнита, и путь
        сообщает через $CREDENTIALS_DIRECTORY. Имена самих credential'ов
        задаёт drop-in Secret Hub через CDNVIDEOHUB_*_CREDENTIAL.

        В окружении при этом лежат только имена, но не значения: `systemctl
        ...
        try:
            ...
            raise LiveBuildError(f"не прочитать credentials: {error}") from None
        if not token or not publisher:
            raise LiveBuildError("пустое значение в файле credentials")
