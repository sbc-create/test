...
def _header(ctx: dict, meta: Meta) -> str:
    ...
    return (
        ...
        + "</ul></nav>"
        + _header_search(ctx)
        # Переключатель темы стоит в подвале, а не здесь: в шапке он
        # переносился на отдельную строку и растил её на 62 пикселя — это
        # измерено эталоном раскладки на 1440 и 768. Вызов отсюда снят при
        # слиянии двух линий.
        + "</div></header>"
    )
...
def _footer(ctx: dict) -> str:
    ...
    # Переключатель рисуется только тогда, когда витрина объявила вторую
    # палитру: кнопка, которая ничего не меняет, хуже её отсутствия —
    # посетитель считает её сломанной, а не отсутствующей. Условие пришло из
    # ветки шаблонов, разметка и ключ хранения по витрине — из производственной.
...
def _seasons_block(title: fx.Title) -> str:
    ...
    if not title.episodic:
        ...
        # Фильм говорит о себе сам, но не чужим заголовком.
        #
        # Прежде этот блок назывался «О фильме» — ровно как содержательный
        # раздел ниже. На странице выходило ДВА одинаковых заголовка, и первым
        # из них шла техническая реплика, занимавшая место описания.
        #
        # Убрать блок целиком тоже нельзя: «сезонов нет» и «сезоны ещё не
        # получены» — разные утверждения, и различать их зритель вправе
        # (`tests/unit/test_lords_series_without_seasons.py`). Поэтому блок
        # остаётся, но под своим заголовком «Серии» — тем же, что у сериала
        # без данных, — и одной короткой строкой вместо абзаца.
        return (
            '<section class="seasons"><h2>Серии</h2>'
            '<p class="lede">Полнометражная запись: серий у неё нет, '
            "просмотр идёт целиком.</p></section>"
        )
    blocks = []
