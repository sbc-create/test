...
#: Каталог для временных location внутри серверного блока сайта.
#:
#: Нужен одноразовой веб-активации. Отдельный серверный блок для того же имени
#: не годится: два блока с одним server_name на одном порту — это не ошибка
#: nginx, а молчаливый выбор первого, и временный адрес просто не отвечал бы.
#: Пустой каталог ничего не добавляет: include по несуществующему шаблону —
#: не ошибка.
