...
def живой_коммит(cell: registry.Cell) -> str:
    ...
    реестре: запись отражает намерение, страница — наблюдение. Разошлись они
    хотя бы раз, и решение о выпуске должно опираться на второе.

    Но и страница не «действительность», как здесь было написано раньше.
    Заголовок берётся из манифеста, загруженного процессом ПРИ СТАРТЕ, поэтому
    после замены манифеста без перезапуска витрина объявляет один выпуск, а
    исполняет другой. Наблюдалось на animedia.icu: манифест приведён к релизу в
    22:39:53, процесс прочитал его в 22:31:18 и держал прежнее значение в
    памяти. `/__template_version` тем же значением и отвечает — это не второй
    источник, а тот же, показанный иначе.

    Исполняемый код доказывает только `/proc/<pid>/cmdline`. Для выпусков
    ячейки расхождение не возникает: кандидат — свежий процесс, и проверка
    сверяет ожидаемый build именно с ним. Для витрин под монолитом заголовок
    может отставать, и здесь он используется как признак «нужен ли выпуск», а
    не как доказательство исполняемого кода.
    """
    домен = cell.domain
    try:
