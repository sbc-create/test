...
def _archive_member(info: tarfile.TarInfo) -> tarfile.TarInfo:
    ...
    Права тоже приводятся к канону, и это не косметика. Сохранялся полный
    режим файла, а он зависит от umask того, кто раскладывал дерево: чистый
    clone при umask 022 даёт 644, рабочий каталог при umask 002 — 664. Две
    сборки одного коммита давали разные digest, и свойство, ради которого
    digest существует, не выполнялось ровно там, где проверяется — между
    машиной сборки и машиной проверки. Значащим остаётся один бит: файл
    исполняемый или нет.
    """
    info.uid = 0
    info.gid = 0
