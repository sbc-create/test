...
def _репозиторий(site_id: str) -> tuple[registry.Cell, Path]:
    ...
    # Обязателен только СБОРЩИК. `deploy/activate.sh` — это ЛЕГАСИ-путь
    # выкладки от root, и он нужен лишь тому, кто его запускает: проверка
    # требуется там, где сценарий исполняется, а не там, где считают digest.
    #
    # Требование здесь мешало запуску новых домена: `trigger` вызывает `план`
    # только чтобы получить digest коммита, а сайт, выпускаемый ИСКЛЮЧИТЕЛЬНО
    # через очередь, такого скрипта не имеет и иметь не должен — второй путь
    # выкладки кладёт выпуск мимо очереди, и ни ветка, ни коммит, ни прогон CI
    # при этом не проверяются. Именно так разошёлся lordserial33.biz.
    if not (корень / СБОРЩИК).is_file():
        raise ExecutorRefused(f"{site_id}: в репозитории нет {СБОРЩИК}")
    return cell, корень
...
def активировать(site_id: str, *, commit: str, dry_run: bool = True,
    ...
    """
    if not dry_run:
        # Этот путь исполняет сценарий РЕПОЗИТОРИЯ от root и не спрашивает
        # GitHub ни о чём. Пока он оставался проходимым, «выкладка без
        # доказанного происхождения» была не теоретической: достаточно было
        # добавить один флаг. Выпуск идёт через очередь, где происхождение
        # доказывается до единой операции над витриной, а код репозитория
        # собирается под учётной записью сайта.
        raise ExecutorRefused(
            f"{site_id}: подтверждённая ручная активация закрыта. Она исполняет "
            "код репозитория от root и не проверяет связку «репозиторий → ветка "
            "→ коммит → успешный прогон». Выпуск подаётся в очередь: "
            "`factory cell trigger --submit` или `factory cell submit`")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if not (repo / СЦЕНАРИЙ).is_file():
            # Проверка стоит здесь, а не в разборе репозитория: отсутствие
            # легаси-сценария мешает только легаси-выкладке.
            raise ExecutorRefused(
                f"{решение.site_id}: в репозитории нет {СЦЕНАРИЙ} — "
                "этот сайт выпускается только через очередь исполнителя, "
                "и выкладка от root для него не предусмотрена")
