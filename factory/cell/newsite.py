...
def _настройка_сообщества(корень: Path, заказ: Заказ):
    """Имена куки и переменных — из самой обвязки, а не из копии её правил.

    Импортируется файлом: `community_http.py` лежит в `automation/host/` и
...
def создать(заказ: Заказ, *, корень: Path, куда: Path,
            ...
            регистрировать: bool = True,
            допустить_грязное: bool = False) -> dict[str, Any]:
    """Собрать проект нового сайта из шаблона этого репозитория.

    `допустить_грязное` существует ради ПРОВЕРОК, а не ради удобства. Сборка
    из грязного дерева невоспроизводима: закрепление назвало бы коммит,
    которому содержимое файлов не соответствует. Поэтому по умолчанию это
    отказ, а с флагом — не умолчание, а запись: `source_dirty: true`
    попадает и в замок, и в происхождение манифеста, и всякий, кто возьмёт
    такой проект за выпуск, увидит это раньше, чем выложит.
    """
    пробелы = заказ.пробелы()
    if пробелы:
        raise NewSiteError(f"BLOCKED_INPUT: в заказе нет {', '.join(пробелы)}")
    коммит, грязно = _коммит(корень)
    if грязно and not допустить_грязное:
        raise NewSiteError(
            "рабочее дерево шаблона грязное: закрепление по коммиту было бы "
            "неправдой. Для проверки состава — допустить_грязное=True, и "
            "происхождение будет помечено source_dirty")
    ...
    настройка_сообщества = _настройка_сообщества(корень, заказ)
    ...
    (куда / "pins.lock.json").write_text(json.dumps({
        ...
        "pins": {
            ...
            # Метка дерева, из которого снят состав. Выпускать проект с
            # `source_dirty: true` нельзя: он описывает не тот коммит.
            "source_dirty": грязно,
            "build_id": f"template-{коммит[:12]}" + ("-dirty" if грязно else ""),
    ...
    config = {
        ...
        "environment": {
            ...
            # Имя переменной выводится ТЕМ ЖЕ правилом, что и в обвязке
            # (`community_http.Настройка.для`), а не собирается здесь заново:
            # разойдясь, они дали бы витрину, которая ищет хранилище не там,
            # куда его положил выпуск, и раздел молча выключался бы.
            настройка_сообщества.путь_env:
    ...
    return {
        ...
        "community": {
            ...
            "visitor_cookie": настройка_сообщества.кука_посетителя,
            "moderator_cookie": настройка_сообщества.кука_модератора,
            "note": ("Ключ модератора задаёт владелец ВНЕ Git. Пока он не "
                     "задан, модератора нет: новые сообщения остаются в "
                     "состоянии «ожидает проверки», автору видны, остальным — "
                     "нет. Это рабочее состояние, а не поломка."),
        },
        "profile": заказ.profile,
        "path": str(куда),
