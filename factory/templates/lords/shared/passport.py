...
def собрать(пакет: pathlib.Path, баллы_корень: pathlib.Path,
    ...
    # В доказательствах кадры лежат на уровень глубже, в screenshots/.
    if (каталог_снимков / "screenshots").is_dir():
        каталог_снимков = каталог_снимков / "screenshots"
    # Паспорт называет кадры, по которым идёт приёмка, а это кадры из
    # доказательств: там лежит то, на что смотрит владелец. Раньше список
    # собирался из рабочих PNG — с тех пор кадры для отпечатков остались PNG,
    # а остальные едут в JPEG, и список стал неполным.
    снимки = sorted(с.name for с in каталог_снимков.glob("*")
                    if с.suffix in (".png", ".jpg")) if каталог_снимков.is_dir() else []
