...
def занятых_ячеек(тип: str, количество: int) -> int:
    """Сколько ячеек сетки займёт блок.

    У мозаики ведущая плитка растянута на два столбца и две строки, то есть
    занимает четыре ячейки вместо одной. Считать её одной — значит промахнуться
    на три и объявить дефект там, где его нет, либо пропустить настоящий.
    """
    return количество + 3 if тип == "mosaic" else количество


def одинокий_хвост(количество: int, колонок: int) -> bool:
    """Останется ли в последнем ряду ровно одна карточка.

    Запрет действует от трёх колонок: в две колонки «хвост из одной» — это
    обычный нечётный ряд, а не дыра. От трёх и выше одинокая карточка в конце
    читается как обрыв сетки, и приёмка это запрещает.
    """
...
def проверить_спецификацию(спец: dict) -> list[str]:
    ...
    # Одинокий хвост сетки. Сломанный измеритель этого класса дефектов не видел
    # вовсе: он искал карточки по селекторам живого рантайма и на пакетах
    # фабрики не находил ни одной. После починки одиннадцать шаблонов оказались
    # с хвостом из одной карточки — значит запрет нужен здесь, до отрисовки.
    for блок in спец.get("blocks", []):
        класс = блок.get("класс")
        количество = блок.get("максимум")
        if not класс or not количество or блок.get("тип") not in ("grid", "mosaic", "feed"):
            continue
        for точка, колонок in лестницы.get(класс, []):
            if одинокий_хвост(занятых_ячеек(блок.get("тип", ""), количество), колонок):
                беды.append(
                    f"{спец['template_id']}: блок «{блок['заголовок']}» — {количество} карточек "
                    f"в {колонок} колонках на {точка or 320}px оставляют в последнем ряду одну"
                )
