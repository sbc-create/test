...
def missing_inputs() -> list[dict]:
    ...
    return [
        ...
        {
            "field": "yummy.theme_binding",
            "why": ("Пакет витрины обязан назвать тему из перечня схемы "
                    "(`tenant.theme`), а для витрин Yummy такого значения в перечне "
                    "нет ни одного. Подставить существующее — значит объявить облик "
                    "чужого направления обликом этого; выдумать новое — значит "
                    "решить за владельца, как витрина выглядит."),
            "format": "соответствие SITE_PROFILE → значение tenant.theme",
            "example_without_secret": "site → <новое значение перечня>, org → <новое значение>",
            "where_to_put": ("schemas/site-package.schema.json (перечень tenant.theme) "
                             "и sites/<siteId>/package.yaml"),
            "blocks": "создание пакета витрины: манифест без темы схему не проходит",
        },
        {
