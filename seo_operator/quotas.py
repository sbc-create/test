...
def classify_exception(exc: BaseException) -> FailureKind:
    """
    Классификация отказа обработчика джоба. Единственная реализация в проекте:
    scheduler.Worker обращается сюда, а не считает задержку сам.

    Неизвестное исключение считается transient: повторить безопасно, а вот
    молча похоронить джоб из-за незнакомого типа ошибки — нет.
    """
    ...
    if any(m in text for m in ("401", "unauthorized", "token expired", "invalid_token")):
        return FailureKind.AUTH
    if any(m in text for m in ("403", "forbidden", "access denied", "permissiondenied")):
        return FailureKind.FORBIDDEN
    if any(m in text for m in ("429", "rate limit", "too many requests", "quota exceeded")):
        return FailureKind.RATE_LIMIT
    if any(m in text for m in ("404", "not found")):
        return FailureKind.NOT_FOUND
    if any(m in text for m in ("schema", "validationerror", "keyerror", "unexpected field")):
        return FailureKind.SCHEMA
    if any(m in text for m in ("rights", "licence", "license", "правообладател")):
        return FailureKind.RIGHTS
    if any(m in text for m in ("500", "502", "503", "504", "bad gateway", "server error")):
