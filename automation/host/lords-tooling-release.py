...
def разложить(ревизия: str) -> Path:
    ...
    # Читаемо и проходимо всем, записываемо никому.
    #
    # `tempfile.mkdtemp` создаёт каталог с правами 0700, и одного снятия записи
    # мало: корень релиза остался бы `dr-x------`, то есть недоступным
    # пользователю claude. Помощник выкладки исполняется от claude и читает
    # оснастку — с 0700 он падал бы PermissionError ещё на разборе констант.
    выполнить("chmod", "-R", "a+rX", str(корень), таймаут=300)
...
def переключить(корень: Path) -> None:
    ...
    # Единый указатель на действующую оснастку.
    #
    # Закреплённый путь был вписан руками в трёх местах сразу: drop-in службы,
    # константа ОСНАСТКА в помощнике и умолчание --repo у публичной проверки.
    # Все три расходились независимо, и за одну ночь это стоило трёх разных
    # отказов. Указатель делает «что сейчас выложено» одним местом, а его
    # смену — атомарной.
    указатель = БАЗА / "current"
    временный = БАЗА / ".current.new"
    if временный.exists() or временный.is_symlink():
        временный.unlink()
    временный.symlink_to(корень)
    os.replace(временный, указатель)
    сказать(f"указатель current -> {корень.name}")
