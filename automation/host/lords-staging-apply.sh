...
# 0. Выделенные сайты
# --------------------------------------------------------------------------
# Сценарий перерисовывает конфигурацию и юниты витрин из монорепозитория. Сайт с
# собственным репозиторием он не трогает вовсе: иначе у одного домена окажутся
# два источника изменений, и «поправить модуль на таком-то домене» перестанет
# иметь однозначный ответ. Проверка стоит до снимка состояния и до первой записи.
stage "проверка выделенных сайтов"
site_list=()
for unit in "${UNITS[@]}"; do site_list+=("${unit%.service}"); done
if ! ( cd "${REPO_ROOT}" && "${PY}" -m factory cell extracted --modules "${site_list[@]}" ); then
  die "в списке есть сайты с собственным репозиторием; выкладывайте их оттуда"
fi
log "ни одна витрина списка не выделена"
