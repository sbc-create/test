...
ВИТРИНЫ = {
    ...
    "zona-01": {"unit": "nova-zona-01.service", "host": "zonafilm.space",
                "port": 9120},
    # zonafilm.cc: второй сайт семейства zona, заведён заданием
    # SITE_RELEASE_FULL_CYCLE. Профиль поставщика тот же, что у zona-01 —
    # кинокаталог целиком, без аниме-среза, — поэтому набор произведений у двух
    # витрин совпадает по устройству, а не по последующей сверке.
    #
    # Запись здесь и есть регистрация витрины в штатной публикации: без неё
    # `--sites zona-02` отклоняется, и каталог пришлось бы собирать в обход
    # инструмента. Имя юнита и порт не выбраны, а выданы аллокатором
    # (`automation/host/zonafilm-cc-allocate.py`) по четырём реестрам.
    "zona-02": {"unit": "nova-zona-02.service", "host": "zonafilm.cc",
                "port": 9123},
