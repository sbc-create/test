...
ВИТРИНЫ = {
    ...
    #
    # Публикация в production для них закрыта до выкладки артефакта шаблонов.
    # Раскладка `ВидАнимедиа` написана и закрыта version-gate
    # (`ПЕРЕРАБОТАНО_С` 1.2.0); выложенный артефакт её не содержит, и все
    # `/title/` отдают 503. Опубликовать больше карточек значило бы увеличить
    # число ссылок в никуда с 3999 до 7408.
    "animedia-01": {"unit": "nova-animedia-01.service", "host": "animedia.icu",
                    "port": 9121, "только_аниме": True, "аниме_видом": True,
                    "production_заблокирована":
                        "раскладка Animedia написана контуром «Шаблоны», но "
                        "закрыта version-gate (ПЕРЕРАБОТАНО_С 1.2.0) и не "
                        "выложена: в текущем артефакте ВИДЫ_1_1 = {lords, zona}, "
                        "все /title/ отдают 503"},
...
def выполнить(*, снимок_путь: Path = СНИМОК, детали: Path = ДЕТАЛИ,
    ...
    сайты = сайты or [s for s in sorted(ВИТРИНЫ)
                      if not ВИТРИНЫ[s].get("production_заблокирована")]
    for s in сайты:
        if s not in ВИТРИНЫ:
            raise PublishError(f"витрина {s} не входит в профиль публикации")
        причина = ВИТРИНЫ[s].get("production_заблокирована")
        if причина and применить:
            # Отказ именно на записи, а не на сборке: собрать и проверить
            # каталог нужно заранее, чтобы публикация после снятия блокера
            # была одной командой, а не новой работой.
            raise PublishError(
                f"{s}: публикация в production закрыта — {причина}. "
                f"Каталог собирается в черновик (--staging-out) и ждёт снятия "
                f"блокера контуром «Шаблоны»")
