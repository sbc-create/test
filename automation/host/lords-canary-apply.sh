...
#
# Версия 29. Отличие от версии 17: animedia-portal — светлая тема стала
# умолчанием витрины (переключатель на тёмную сохранён), контейнер получил
# токен gutter вместо зашитого отступа, /collections/ включён контентным
# типом пакета.
#
...
# блокер выкладки.
#
# Версия 5 (434f9397b308f8f75598004b076cbc28fb62a50ffe0ecc56c48d841323921b37)
# собрана интеграцией: в дерево шаблонов вошли изменения CORE — страница
# содержимого, каталог, разбивка на страницы, плеер и отрисовка. Версия 4
# принадлежала ветке TEMPLATES и на объединённом дереве уже не сходится: это
# не поломка сверки, а другой артефакт. Подменять сверку «свежим отпечатком по
# факту» нельзя — тогда она перестанет что-либо доказывать.
#
# То же самое повторилось на этом слиянии. Ветка шаблонов дошла до версии 24
...
readonly EXPECT_DIGEST="bec794536c84916f49af54660122a440160c4081eb4769146d7c8dc25434bbae"
