...
ЗАПРЕЩЕНО_В_ОКРУЖЕНИИ = (
    "AUDIT_TOKEN_",
    "CHANGESET_APPROVAL_KEY",
    "SITE_ENGINE_CONTROL_TOKENS",
    "APPROVAL_CALLER",
)
ЧУЖИЕ_CREDENTIALS = (
    "approval-signing-key",
    "approval-caller-control-api",
    "approval-caller-changeset-worker",
    "approval-caller-fingerprints",
    "site-engine-control-tokens",
    "audit-token-architect",
    "audit-token-audit-bridge",
    "audit-token-changeset-worker",
    "audit-token-registry",
    "audit-token-seo",
    "audit-token-content",
    "audit-token-monitoring",
    "audit-token-backup",
    "audit-token-integrations",
    "audit-token-qwen",
    "audit-token-fingerprints",
)


def показать(*ключи):
    вывод = subprocess.run(
        ["systemctl", "show", ЮНИТ, *[f"-p{к}" for к in ключи]], capture_output=True, text=True
    ).stdout
...
if pid and pid != "0":
    with open(f"/proc/{pid}/environ", "rb") as ф:
        сырое = ф.read().decode("utf-8", "replace")
    ...
    with open(f"/proc/{pid}/cmdline", "rb") as ф:
        аргументы = ф.read().decode("utf-8", "replace").replace("\0", " ").strip()
    дети = subprocess.run(["pgrep", "-P", pid], capture_output=True, text=True).stdout.split()
    for д in дети:
        with contextlib.suppress(OSError), open(f"/proc/{д}/cmdline", "rb") as ф:
            потомки.append(ф.read().decode("utf-8", "replace").replace("\0", " ").strip())

итог["env_var_names"] = имена
итог["secrets_in_env"] = sorted(
    и for и in имена if any(и.startswith(п) or и == п for п in ЗАПРЕЩЕНО_В_ОКРУЖЕНИИ)
)
итог["visible_credentials"] = видимые
итог["foreign_credential_refs"] = sorted(set(видимые) & set(ЧУЖИЕ_CREDENTIALS))
итог["long_lived_credential_refs"] = [и for и in видимые if и.startswith("audit-token-templates")]
итог["process_args"] = аргументы
итог["child_process_args"] = потомки
итог["denials"] = {
    "approval_private_key": как(
        "templates-cp", "cat /etc/site-factory/credentials/approval-signing-key"
    )
    != 0,
    "credentials_dir": как("templates-cp", "ls /etc/site-factory/credentials") != 0,
    "sudo": как("templates-cp", "sudo -n true") != 0,
    "site_unit_restart": как("templates-cp", "systemctl restart lords-01.service") != 0,
