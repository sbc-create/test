...
#: Публичный адрес связи. Значение подтверждено владельцем и объявлено в
#: `legal_profile.contact_email` профиля каждой витрины; тем же адресом
#: подписан подвал в `factory/lords/render.py`. Здесь он один на модуль, а не
#: повторён в каждой странице, и переопределяется окружением юнита или ключом
#: `contact_email` манифеста — витрине с другим адресом не придётся править код.
#: Пустое значение означает, что строки связи просто нет: выдумывать адрес,
#: которого владелец не называл, нельзя.
КОНТАКТ = (
    os.environ.get("LORDS_CONTACT_EMAIL")
    or МАНИФЕСТ.get("contact_email")
    or "sbc.claude@yandex.ru"
).strip()
...
def _строка_связи() -> str:
    """Строка связи в подвале витрины.

    Адрес — способ написать редакции сайта, и подписан он именно так: ни
    правообладателем, ни юридическим лицом владелец его не называл. Если
    адреса нет, строки тоже нет: пустое место честнее выдуманных реквизитов и
    честнее сообщения о незаполненной переменной.
    """
    if not КОНТАКТ:
        return ""
    адрес = html.escape(КОНТАКТ)
    return f'<span>Связаться: <a href="mailto:{адрес}">{адрес}</a></span>'
