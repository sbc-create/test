...
#: процесс обслуживает один домен, а счётчик привязан к домену. Пустое
#: значение означает «счётчика нет» и даёт страницу без тега, а не тег,
...
#: Признак уже вставленного тега. По нему же проверяется идемпотентность:
#: страница, прошедшая через отдачу дважды, не должна получить два счётчика.
...
def тег_метрики() -> str:
    ...
    Вебвизор выключен намеренно — реестр аналитики требует от него `false`,
    а включается он отдельным решением владельца.
    """
    if not СЧЁТЧИК_МЕТРИКИ.isdigit():
        return ""
    н = СЧЁТЧИК_МЕТРИКИ
    return (
        f'<script {МЕТКА_МЕТРИКИ}"{н}">'
        "(function(){"
...
def со_счётчиком(тело: bytes, тип: str) -> bytes:
    """Вставить тег аналитики в HTML-ответ. Единственное место, где это делается.

    Раньше вставка стояла в самих шаблонах. Так она и теряется: каждый новый
    вид оформления — это новый `<head>`, про который надо не забыть, и в
    переработке шаблонов Zona и Animedia про него забыли ровно так. Теперь
    вставка живёт в отдаче ответа, через которую проходят все двадцать семь
    выходов обработчика, и новому шаблону нечего забывать.

    Идемпотентность проверяется по метке, а не по доверию: страница, уже
    несущая счётчик, второго не получит.
    """
    if not СЧЁТЧИК_МЕТРИКИ.isdigit():
        return тело
    if "text/html" not in тип:
        return тело
    if МЕТКА_МЕТРИКИ.encode("utf-8") in тело:
        return тело
    if b"</head>" not in тело:
        return тело
    return тело.replace(b"</head>", тег_метрики().encode("utf-8") + b"</head>", 1)
