...
#: СТРУКТУРНО — составом и порядком блоков, подачей героя, ритмом навигации,
#: типом сетки. При этом функциональные и доступностные свойства у всех одни:
#: ровно один H1, живые органы управления, отсутствие переполнения, честные
#: данные. Поэтому здесь объявление, а отрисовка — общая.
#:
#: Поля шаблона:
#:   герой   — "slider" (дорожка из восьми), "single" (один редакционный кадр
#:             без органов управления), "compact" (лид без кадра);
#:   недельный — "tabs" (вкладки) или "stacked" (три ленты подряд);
#:   новое   — "list" (строки с датой) или "rail" (горизонтальная лента);
#:   свежие  — сколько карточек в плотной сетке новинок (0 — блока нет);
#:   порядок — состав и порядок блоков главной.
#:
#: Шаблон выбирается манифестом витрины (`template_profile`). Неизвестное имя
#: и отсутствие поля дают `classic` — то, что витрина отдаёт сегодня.
ЗОНА_ШАБЛОН_ПО_УМОЛЧАНИЮ = "classic"
ЗОНА_ШАБЛОНЫ = {
    "classic": {
        "имя": "Классическая витрина",
        "описание": "Слайдер героя, вкладки недели, поступления списком.",
        "герой": "slider", "недельный": "tabs", "новое": "list", "свежие": 0,
        "порядок": ["hero", "weekly", "added", "kinds", "genres", "collections", "seo"],
    },
    "catalog-first": {
        "имя": "Каталог сразу",
        "описание": "Один кадр героя, следом плотная сетка новинок; ленты ниже.",
        "герой": "single", "недельный": "stacked", "новое": "rail", "свежие": 21,
        "порядок": ["hero", "fresh", "kinds", "weekly", "added", "genres",
                    "collections", "seo"],
    },
    "editorial": {
        "имя": "Редакционная",
        "описание": "Слайдер, сразу поступления списком, затем ленты недели.",
        "герой": "slider", "недельный": "stacked", "новое": "list", "свежие": 0,
        "порядок": ["hero", "added", "weekly", "genres", "kinds", "collections", "seo"],
    },
    "discovery": {
        "имя": "Навигационная",
        "описание": "Без кадра героя: сверху жанры и подборки, поиск по вкусу.",
        "герой": "compact", "недельный": "tabs", "новое": "list", "свежие": 14,
        "порядок": ["hero", "genres", "collections", "weekly", "fresh", "kinds",
                    "added", "seo"],
    },
    "rail-dense": {
        "имя": "Ленточная",
        "описание": "Один кадр, три ленты недели подряд, поступления лентой.",
        "герой": "single", "недельный": "stacked", "новое": "rail", "свежие": 28,
        "порядок": ["hero", "weekly", "added", "fresh", "kinds", "genres",
                    "collections", "seo"],
    },
    "grid-lead": {
        "имя": "Сеточная",
        "описание": "Лид вместо кадра и крупная сетка новинок первым экраном.",
        "герой": "compact", "недельный": "tabs", "новое": "list", "свежие": 35,
        "порядок": ["hero", "fresh", "weekly", "kinds", "added", "genres",
                    "collections", "seo"],
    },
}

#: Имя шаблона ЭТОЙ витрины. Объявляет манифест; неизвестное имя не подменяет
#: витрину молча — оно уходит в stderr и откатывается к классической.
...
if ЗОНА_ШАБЛОН not in ЗОНА_ШАБЛОНЫ:
    if СЕМЕЙСТВО == "zona":
        print(f"[zona] манифест объявляет неизвестный шаблон {ЗОНА_ШАБЛОН!r}; "
              f"известны: {', '.join(sorted(ЗОНА_ШАБЛОНЫ))}. "
