...
{self._подвал_зона()}
    ...
    def тайтл(self, запись: dict, деталь: dict) -> str:
        ...
        тело = (
            ...
            f'{self._точка_комментариев(запись)}{expand_js}')
        разметка = [
    ...
    def _точка_комментариев(self, запись: dict) -> str:
        ...
        if СЕМЕЙСТВО != "zona":
            return ""
        slug = (запись or {}).get("slug") or ""
        if not slug:
            return ""
        return (f'<section class="zcomments" id="comments" data-comments="0" '
                f'data-comments-space="zona" '
                f'data-comments-subject="{html.escape(slug)}" '
