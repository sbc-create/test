...
def нормализовать(с: str) -> str:
    с = unicodedata.normalize("NFKD", (с or "").lower())
    return re.sub(r"[^a-zа-яё0-9]+", "", с.replace("ё", "е"))
...
class Данные:
    ...
    def искать(self, q: str, предел: int = 120) -> list[dict]:
        """Терпимый поиск: подстрока, затем совпадение по началу слов."""
        нq = нормализовать(q)
        if not нq:
            return []
        точные = [з for з in self.items if нq in з["_n"]]
        if len(точные) >= предел:
            return точные[:предел]
        # Терпимость к опечатке: допускаем одну лишнюю/пропущенную букву.
        мягкие = []
        for з in self.items:
            if з in точные:
                continue
            н = з["_n"]
            if abs(len(н) - len(нq)) <= 2 and sum(1 for a, b in zip(н, нq) if a != b) <= 2:
                мягкие.append(з)
            if len(точные) + len(мягкие) >= предел:
                break
        return (точные + мягкие)[:предел]
