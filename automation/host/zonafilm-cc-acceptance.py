...
class Приёмка:
    ...
    def коллекции(self, плитки: list[str]) -> dict:
        ...
        #
        # Состояния сервера: `resolving` — издатель выдан и кандидаты источника
        # найдены, дорожку подтверждает SDK в браузере; `nosource` — источника
        # у записи нет; `noaccess` — издатель витрине не выдан.
        с_видео = наборы.get("/collection/video_available/", [])
        состояния: dict[str, int] = {}
        без_блока, пустой_iframe = [], []
        for href in с_видео[:12]:
            о = self.к.get(href)
            # Совпадение обязано лежать внутри ОДНОГО тега. Прежний шаблон
            # `data-player[^>]*data-state="…"` этого не требовал, а в странице
            # есть CSS, где оба имени встречаются в одном правиле и `>` между
            # ними нет: `[^>]*` перепрыгивал из таблицы стилей в селектор и
            # возвращал `noaccess` там, где контейнер отдавал `resolving`.
            # Измерение было артефактом регулярки, а не состоянием витрины.
            состояние = найти_все(
                r'<[a-z-]+(?=[^>]*\bdata-player\b)[^>]*\bdata-state="([a-z]+)"[^>]*>',
                о.body)
