...
#: Что считается разделителем в названии при сравнении запроса и названия.
#: Дефис, двоеточие, точка и пробел — одно и то же: «Жил-был воин» и
#: «Жил был воин» обязаны совпасть, иначе поиск зависит от того, как человек
#: набрал разделитель.
_РАЗДЕЛИТЕЛИ_НАЗВАНИЯ = re.compile(r"[^0-9a-zа-яё]+")


def сверить(строка: str) -> str:
    """Название и запрос приводятся к одному виду. Правило объявлено здесь.

    Регистр, «ё», знаки препинания и кратность пробелов не должны решать,
    найдётся тайтл или нет.
    """
    с = (строка or "").casefold().replace("ё", "е")
    return _РАЗДЕЛИТЕЛИ_НАЗВАНИЯ.sub(" ", с).strip()


def поиск(соед, запрос: str, предел: int = 60) -> list[dict]:
    """Поиск по собственному каталогу контура. Запасной, а не основной.

    Основной поиск ведёт приложение, и подменять его нельзя: у него свой
    индекс и свои правила. Но измерено, что на целом классе запросов он
    отдаёт ноль при непустом каталоге — он спрашивает поставщика по всему
    его каталогу (фильмы, сериалы, аниме), берёт первую сотню найденного и
    только потом отсеивает не-аниме. Для частого слова первая сотня
    поставщика состоит из не-аниме целиком, и до посетителя не доходит
    ничего: «воин» — ноль результатов при 115 аниме с этим словом в
    названии.

    Здесь — ровно то же содержимое, которое витрина и так показывает, взятое
    из контура: ни одной записи, которой в каталоге нет. Порядок объявлен и
    детерминирован: точное совпадение, затем начало названия, затем вхождение;
    при равенстве — более короткое название, затем идентификатор.
    """
    цель = сверить(запрос)
    if not цель:
        return []
    ...
    строки = соед.execute(
        ...
        SELECT entity_id, canonical_path, title_ru, title_original, poster_path,
               year, kind
          FROM entity
         WHERE canonical_path IS NOT NULL AND canonical_path != ''""").fetchall()
    найдено = []
    for с in строки:
        имена = [сверить(с["title_ru"]), сверить(с["title_original"])]
        имена = [и for и in имена if и]
        ранг = None
        for и in имена:
            if и == цель:
                ранг = 0
            elif и.startswith(цель):
                ранг = min(ранг, 1) if ранг is not None else 1
            elif цель in и:
                ранг = min(ранг, 2) if ранг is not None else 2
        if ранг is None:
            continue
        найдено.append((ранг, len(имена[0]), с["entity_id"], с))
    найдено.sort(key=lambda п: (п[0], п[1], п[2]))
    return _карточки([с for _, _, _, с in найдено[:предел]])
