...
#: ANIMEDIA-MODERATION-01: премодерация получила модерацию. Под 1.2.7
#: комментарий уходил в pending, виден был только автору, и одобрить его было
#: нечем — очередь без двери.
ОФОРМЛЕНИЕ_1_2_8 = "1.2.8"

#: ANIMEDIA-MODERATION-02: очередь разделена на ожидающие и отклонённые.
#: Под 1.2.8 счётчик считал одно, а список показывал другое.
ОФОРМЛЕНИЕ_1_2_9 = "1.2.9"
...
class ВидОснова(Вид):
    ...
    def блок_обсуждения(self, запись: dict, *, возврат: str = "") -> str:
        ...
        def строка_сообщения(к: dict) -> str:
            ...
            # Автору его собственное ожидающее сообщение приходит В ЛЕНТЕ —
            # модуль отдаёт его нарочно, иначе человек решит, что отправка не
            # сработала, и напишет ещё раз. Но без пояснения он решит
            # обратное: что сообщение уже опубликовано. Поэтому автору
            # сказано прямо, что видно оно пока только ему.
            моё = bool(к.get("моё"))
            if ждёт and моё:
                метка = ('<span class="acomm__pending" '
                         'title="Появится в ленте после одобрения модератором">'
                         'на проверке — видно только вам</span>')
            elif ждёт:
                метка = '<span class="acomm__pending">на проверке</span>'
            elif str(к.get("status") or "") == СООБЩЕСТВО.СТАТУС_ОТКЛОНЁН:
                метка = '<span class="acomm__pending">отклонено</span>'
            else:
                метка = ""
            ...
            отклонено = str(к.get("status") or "") == СООБЩЕСТВО.СТАТУС_ОТКЛОНЁН
            решение = ""
            if модератор and (ждёт or отклонено):
                # У уже отклонённого «Отклонить» ничего не меняет и выглядит
                # сломанным, поэтому кнопка там одна — вернуть в ленту.
                кнопки = (
                    '<button class="acomm__ok" type="submit" name="decision" '
                    'value="approved">Одобрить</button>'
                    '<button class="acomm__no" type="submit" name="decision" '
                    'value="rejected">Отклонить</button>' if ждёт else
                    '<button class="acomm__ok" type="submit" name="decision" '
                    'value="approved">Вернуть в ленту</button>')
                решение = (
        ...
        # Очередь модерации — отдельным списком, потому что модуль отдаёт её
        # отдельно: в ленту ожидающее не попадает никому, кроме автора. Без
        # этого блока ключ модератора ничего не открывал бы — кнопки решения
        # просто негде было бы показать.
        очередь = ""
        if модератор and с.на_модерации:
            # ВНИМАНИЕ НА ИМЕНА. `на_модерации` содержит и ожидающие, и
            # отклонённые, а `всего_на_модерации` считает ТОЛЬКО ожидающие.
            # Измерено: одно отклонённое плюс одно ожидающее дают список из
            # двух и счётчик 1. Смешать их — значит написать «На проверке: 1»
            # над списком из двух и предложить «Отклонить» тому, что уже
            # отклонено.
            #
            # Отклонённое модератору показывать надо: иначе ошибочное решение
            # нечем отменить. Но это другая группа и другое действие.
            ожидают = [к for к in с.на_модерации
                       if str(к.get("status") or "") == СООБЩЕСТВО.СТАТУС_ОЖИДАЕТ]
            отклонены = [к for к in с.на_модерации
                         if str(к.get("status") or "") == СООБЩЕСТВО.СТАТУС_ОТКЛОНЁН]
            части = []
            if ожидают:
                ещё = ("" if len(ожидают) <= 20 else
                       f'<p class="acomm__none">…и ещё {len(ожидают) - 20}.</p>')
                части.append(
                    f'<h3 class="acomm__h">На проверке: {len(ожидают)}</h3>'
                    f'<ul class="acomm__list">'
                    f'{"".join(строка_сообщения(к) for к in ожидают[:20])}'
                    f'</ul>{ещё}')
            if отклонены:
                части.append(
                    f'<h3 class="acomm__h">Отклонённые: {len(отклонены)}</h3>'
                    f'<ul class="acomm__list">'
                    f'{"".join(строка_сообщения(к) for к in отклонены[:20])}'
                    f'</ul>')
            if части:
                очередь = (
                    f'<section class="acomm__queue" data-moderation-queue="1" '
                    f'data-pending="{len(ожидают)}" '
                    f'data-rejected="{len(отклонены)}" '
                    f'aria-label="Очередь модерации">{"".join(части)}</section>')
        ...
        список_сообщений = (
            очередь + (f'<ul class="acomm__list">{лента}</ul>' if лента else пусто))
        форма = (
...
class Обработчик(BaseHTTPRequestHandler):
    ...
    @staticmethod
    def _ключ_модератора() -> str:
        """Ключ модератора — из credential systemd, а не из окружения.

        Правило контура: секрет приходит файлом, который systemd кладёт в
        приватный каталог службы и убирает вместе с ней. `Environment=` в
        юните для этого не годится — значение видно в `systemctl show`, в
        `/proc/<pid>/environ` и утекает в любой отчёт, снявший окружение
        процесса.

        Переменная окружения оставлена запасным путём: она нужна стендам, где
        systemd нет вовсе. На витрине задаётся credential.
        """
        каталог = os.environ.get("CREDENTIALS_DIRECTORY")
        if каталог:
            путь = Path(каталог) / os.environ.get(
                "ANIMEDIA_MODERATOR_CREDENTIAL", "animedia_community_moderator_key")
            try:
                if путь.is_file():
                ...
                pass
        return os.environ.get("ANIMEDIA_COMMUNITY_MODERATOR_KEY", "")

    def _модератор(self) -> bool:
        ключ = self._ключ_модератора()
        if not ключ:
            return False
        сырое = self.headers.get("Cookie") or ""
        for кусок in сырое.split(";"):
