...
def карточка_тайтла(с: dict, каппы: dict | None = None,
    ...
    return (
        # Блок приходит последним узлом body и там остаётся: чужой ребёнок
        # в контейнере React ломает восстановление страницы, а перенос
        # скриптом после гидратации — та же гонка, только позже.
