...
def действия(стр, база: str, снимки: Path, метка: str,
             без_записи: bool = False) -> list:
    ...
    if без_записи:
        # На боевом домене комментарий не отправляем: он останется на глазах
        # у посетителей навсегда, а проверка формы стоит не дороже. Форма и
        # включённость раздела проверяются разметкой, отправка — на том же
        # артефакте локально.
        итог.append({"action": "комментарий",
                     "pass": True,
                     "details": {"skipped_on_live":
                                 "форма и раздел проверены разметкой; "
                                 "отправка проверена на том же артефакте локально"}})
    else:
...
def main() -> int:
    ...
    р.add_argument("--no-writes", action="store_true",
                   help="не оставлять на витрине необратимых следов (комментарий)")
    a = р.parse_args()
