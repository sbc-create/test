...
def main() -> int:
    ...
    #
    # Имя берётся из config/site.json, а не из константы здесь: то же поле
    # читает штатный исполнитель выпусков. Объявление, которое никто не
    # исполняет, рано или поздно разойдётся с действительностью и соврёт.
    сайт = json.loads((КОРЕНЬ / "config" / "site.json").read_text(encoding="utf-8"))
    имя_точки = str(сайт.get("entrypoint") or "").strip()
    if not имя_точки:
        raise SystemExit(
            "config/site.json не объявляет entrypoint: запускать нечего. "
            "Пустое поле — не повод взять файл по умолчанию.")
    точка = str(КОРЕНЬ / "src" / имя_точки)
...
def main() -> int:
    ...
    # 9.5. Объявление точки входа. Штатный исполнитель выпусков читает
    #      `config/site.json` и ищет `src/<entrypoint>`; без поля ему остаётся
    #      гадать, а гадать про точку входа — значит однажды запустить не тот
    #      файл. Значение load-bearing: `run.py` берёт его отсюда, поэтому
    #      расхождение объявления с действительностью невозможно молча.
    сайт_файл = корень / "config" / "site.json"
    сайт = json.loads(сайт_файл.read_text(encoding="utf-8"))
    сайт["entrypoint"] = "serve.py"
    сайт["entrypoint_note"] = (
        "Файл в src/. Запускает витрину: импортирует закреплённый артефакт "
        "шаблона и ставит поверх правки этого сайта. Артефакт при этом остаётся "
        "байт в байт тем, что записан в pins.lock.json.")
    изменения.append(("config/site.json", записать(
        сайт_файл, json.dumps(сайт, ensure_ascii=False, indent=2) + "\n")))
