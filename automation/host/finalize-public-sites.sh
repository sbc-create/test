...
# --------------------------------------------------------------------------
# 0. Выделенные сайты: отказ ДО первой мутации
# --------------------------------------------------------------------------
# Сценарий перерисовывает nginx-конфигурацию и перезапускает юниты витрин из
# монорепозитория. Для сайта, уехавшего в собственный репозиторий, это молчаливая
# перезапись чужого выпуска: снаружи ничего не меняется, а внутри у домена
# появляется второй источник изменений. Проверка стоит до снимка nginx и до
# первой записи — «половина сценария» хуже, чем ни одной.
stage "проверка выделенных сайтов"
if ! ( cd "${REPO}" && "${PY}" -m factory cell extracted --modules "${LORDS_SITES[@]}" ); then
  die "в списке есть сайты с собственным репозиторием; выкладывайте их оттуда"
fi
log "  ни один сайт списка не выделен — продолжаю"
