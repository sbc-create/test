...
def _прочитать_позицию() -> str:
    try:
        return str(json.loads(ПОЗИЦИЯ.read_text(encoding="utf-8")).get("after") or "")
    except (OSError, ValueError):
        return ""


def _записать_позицию(после: str, пройдено: int) -> None:
    ПОЗИЦИЯ.parent.mkdir(parents=True, exist_ok=True)
    ПОЗИЦИЯ.write_text(json.dumps(
        {"after": после, "walked_last_run": пройдено,
         "at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())},
        ensure_ascii=False, indent=2) + "\n", encoding="utf-8")


def _круг(идентификаторы: list[str], кэш: Path, *, кроме: set[str],
          сколько: int) -> list[str]:
    ...
    """
    кольцо = [ид for ид in sorted(_сериалы(идентификаторы, кэш)) if ид not in кроме]
    if not кольцо or сколько == 0:
        return []
    if сколько < 0:                       # весь круг за один прогон
        сколько = len(кольцо)
    после = _прочитать_позицию()
    начало = 0
    if после:
...
def main(argv=None) -> int:
    ...
    # Умолчание — ВЕСЬ круг за прогон. Замерено: сериалов 20 469 из 53 688
    # записей каталога, 0.2–0.28 с на запрос, то есть круг целиком около часа
    # с четвертью при пределе юнита в шесть часов.
    #
    # Дробить круг между прогонами здесь нечем: узкое место не бюджет, а
    # расписание. Таймер суточный, и круг на одиннадцать прогонов означал бы
    # одиннадцать суток до новой серии — то есть ровно ту задержку, ради
    # которой круг и вводится. Считать надо от частоты запуска, а не от
    # размера бюджета.
    ap.add_argument("--ring-budget", type=int, default=-1,
                    help="предел запросов на круговой обход сериалов; "
                         "-1 весь круг за прогон, 0 выключает круг")
    ...
    отрезок = _круг(идентификаторы, a.cache, кроме=set(продолжающиеся),
                    сколько=a.ring_budget)
    if отрезок:
        print(f"[круг] {len(отрезок)} сериалов начиная после "
              f"{_прочитать_позицию() or 'начала'}", file=sys.stderr)
        ...
        кольцевые_записи = [з for з in записи
                            if str(з.get("external_id")) in set(отрезок)]
        _, отчёт_круга = detail_enrichment.enrich_items(
            кольцевые_записи, fetcher=fetcher, contract=contract,
            cache=кольцевой, budget=len(отрезок), order=отрезок)
        # Позиция двигается на число ПРОЙДЕННЫХ по кругу, а не на размер
        # очереди: иначе срочные незаметно замедляли бы продвижение.
        _записать_позицию(отрезок[-1], len(отрезок))
    else:
        отчёт_круга = {"skipped": "круг выключен" if a.ring_budget == 0
                       else "сериалов нет"}
    ...
    итог = {
        ...
        "ongoing": {"count": len(продолжающиеся), "ttl": a.ongoing_ttl,
                    "report": отчёт_идущих},
        "ring": {"walked": len(отрезок), "budget": a.ring_budget,
                 "ttl": a.ring_ttl, "position_after": _прочитать_позицию(),
                 "report": (отчёт_круга if isinstance(отчёт_круга, dict)
                            else {k: v for k, v in vars(отчёт_круга).items()
