...
def _снижения(прежние: dict[str, dict[int, tuple[int, int]]], кэш: Path) -> dict:
    """Записать, где число доступных серий УМЕНЬШИЛОСЬ. Не откатывать.

    Сначала здесь стоял безусловный откат: «понижение — это сбой выдачи, а не
    серий стало меньше». Это неверно, и разделять надо по ИСТОЧНИКУ, а не по
    направлению. Отзыв прав и снятие потока — настоящие события: ссылка на
    снятую серию ведёт на страницу, где нечего смотреть, и это хуже
    заниженного числа. На соседней витрине так и вышло: в снимке держалось
    218 доступных серий при пятидесяти в прямом ответе поставщика.

    Здесь источник всегда прямой ответ: и круги, и проход по срочным спрашивают
    API и кладут ответ в кэш. Значит понижение принимается. Защищать надо
    обратное направление — доставленный СНИМОК бывает старше опроса, — и это
    забота того, кто накладывает снимок, а не добора.

    Остаётся видимость: понижения перечисляются в отчёте, потому что пачка
    снятий за один прогон — повод посмотреть на источник, а не молча принять.
    """
    снижено = []
