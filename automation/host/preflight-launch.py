...
плохо: list[str] = []
внимание: list[str] = []
#: Мешает не команде, а ВЫПУСКУ после неё. Отдельный список, потому что
#: смешивать их нельзя в обе стороны: сказать «команду передавать нельзя»
#: из-за того, что случится на следующем шаге, — значит задержать нужное
#: действие; промолчать — значит отдать домен, который не наполнится.
потом: list[str] = []
...
def итог() -> None:
    ...
    if внимание:
        ...
        for и in внимание:
            print("   [ ! ] ", и)
    if потом:
        print(f"\nмешает не команде, а выпуску после неё: {len(потом)}")
        for и in потом:
            print("   [позже]", и)
    if плохо:
        print(f"\nНЕ ГОТОВО: {len(плохо)}")
        ...
        if потом:
            print("Выпуск после неё пока не пройдёт — см. список выше.")
