...
#: Сколько ждать ответа приложения, прежде чем отдать 503.
#:
#: Было 25 секунд, и приёмка ловила 503 на главной: пять раз из ста,
#: всегда на первой странице новой сессии браузера. Это не зависание —
#: измерено подряд шесть заходов: 6,0–7,4 с и полные 248 КБ каждый раз.
#: Под параллельной нагрузкой (два движка, три ширины, три домена) сборка
#: главной выходит за 25 с, и посредник превращал медленный, но успешный
#: ответ в ошибку. Ждать дольше лучше, чем показать 503: страница всё
#: равно придёт, а ошибку посетителю уже не отменить.
#:
...
#: Человеческие имена провайдеров оценок. Один словарь на файл: два
#: написания одного источника однажды разойдутся, и «IMDb» в разделе
#: перестанет совпадать с «imdb» в Топ-100.
ИМЕНА_ПРОВАЙДЕРОВ = {"imdb": "IMDb", "kp": "Кинопоиск"}

#: Слова, обозначающие форму издания, а не название. Приложение ищет по
#: названию, и «Season 3» в запросе — это уточнение, а не часть имени.
#:
#: Слова конкретного тайтла сюда не попадают. «dreaming» тут стояло: его
#: внесли, разбирая запрос «Grand Blue Dreaming Season 3». Список после
#: этого выбрасывал слово «Dreaming» из ЛЮБОГО названия, а нужный случай
#: закрывается и без него — обычным сокращением справа.
ИЗДАНИЕ = ("season", "сезон", "сезона", "сезонов", "series", "серия", "серии",
           "part", "часть", "tv", "ova", "ona", "movie", "фильм")

#: Чем разделены слова запроса. Одних пробелов мало: «Жил-был-воин» — та же
#: строка, что «Жил-был воин», но приложение по ней не находит ничего, и
#: сокращение справа без разбиения по дефису тоже не спасало: сокращать было
#: нечего, вся строка считалась одним словом.
РАЗДЕЛИТЕЛИ = re.compile(r"[\s,;:/\\_·–—-]+")


def варианты_запроса(q: str) -> list[str]:
    ...
    запрос сокращается справа — сначала отбрасываются слова издания и числа,
    затем по одному слову с конца.

    Это расширение поиска, а не подмена: варианты используются только когда
    полный запрос не нашёл ничего, и каждый проверяется по настоящему
    индексу приложения. Вариант, который тоже ничего не нашёл, никуда не
    попадает — выдуманных результатов здесь появиться неоткуда.
    """
    # Ведущие и замыкающие разделители снимаются до разбора: иначе пустой
    # кусок в начале сдвигает список разделителей относительно слов, и
    # «,Жил-был» собиралось обратно как «Жил,был» — строкой, которой нет ни
    # в одном названии.
    строка = q.strip()
    while строка and РАЗДЕЛИТЕЛИ.fullmatch(строка[0]):
        строка = строка[1:]
    while строка and РАЗДЕЛИТЕЛИ.fullmatch(строка[-1]):
        строка = строка[:-1]
    куски = [ч for ч in РАЗДЕЛИТЕЛИ.split(строка) if ч]
    если_нет = РАЗДЕЛИТЕЛИ.findall(строка)
    if not куски:
        return []
    # Разделители сохраняются как набраны. Приложение ищет вхождение строки
    # целиком, поэтому «Жил-был» находит «Жил-был воин», а «Жил был» — нет:
    # склеивать куски пробелом значило бы предлагать приложению строку,
    # которой ни в одном названии нет.
    def собрать(n: int) -> str:
        части = [куски[0]]
        for i in range(1, n):
            части.append(если_нет[i - 1] if i - 1 < len(если_нет) else " ")
            части.append(куски[i])
        return "".join(части).strip()

    варианты = []
    # Первым — запрос без хвоста из слов издания и чисел: «… Season 3» —
    # уточнение, а не часть имени. Отбрасывается именно ХВОСТ: выброшенное
    # слово из середины разорвало бы строку, которую приложение ищет целиком.
    конец = len(куски)
    while конец > 1 and (куски[конец - 1].lower() in ИЗДАНИЕ
                         or куски[конец - 1].isdigit()):
        конец -= 1
    if конец < len(куски):
        варианты.append(собрать(конец))
    for n in range(конец - 1, 0, -1):
        в = собрать(n)
        if в and в not in варианты and в != строка:
            варианты.append(в)
    # Замена разделителей пробелами — отдельный, последний вариант: человек
    # мог набрать дефис там, где в названии пробел.
    пробелами = " ".join(куски)
    if пробелами != строка and пробелами not in варианты:
        варианты.append(пробелами)
    return [в for в in варианты if в and в != строка]
...
class Обработчик(BaseHTTPRequestHandler):
        ...
        if ВЕРХОВОЙ:
            ...
            if путь.rstrip("/") in ("/search", "/api/search"):
                ...
                if ответ is not None:
                    тело, тип, код = ответ
                    # Индекс приложения на целом классе запросов отдаёт ноль
                    # при непустом каталоге. Тогда — и только тогда — отвечает
                    # собственный каталог контура.
                    if код == 200 and self._пусто(путь.rstrip("/"), тело):
                        своё = self._запасной_поиск(путь.rstrip("/"),
                                                    (зпр.get("q") or [""])[0])
                        if своё is not None:
                            с_телом, с_типом = своё
                            return self._отдать(self._обогатить(с_телом, с_типом),
                                                с_типом)
    ...
    #: Сколько совпадений показывает запасной поиск. Одна страница без
    #: листалки: листать выдачу, которой у приложения нет вовсе, нечем.
    НАЙДЕНО_ПРЕДЕЛ = 60

    def _запасной_поиск(self, путь: str, q: str):
        """Ответ собственного каталога, когда индекс приложения отдал ноль.

        Это не подмена поиска приложения: собственный каталог спрашивается
        ТОЛЬКО после того, как приложение и все расширенные варианты запроса
        вернули пусто. Если приложение что-то нашло — ответ его.

        Зачем вообще: измерено на боевом каталоге, что запрос «воин» отдаёт
        ноль результатов при 115 аниме с этим словом в названии. Причина в
        приложении и названа в `docs/handoff/YUMMY-SEARCH-ANIME-FILTER.md`:
        оно спрашивает поставщика по всему его каталогу, берёт первую сотню
        найденного и только потом отсеивает не-аниме. На частом слове первая
        сотня поставщика состоит из не-аниме целиком.

        Показывается ровно то, что уже есть в контуре витрины: ни одной
        записи, которой нет в каталоге, и ни одного выдуманного поля.
        """
        if СВЯЗЬ is None or not (q or "").strip():
            return None
        try:
            with self._соединение() as соед:
                найдено = СВЯЗЬ.поиск(соед, q, self.НАЙДЕНО_ПРЕДЕЛ)
        except Exception:
            return None
        if not найдено:
            return None
        if путь == "/api/search":
            # Подсказки отдаются в той же форме, что у приложения: иначе их
            # клиент просто не покажет. Форма взята из его же ответа.
            элементы = [{
                "slug": (з["canonical_path"] or "").rsplit("/", 1)[-1],
                "href": з["canonical_path"],
                "name": з["title"],
                "posterUrl": з.get("poster"),
                "year": з.get("year"),
                "typeLabel": СТРАНИЦЫ.ВИДЫ.get(з.get("kind") or ""),
            } for з in найдено[:8]]
            тело = json.dumps({"status": "ok", "items": элементы},
                              ensure_ascii=False).encode("utf-8")
            return тело, "application/json; charset=utf-8"
        оболочка = self._оболочка()
        if оболочка is None:
            return None
        в = ВАРИАНТЫ_МОД.вариант(ВАРИАНТ_ДОМЕНА) if ВАРИАНТЫ_МОД else {}
        хвост = (f" Показаны первые {self.НАЙДЕНО_ПРЕДЕЛ}."
                 if len(найдено) >= self.НАЙДЕНО_ПРЕДЕЛ else "")
        тело = (
            '<p class="portal-empty">Индекс приложения по этому запросу не вернул '
            f'ничего. Ниже — {len(найдено)} совпадений из собственного каталога '
            f'витрины.{хвост} Порядок: точное совпадение, затем начало названия, '
            'затем вхождение.</p>'
            + СТРАНИЦЫ.секция(f"Найдено по запросу «{q}»",
                              "Совпадение по названию и оригинальному названию.",
                              найдено))
        страница = СТРАНИЦЫ.собрать(
            СТРАНИЦЫ.подставить_запрос(оболочка, q), f"Поиск: {q}",
            "Результаты собственного каталога витрины.", тело, вариант=в)
        return страница, "text/html; charset=utf-8"
    ...
    def _дополнение_карточки(self, путь: str) -> bytes:
        """Дополнение карточки в production не выводится.

        Блок умеет показать поля контракта, которых нет на карточке витрины
        (сезоны, фон, рекомендации, внутренняя оценка), и проверен на
        фикстуре. Но встроить его в саму карточку может только приложение, а
        оно не видит контур чтения: тот живёт файлом на хосте витрины.

        Оставлять блок отдельным узлом в конце body нельзя — это и есть тот
        DOM-мост, из-за которого ломалась гидратация. Поэтому наружу он не
        идёт вовсе, а условия нативного включения переданы отдельно:
        docs/handoff/YUMMY-CARD-CONTRACT.md.
        """
        return b""
    ...
    def _страница_топ(self, зпр) -> bytes | None:
        ...
        плитки = []
        for з in свод["items"]:
            # Словарь карточки — тот же, что у всех поверхностей контура:
            # отдельное написание «для Топа» однажды разошлось бы с общим,
            # и карточка перестала бы рисоваться молча. Так и случилось,
            # когда имена полей сменились в одном месте из двух.
            разметка = СТРАНИЦЫ.карточка({
                "entity_id": з["entityId"], "canonical_path": з["canonicalPath"],
                "title": з["title"], "poster": з.get("poster"),
                "year": з.get("year"), "kind": з.get("kind"),
            })
    ...
    #: Готовые страницы держатся недолго: сборка — это обращение наверх за
    #: оболочкой и запросы к контуру. Пять минут достаточно, чтобы обычный
    #: заход был мгновенным, и мало, чтобы страница успевала устаревать.
    _готовые: dict = {}
    _готовые_до: dict = {}
    ...
    #: Какая поверхность контура на какой собственной странице.
    #:
    #: Поверхность — ровно на одной. /new/ и /schedule/ не должны показывать
    #: один массив под разными заголовками: именно так /collections/ и
    #: /schedule/ однажды совпали на 100 % и обе выглядели работающими.
    ПОВЕРХНОСТИ_СТРАНИЦ = {
        "/new": ("new_episodes", "fresh", "ongoing"),
        "/collections": ("trending", "rated", "news"),
        "/schedule": ("schedule", "announcements"),
    }

    #: Заголовок и ОБЪЯВЛЕННЫЙ критерий попадания у каждой поверхности свой.
    #:
    #: Подпись здесь не украшение: раздел без объявленного критерия нельзя ни
    #: проверить, ни оспорить, и «Актуальное» перестаёт отличаться от «что
    #: попалось». Формулировки совпадают с тем, что делает запрос.
    ПОДПИСИ_ПОВЕРХНОСТЕЙ = {
        "new_episodes": ("Новые серии",
                         "События появления серии: сезон, номер и озвучка."),
        "fresh": ("Новое в каталоге",
                  "Порядок — дата публикации записи в источнике, по убыванию."),
        "ongoing": ("Сейчас выходят",
                    "Только подтверждённый источником статус показа."),
        "trending": ("Актуальное",
                     "Формула: свежесть, сглаженный рейтинг и статус показа."),
        "rated": ("Высокие оценки",
                  "Внешние оценки с указанием провайдера и шкалы."),
        "news": ("Новости", "Редакционные материалы витрины."),
        "announcements": ("Анонсы", "Заявленные релизы."),
        "schedule": ("Ближайшие серии",
                     "Только подтверждённые даты будущих серий."),
    }

    #: Поверхности, которые рисуются не плиткой, а списком материалов.
    МАТЕРИАЛЫ = ("news", "announcements")

    #: Профиль домена называет свои разделы словами витрины (`секции` в
    #: `yummy_variants`). Здесь единственный перевод этих слов в имена
    #: поверхностей контура: два словаря одного и того же разошлись бы молча.
    СЕКЦИЯ_ПОВЕРХНОСТЬ = {
        "новые-серии": "new_episodes",
        "сейчас-выходят": "ongoing",
        "актуальное": "trending",
        "высокие-оценки": "rated",
        "новое-в-каталоге": "fresh",
    }

    #: Сколько карточек в разделе собственной страницы.
    В_РАЗДЕЛЕ = 24

    def _поверхности(self, нужные: tuple) -> dict:
        """Данные разделов — из контура чтения, а не разбором чужой разметки.

        Разбор полезной нагрузки приложения отсюда убран целиком. Он ломался
        молча: приложение сменило порядок полей в плитке (`src` перед `alt`)
        и перестало печатать рейтинг у части карточек — разбор стал возвращать
        ноль, и наружу это вышло пустыми разделами и плитками без постеров.

        Чинить выражение было бы лечением симптома. Документ приложения
        приходит потоком, и его текстовый порядок не совпадает с порядком на
        экране: измерено на живом приложении — раздел «Новое на сайте» давал
        80 карточек вместо 18, «Анонсы» — 2 вместо 12. Извлечение «по тексту
        между заголовками» надёжным не станет.

        Контур отдаёт объявленный адрес, постер, год, вид и дату у каждой
        записи. Ничего не выдумывается: чего в контуре нет, того нет и здесь.
        """
        итог = {}
        контрактные = tuple(п for п in нужные if п in (СВЯЗЬ.ИСТОЧНИКИ if СВЯЗЬ else ()))
        if контрактные:
            итог.update(self._ленты_контура(нужные=контрактные))
        if СВЯЗЬ is None:
            return итог
        try:
            with self._соединение() as соед:
                if "fresh" in нужные:
                    итог["fresh"] = СВЯЗЬ.новое_в_каталоге(соед, self.В_РАЗДЕЛЕ)
                if "rated" in нужные and ВАРИАНТЫ_МОД is not None:
                    итог["rated"] = self._оценённые(соед)
        except Exception:
            # Недоступный контур — пустые разделы, а не выдуманные.
            pass
        return итог

    def _оценённые(self, соед) -> list:
        """«Высокие оценки» — тот же свод, что и у /top/, первые записи.

        Отдельной формулы здесь нет намеренно: два порядка «по оценке» на
        одном домене однажды разойдутся, и посетитель увидит разные первые
        места в разделе и в Топ-100.
        """
        свод = ВАРИАНТЫ_МОД.топ(соед, self.В_РАЗДЕЛЕ, "imdb")
        карточки = []
        for з in свод["items"]:
            свой = next((р for р in з["ratings"] if р["provider"] == "imdb"), None)
            есть_оценка = свой is not None and свой["value"] is not None
            карточки.append({
                "entity_id": з["entityId"], "canonical_path": з["canonicalPath"],
                "title": з["title"], "poster": з.get("poster"),
                "year": з.get("year"), "kind": з.get("kind"),
                # Оценка печатается с той шкалой, в которой пришла: сводить
                # разные источники в одно безымянное число запрещено.
                "rating": (f'{свой["value"]:.1f}' if есть_оценка else None),
                "rating_source": (
                    f"{ИМЕНА_ПРОВАЙДЕРОВ.get(свой['provider'], свой['provider'])}, "
                    f"шкала {свой['scale']:.0f}" if есть_оценка else None),
            })
        return карточки

    def _разделы(self, разд: str) -> tuple[str, list]:
        """Разметка разделов страницы и список поверхностей, оставшихся пустыми."""
        нужные = self.ПОВЕРХНОСТИ_СТРАНИЦ.get(разд, ())
        if not нужные:
            return "", []
        данные = self._поверхности(нужные)
        в = ВАРИАНТЫ_МОД.вариант(ВАРИАНТ_ДОМЕНА) if ВАРИАНТЫ_МОД else {}
        # Порядок разделов — часть профиля домена, а не общий шаблон.
        #
        # Сначала список `секции` профиля (он называет разделы словами
        # витрины), затем объявленный порядок поверхностей, затем остальное.
        # Без первого шага три витрины отдавали один и тот же порядок, и
        # «редакционный гид» открывался тем же, чем каталожная витрина.
        по_словам = [self.СЕКЦИЯ_ПОВЕРХНОСТЬ.get(с) for с in (в.get("секции") or [])]
        порядок = [п for п in по_словам if п in нужные]
        порядок += [п for п in (в.get("поверхности") or [])
                    if п in нужные and п not in порядок]
        ...
        отобрано, куски, пустые = {}, [], []
        for п in порядок:
            элементы = данные.get(п) or []
            if not элементы:
                пустые.append(п)
                continue
            if ВИД is not None and ВИД.совпадения({**отобрано, п: элементы}):
                пустые.append(п)
                continue
            отобрано[п] = элементы
            заг, подпись = self.ПОДПИСИ_ПОВЕРХНОСТЕЙ.get(п, (п, ""))
            if п in self.МАТЕРИАЛЫ and ВИД is not None:
                # Материалы — не плитки: у них заголовок, источник и дата.
                куски.append(ВИД.лента(п, элементы, заголовок=заг, подпись=подпись,
                                       плитка=ВИД.пост, сеткой=False))
            else:
                куски.append(СТРАНИЦЫ.секция(заг, подпись, элементы))
        return "".join(куски), пустые

    def _возраст(self) -> str:
        """Строка о последнем пополнении контура. Неизвестность — не свежесть."""
        if СВЯЗЬ is None:
            return ""
        try:
            with self._соединение() as соед:
                return СТРАНИЦЫ.возраст_данных(СВЯЗЬ.пополнение(соед))
        except Exception:
            return ""

    #: Заголовок и лид собственной страницы: свои у каждого варианта домена.
    ЗАГОЛОВКИ_СТРАНИЦ = {
        "/new": ({"episodes-schedule": "Новые серии и обновления",
                  "editorial-guide": "Что смотреть прямо сейчас",
                  "catalog-search": "Свежие поступления каталога"},
                 "Новинки и свежие серии",
                 {"episodes-schedule": ("События появления серий и то, что продолжает "
                                        "выходить. Время события — настоящее, а не "
                                        "время пересборки сайта."),
                  "editorial-guide": ("Поводы посмотреть: что вышло только что и что "
                                      "недавно появилось в каталоге."),
                  "catalog-search": ("Что недавно опубликовано в источнике и какие "
                                     "серии вышли. Отсюда удобно уходить в поиск.")},
                 "Свежие серии и поступления."),
        "/collections": ({"editorial-guide": "Редакционные подборки",
                          "episodes-schedule": "Актуальное и высокие оценки",
                          "catalog-search": "Актуальное и высокие оценки"},
                         "Подборки",
                         {}, "Актуальное по объявленной формуле и оценки с указанием "
                             "провайдера."),
        "/schedule": ({"episodes-schedule": "Расписание выходов",
                       "editorial-guide": "Что выходит и что заявлено",
                       "catalog-search": "Расписание и онгоинги"},
                      "Расписание",
                      {}, "Подтверждённые даты будущих серий и заявленные релизы."),
    }
    ...
    def _собрать_страницу(self, разд: str) -> bytes | None:
        ...
        if оболочка is None:
            return None
        в = ВАРИАНТЫ_МОД.вариант(ВАРИАНТ_ДОМЕНА) if ВАРИАНТЫ_МОД else {}
        заголовки, заг_по_умолчанию, лиды, лид_по_умолчанию = \
            self.ЗАГОЛОВКИ_СТРАНИЦ.get(разд, ({}, "Раздел", {}, ""))
        заг = заголовки.get(в.get("variant_id"), заг_по_умолчанию)
        лид = лиды.get(в.get("variant_id"), лид_по_умолчанию)
        тело, пустые = self._разделы(разд)
        if not тело:
            # Страница из одного заголовка читается как поломка: она обязана
            # назвать, каких поверхностей не хватило, и куда уйти.
            тело = СТРАНИЦЫ.нечего_показать(заг, пустые)
        тело = self._возраст() + тело
        if разд == "/collections":
            тело += (
        ...
        if разд == "/schedule":
            тело += (
                ...
                '<p class="portal-empty">Дни недели строятся из подтверждённых дат '
                'будущих серий. Контур их сейчас не отдаёт: поле даты следующей '
                'серии пусто у всех записей — значит, показывать сетку было бы '
                'показывать выдуманные серии. Календарь премьер витрины — '
                '<a href="/catalog/schedule">/catalog/schedule</a>; он наполнится '
                'в тот же момент, что и этот раздел.</p>')
        return СТРАНИЦЫ.собрать(оболочка, заг, лид, тело, вариант=в, активный=разд)

    def _обогатить(self, тело: bytes, тип: str) -> bytes:
        """Свои страницы объявляют версию; чужая разметка не трогается.

        Раньше здесь в проксируемую страницу дописывались мета-теги версии,
        стиль, атрибуты на `<html>`, атрибуты формы, мета-тег robots, скрипт,
        полоса разделов, бейдж и дополнение карточки. Каждое из этого — чужой
        узел или чужой атрибут в дереве React, и на них ломалась гидратация:
        `Minified React error #418`, а следом отказ всей клиентской части
        страницы, включая поиск.

        Теперь всё это делает приложение: меню — `header-nav.tsx`, форма
        поиска — `site-search.tsx`, robots — `policy.ts`, версия и бейдж —
        `layout.tsx`. Проксируемый ответ уходит наружу байт в байт.

        Остаётся только собственная страница витрины: её собирает сама
        витрина, и объявление версии на ней — её собственная разметка.
        """
        if "text/html" not in тип or b"</head>" not in тело:
            return тело
        if b'data-sf-own="1"' not in тело:
            return тело              # чужая страница — ни одного изменения
        вставка = (
