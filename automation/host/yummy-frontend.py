...
class Обработчик(BaseHTTPRequestHandler):
    ...
    def _семейная_страница(self, разд: str) -> bytes | None:
        ...
        # Каталог читается один раз и служит источником постеров для лент.
        каталог_плиток = СТРАНИЦЫ.плитки_каталога(self._полезная("/catalog"), предел=60)

        def лента(имя, предел=24):
            return СТРАНИЦЫ.дополнить_постерами(
        ...
        if разд == "/new":
            ...
            return СТРАНИЦЫ.собрать(
                оболочка, "Новинки и свежие серии",
                "Только что вышедшие эпизоды, появившееся видео и продолжающиеся "
                "сериалы — из тех же данных, что показывает главная.", тело)
        ...
        if разд == "/collections":
            ...
            return СТРАНИЦЫ.собрать(
                оболочка, "Подборки",
                "Сезонные и редакционные группы витрины. Ничего не придумано: "
                "это те же данные, что витрина показывает у себя.",
                "".join(части))
        ...
        if разд == "/schedule":
            ...
            тело = (СТРАНИЦЫ.секция(
                ...
                '<p class="portal-empty">Подтверждённых дат и номеров серий витрина '
                'сейчас не отдаёт: у неё нет ни маршрута расписания, ни поля даты '
                'выхода. Показывать дни с выдуманными сериями нельзя, поэтому здесь '
                'честно пусто — а выше перечислено то, что действительно известно: '
                'что выходит сейчас и что заявлено.</p>')
            return СТРАНИЦЫ.собрать(
                оболочка, "Расписание",
                "Что выходит сейчас и что заявлено. Дни недели появятся, когда "
                "витрина начнёт отдавать подтверждённые даты серий.", тело)
        return None
