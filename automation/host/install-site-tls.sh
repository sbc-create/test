...
# В настоящем прогоне HTTP-конфигурация обязана уже стоять: без неё certbot
# не подтвердит домен. В сухом прогоне её поставил бы предыдущий шаг, и
# требовать её здесь значило бы обрывать проверку всей команды на пустом
# месте — именно это и случилось при первой сверке.
if [ ! -f "${TARGET_DIR}/${site}.conf" ]; then
  if [ "$dry_run" = 1 ]; then
    echo "   [сухой прогон] ${TARGET_DIR}/${site}.conf поставит предыдущий шаг"
  else
    die "сначала HTTP-конфигурация ${TARGET_DIR}/${site}.conf"
  fi
fi
