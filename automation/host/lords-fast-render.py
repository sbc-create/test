...
def main(argv: list[str] | None = None) -> int:
    ...
    # Конвейер создаёт каталог сборки заранее: `staging="$(mktemp -d)"`. А
    # `incremental.clone` отказывается писать в существующую цель — и правильно
    # делает: `cp -al base target` при существующем target положил бы копию
    # ВНУТРЬ него, и релиз собрался бы на уровень глубже.
    #
    # Из-за этого быстрый путь падал на первом же настоящем цикле:
    # «IncrementalError: цель уже существует: /tmp/tmp.oLkxTcppsA», код 1,
    # конвейер уходил в полную сборку. Отказ был безопасным — витрина
    # собиралась как прежде, — но быстрый путь не работал ни разу.
    #
    # В пробах этого не было видно: я всегда передавал несуществующий путь и тем
    # самым проверял не тот вызов, который делает конвейер.
    #
    # Пустой каталог снимается, непустой — повод для полного рендера, а не для
    # догадок о том, что в нём лежит.
    цель = Path(args.staging)
    if цель.exists():
        if any(цель.iterdir()):
            отчёт["решение"] = f"каталог сборки не пуст ({цель}) — нужен полный рендер"
            print(json.dumps(отчёт, ensure_ascii=False))
            return НУЖЕН_ПОЛНЫЙ
        цель.rmdir()
    ...
    итог = fast_path.apply(
        ...
        write=True,
        # Сличение по ходу сборки. Без него рендер держал тела всех 9721
        # страницы разом — 1791 МБ на витрину, из-за чего три витрины нельзя
        # было собирать одновременно и третья выходила за срок.
        stream=True,
    )
