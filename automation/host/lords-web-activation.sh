...
#
# Сайты Lords публичны: Basic Auth снят навсегда отдельным сценарием
# remove-lords-basic-auth.sh. Здесь пароль не ставится, не снимается и не
# восстанавливается ни при каком исходе — ни по таймауту, ни при неверном
# токене, ни при откате. Форма просто добавляется адресом в серверный блок
# уже публичного сайта.
#
...
# ни адрес, ни код не печатаются, форма снимается, конфигурация возвращается из
# копии, и печатается диагностика. Пароль при этом не появляется.
#
...
# --------------------------------------------------------------------------
restore_config() {
  # Возврат конфигурации сайтов из копии. Пароля в ней нет и не появится:
  # копия снята с уже публичной конфигурации, а на всякий случай строки
  # auth_basic вычищаются ниже.
  [[ "${CONFIG_APPLIED}" -eq 1 && -d "${BACKUP_DIR}/nginx-lords" ]] || return 0
  for site in "${SITES[@]}"; do
    source_conf="${BACKUP_DIR}/nginx-lords/${site}.conf"
    [[ -f "${source_conf}" ]] || continue
    # Пароль не возвращается никогда: даже если он каким-то образом попал в
    # копию, здесь он вычищается.
    sed -e '/auth_basic_user_file/d' -e '/auth_basic[[:space:]]*"/d' \
        "${source_conf}" > "${NGINX_DIR}/${site}.conf"
    chmod 0644 "${NGINX_DIR}/${site}.conf"
  done
  rm -f "${ACTIVATION_DIR}"/*.conf 2>/dev/null || true
  if nginx -t >/dev/null 2>&1; then
    systemctl reload nginx >/dev/null 2>&1 || true
    warn "конфигурация возвращена из копии; сайты остаются публичными"
  else
...
remove_form_keep_auth_off() {
  # Форма жить не должна: адрес обязан отвечать 404. Сайты остаются публичными.
  log "убираю форму; сайты остаются публичными"
...
# --------------------------------------------------------------------------
log "ставлю форму на уже публичный сайт"
( cd "${REPO}" && "${PY}" -m factory lords-activation-config \
...
if [[ "${GATE_FAILURES}" -gt 0 ]]; then
  ...
  stop_intake
  restore_config
  die "фактическая проверка не прошла; форма снята, сайты остаются публичными"
fi
...
echo
warn "сайты публичны (пароль снят навсегда); индексация остаётся закрытой"
echo
...
if [[ "${ACCEPTED}" != "yes" ]]; then
  ...
  restore_config
  die "форма снята, ничего не изменено; сайты остаются публичными"
fi
...
if LORDS_NONINTERACTIVE=1 LORDS_RIGHTS_CONFIRMED=yes \
  ...
  restore_config
  die "стенд возвращён на fixture-релиз; сайты остаются публичными.
