...
# --- доставка обновлённого контента выделенным витринам --------------------
# Выделенная витрина читает СВОЁ хранилище. Активация копирует снимок каталога
# один раз, и без этого шага витрина замерла бы на версии момента переключения:
# снаружи это не поломка, а сайт, который «перестал пополняться», и замечают
# это через сутки.
#
# Шаг не обязателен для успеха обновления: если доставка не удалась, общий
# каталог всё равно обновлён, а витрина продолжает отвечать прежним содержимым.
# Поэтому предупреждение, а не fail.
if ! "${PYTHON}" -m factory cell deliver >/dev/null 2>&1; then
  log "ВНИМАНИЕ: доставка контента выделенным витринам не выполнена"
fi
