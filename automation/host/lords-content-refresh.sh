...
# Полный обход каталога — 53 180 записей и 10 мин 42 с (замер 2026-09-02); из-за
# него первая витрина выходила за пятнадцатиминутный срок. Источник умеет отдавать
# изменения с отметки времени, и по умолчанию спрашиваются они. Полный обход при
# этом никуда не делся: он повторяется сам раз в шесть часов (иначе исчезнувшие
# записи остались бы на витрине) и включается принудительно LORDS_FULL_REFRESH=1.
FETCH_ARGS=()
if [[ "${LORDS_FULL_REFRESH:-0}" != "1" ]]; then
  FETCH_ARGS+=(--incremental)
fi

log "обновляю живой каталог${FETCH_ARGS[0]+ (приращением, если можно)}"
...
if ! "$PYTHON" -m factory lords-live ${FETCH_ARGS[0]+"${FETCH_ARGS[@]}"} \
     >"${STATE_DIR}/last-fetch.log" 2>&1; then
...
for site in "${SITES[@]}"; do
  ...
  #
  # Ниже идентификатор релиза считается от содержимого уже собранной витрины, и
  # потому цикл всегда платил полную цену: два часа двадцать две минуты на
  # витрину, даже когда менять было нечего. Измерено 30 августа — рендер
  # lords-01 шёл 2 ч 15 мин и завершился публикацией, отличавшейся лишь
  # обогащением.
  #
  # Ворота считают отпечаток входа — каталог, версии кода и шаблонов, профиль,
  # состав полок, реестр адресов — за полторы секунды.
  #
  # Код 10 означает «вход тот же». Любой другой исход, включая поломку самих
  # ворот, означает «рендерим»: ворота, которые при собственном отказе отвечают
  # «не надо», останавливают обновление каталога навсегда и незаметно.
  # Канареечная выкладка воротами не отменяется. Ворота считают отпечаток
  # входа — каталог, версии кода и шаблонов, профиль, полки, реестр адресов —
  # и при совпадении говорят «рендерить нечего». Но отпечаток описывает вход, а
  # не то, что фактически лежит в релизе: после выкладки быстрым путём страницы
  # произведений остались от прежнего шаблона при совпадающем отпечатке, и
  # повторная выкладка была пропущена как ненужная.
  #
  # Решение выложить шаблон принято снаружи и не пересматривается оптимизацией.
  gate_allowed="${LORDS_RENDER_GATE:-1}"
  if [ "$site_canary" = "1" ]; then
    gate_allowed=0
    log "${site}: смена шаблона — ворота рендера не применяются"
  fi

  if [ "$gate_allowed" = "1" ] && [ -x "${REPO}/automation/host/lords-render-gate.py" ]; then
    gate_code=0
  ...
  # --- СМЕНА ШАБЛОНА ОТМЕНЯЕТ БЫСТРЫЙ ПУТЬ ---------------------------------
  # Быстрый путь строит сборку связанной копией текущего релиза и переписывает
  # только различающиеся страницы. При смене шаблона различаются ВСЕ страницы,
  # но узнаёт он об этом по содержимому записи, а не по шаблону, и страницы
  # произведений остаются от прежней версии.
  #
  # Измерено на канареечной выкладке lords-02 12:08: страницы разделов
  # пересобрались, а карточка произведения осталась прежней и по-прежнему несла
  # "duration": "PTNoneM", исправленный в выкладываемой ревизии. Витрина в таком
  # состоянии наполовину новая и наполовину старая, а приёмка по разделам её
  # объявляет принятой.
  fast_allowed="${LORDS_FAST_PATH:-1}"
  if [ "$site_canary" = "1" ]; then
    fast_allowed=0
    log "${site}: смена шаблона — быстрый путь отключён, нужен полный рендер"
  fi

  fast_done=0
  if [ "$fast_allowed" = "1" ] && [ -x "${REPO}/automation/host/lords-fast-render.py" ] \
...
PYEOF
  ...
  if [ ! -d "$target" ]; then
    ...
    mkdir -p "${target}"
    # Жёсткие ссылки, а не копия. Раскладка копировала 61 тысячу файлов и
    # 1,7 ГБ на каждую витрину: замер 3 сентября — 3 мин 05 с из 9 мин 41 с
    # цикла, то есть треть времени уходила на переписывание того, что и так
    # уже лежит на том же разделе.
    #
    # Сборка получена из прошлого релиза тем же способом (`cp -al`) и после
    # раскладки удаляется, поэтому связывание безопасно: удаление сборки лишь
    # уменьшает счётчик ссылок, а файлы остаются за релизом. Изменившиеся
    # страницы записываются заменой файла, а не правкой на месте, — иначе
    # прежний релиз портился бы уже сегодня, на связке «сборка ← база».
    #
    # Запасной путь оставлен: если жёсткая ссылка невозможна (например, раздел
    # окажется другим), раскладка выполняется копией, а не падает.
    ...
    # --- УКАЗАТЕЛЬ ПОИСКА ---------------------------------------------------
    # Собирается по СОБРАННЫМ страницам релиза, а не по каталогу: адрес страницы
    # вычисляет отрисовщик, и второй источник правды разошёлся бы с ним на
    # записях с совпадающими слагами — там, где ошибку труднее всего заметить.
    #
    ...
    if [ "$search_ready" = "1" ] \
      ...
      log "${site}: указатель поиска не собран — поиск ответит отказом с причиной"
    fi
  ...
  if [ -z "$current" ] || ! systemctl is-active --quiet "${site}.service"; then
    ...
    need_restart=1
  elif ! diff -rq "${current}/lib" "${target}/lib" >/dev/null 2>&1; then
    # Рантайм — это НЕ ОДИН `serve.py`.
    #
    # `serve.py` кладёт `lib/` в `sys.path` и импортирует оттуда
    # `factory.lords.search` и `search_index`; эти модули загружаются в память
    # процесса один раз, при старте. Сравнение одного `serve.py` объявляло
    # рантайм неизменным, когда менялся только `lib/`, — и витрина продолжала
    # работать СТАРЫМ кодом при новом релизе.
    #
    # Измерено 8 сентября 2026 на канарейке lords-01: релиз 0dea0dc5e4d8
    # принят, `lib/factory/lords/search.py` в нём новый, `serve.py` побайтово
    # прежний — перезапуска не произошло, и поиск отвечал прежней логикой.
    # Обнаружилось только сверкой выдачи с соседними витринами.
    log "${site}: библиотеки рантайма изменились — перезапуск нужен"
    need_restart=1
  fi
  ...
  for _ in $(seq 1 10); do
    ...
    # Сравнение средствами оболочки, без трубы. `grep -q` закрывается на первом
    # совпадении, и `printf`, не успевший дописать страницу, получает SIGPIPE;
    # при `pipefail` это провал всего цикла обновления. Пока главная была
    # меньше, запись успевала уместиться в буфер трубы и всё сходило с рук.
    if [ "${#body}" -gt 4000 ] && [[ "$body" == *'class="card'* ]]; then
      ...
      # Первый адрес произведения вырезается средствами оболочки. Любой
      # потребитель, закрывающийся на первом совпадении — и `head`, и
      # `grep -q`, и `grep -m1`, — обрывает `printf` сигналом SIGPIPE, а при
      # `pipefail` это провал всего цикла обновления.
      slug=""
      if [[ "$body" == *'href="/title/'* ]]; then
        # Без проверки на наличие подстроки отрезание вернуло бы страницу
        # целиком, и мы бы пошли запрашивать несуществующий адрес.
        slug="${body#*href=\"/title/}"
        slug="/title/${slug%%\"*}"
      fi
      if [ -n "$slug" ] && [ "$new_players" -gt 0 ]; then
        tp="$(curl -sS --max-time 10 "http://127.0.0.1:${port}${slug}" 2>/dev/null || true)"
        [[ "$tp" == *"<video-player"* ]] || continue
      fi
      ok=1; break
    fi
  done
  ...
  # Отпечаток входа записывается только здесь — после того, как релиз принят.
  ...
  #
  # Приёмка выше проходила успешно, а сайт ложился позже: рабочий процесс был
  # привязан к каталогу, который хранение потом удаляло, и все адреса разом
  # начинали отдавать 404. Между «релиз принят» и «сайт цел» оказалось время,
  # и проверять надо в обоих концах этого промежутка.
  after="$(curl -sS -o /dev/null -w "%{http_code}" --max-time 10 \
    "http://127.0.0.1:${port}/" 2>/dev/null || echo 000)"
  if [ "$after" != "200" ]; then
    log "${site}: после уборки главная отвечает ${after} — перезапускаю рантайм"
    systemctl restart "${site}.service"
    sleep 2
    after="$(curl -sS -o /dev/null -w "%{http_code}" --max-time 10 \
      "http://127.0.0.1:${port}/" 2>/dev/null || echo 000)"
    [ "$after" = "200" ] || fail "${site}: главная отвечает ${after} после уборки"
  fi
