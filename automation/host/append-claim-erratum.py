...
def собрать(*, документ: str, source_event_id: str, source_seq: int, source_type: str) -> dict:
    with open(документ, "rb") as ф:
        сырое = ф.read()
    ...
    основа = {
        ...
        "corrections": sorted(
            [(и["path"], и["disposition"], и.get("replacement_value")) for и in исправления],
            key=lambda x: x[0],
        ),
    }
    ключ = (
        "erratum:"
        + hashlib.sha256(
            json.dumps(основа, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode()
        ).hexdigest()[:32]
    )
    ...
    return {
        ...
        "prompt_id": "FLEET-CORE-003-CHANGESET-WORKFLOW",
        "prompt_rev": "R2",
        "source_event_id": source_event_id,
        ...
        "summary": (
            "исправление утверждений FLEET-TPL-005: "
...
def отправить(событие: dict) -> dict:
    ...
    try:
        ...
        return {"status": ош.code, "body": json.loads(ош.read() or b"{}")}


if __name__ == "__main__":
    документ, sid, seq, тип = sys.argv[1], sys.argv[2], int(sys.argv[3]), sys.argv[4]
    с = собрать(документ=документ, source_event_id=sid, source_seq=seq, source_type=тип)
    ответ = отправить(с)
    print(
        json.dumps(
            {
