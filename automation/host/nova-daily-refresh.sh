...
if [[ "${RELEASE_ORCHESTRATOR_REQUIRED:-0}" == "1" ]]; then
  echo "[RELEASE_ORCHESTRATOR_REQUIRED] nova-daily-refresh.sh is not a supported automation path. Use bin/site-factory-release." >&2
  exit 78
fi
echo "[deprecated] nova-daily-refresh.sh: prefer site-factory-release (set RELEASE_ORCHESTRATOR_REQUIRED=1 to refuse)." >&2
