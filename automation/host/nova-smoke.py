...
def проверить(site: str, база: str, домен: str, ожидания: dict) -> dict:
    ...
    if тайтлы:
        ...
        плееров = len(re.findall(r"<iframe\b", тайтл))
        итог["metrics"]["player_instance_max"] = плееров
        отметить("player_instance_max_1", плееров <= 1, f"{плееров} iframe")
        автозапусков = len(re.findall(r"autoplay\s*=\s*[\"']?1|autoplay=true|\ballow=\"autoplay", тайтл))
