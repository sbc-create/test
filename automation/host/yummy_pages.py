...
#: Выпадающие списки шапки на статической странице открывает не скрипт.
#:
#: Шапка копируется у приложения без скриптов — странице они не нужны, и
#: именно их вставка когда-то ломала восстановление страницы React. Но вместе
#: со скриптами пропадает и единственный способ открыть два выпадающих списка:
#: измерено на собственной странице — клик по кнопке не меняет ничего,
#: `display` остаётся `none`, `aria-expanded` остаётся `false`, и десять
#: ссылок шапки недостижимы ни мышью, ни с клавиатуры.
#:
#: Открытие переносится на состояние: наведение и фокус внутри. В покое
#: страница выглядит ровно как у приложения, а список открывается и мышью, и
#: клавишей Tab. Признаки скриптового поведения с кнопки снимаются: кнопка,
#: объявляющая `aria-expanded="false"` над открытым списком, врёт диктору.
СТИЛЬ_МЕНЮ = (
    "<style>"
    "[data-sf-own] .portal-dropdown:hover>.portal-dropdown-menu,"
    "[data-sf-own] .portal-dropdown:focus-within>.portal-dropdown-menu"
    "{display:block}"
    "</style>")
...
def собрать(оболочка: dict, заголовок: str, лид: str, тело: str,
    ...
    return (
        ...
        + голова.replace("</head>", ПУСТО_СТИЛЬ + СТИЛЬ_МЕНЮ + токены + мета + сводка
                          + "</head>", 1)
