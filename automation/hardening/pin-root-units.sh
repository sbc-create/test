...
# ========================= 14. Публикация панели ========================
# Только теперь и только после зелёного аудита. Публикация — отдельный
# идемпотентный шаг штатного установщика, который сам делает бэкап vhost,
# проверяет nginx -t, ждёт применения конфигурации, проводит живую проверку по
# публичному имени и СНИМАЕТ панель обратно, если проверка не прошла.
say ""
say "аудит зелёный — можно публиковать панель Secret Hub"
PUBLISH="${BUNDLE_DIR}/automation/secret-hub/install-secret-hub.sh"
if [ -x "$PUBLISH" ]; then
  nginx -t >/dev/null 2>&1 || fail "nginx -t не проходит ещё ДО правки — публикация не начинается"
  SECRET_HUB_REPO="$BUNDLE_DIR" bash "$PUBLISH" \
    || fail "публикация панели не удалась; её собственный откат вернул nginx"
else
  warn "публикатор не найден в бандле: ${PUBLISH}"
  warn "закрепление выполнено и проверено; панель публикуется отдельным шагом"
fi

trap - ERR
say ""
say "ТРАНЗАКЦИЯ ЗАВЕРШЕНА"
say "  релиз:  $RELEASE_ID"
say "  бэкап:  $BACKUP_DIR"
say "  аудит:  $AUDIT_OUT"
