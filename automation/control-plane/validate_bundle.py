...
шаг("JSON Schemas разбираются", len(схемы) == 13, f"{len(схемы)}")
...
сущ = [k for k, v in aa["channels"].items()
       if v["subscribe"].get("x-status") == "AVAILABLE"]
шаг("существующие Registry-события остались AVAILABLE", len(сущ) == 4, str(len(сущ)))
