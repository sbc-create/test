...
`factory deploy` берёт `flock` на пару `site+environment`, поэтому два конвейера один
сайт одновременно не изменят — второй получит `QUARANTINED` с причиной.

Интеграционные тесты обращаются к цели напрямую, минуя конвейер, поэтому полный прогон
`tests/run-all.sh` сам сериализуется через `flock var/locks/run-all.lock`. Не запускай
`factory deploy` для того же сайта параллельно с прогоном тестов: артефакты проверок
пишутся по пути `artifacts/qa/<site_id>/` и будут перетёрты.

## Списание стенда

```bash
python3 -m factory decommission --site <id>            # остановить сервер, освободить порт
python3 -m factory decommission --site <id> --purge    # плюс удалить каталог цели с релизами
```

Без списания процессы стендов накапливаются и занимают весь разрешённый диапазон
портов; следующий деплой получит `BLOCKED_ACCESS` с прямым указанием на причину.
