...
**Рантайм Lords терял SIGTERM.** Примерно один запуск из семи не
останавливался по сигналу и доживал до таймаута соединения в тридцать секунд:
SIGTERM доставлен и снят (`SigPnd` пуст, `SigCgt` содержит `0x4000`), главный
поток в `futex_wait_queue_me`, обработчик уровня Python не выполнялся из-за
бесконечного ожидания при `SA_RESTART`. В боевом режиме systemd добивал бы юнит
по `TimeoutStopSec` вместо чистой остановки. Исправлено ожиданием с таймаутом;
проверка — 80 запусков под нагрузкой без единого зависания.
