...
**Пункт закрыт: ночной запуск 04.09 прошёл целиком.** Дожидаться его стоило —
он снял и сам гейт, и два моих ошибочных прогноза.

```
03:20:41 → 03:26:00   5 мин 19 с, код 0
host-20260904T032041Z.tar.gz   подтверждён (.verified.json есть)
диск: 8,1 ГБ до → 3,0 на пике → 8,0 после
```

Расчёт потребления оказался верен: предсказывались пик около 5,3 ГиБ и остаток
около 2,8, наблюдалось 3,0 ГБ свободных на пике.

**А расчёт запаса в сутках был неверен, и его следует читать как отменённый.**
Здесь стояла таблица, обещавшая отказ 06.09 из-за того, что уборки в
`deploy/day05-lords` якобы нет и каждый запуск оставляет 1,2 ГБ навсегда.
Наблюдение опровергает: до запуска восемь архивов и 8,1 ГБ свободно, после —
восемь архивов и 8,0 ГБ. Старейший убран, уборка есть, роста нет. Я принял
«не нашёл уборку в коде» за «её нет», не проверив поведением.

**Что остаётся сделать.** Осиротевший `host-20260903T032419Z.tar.gz` (1,2 ГБ,
без `.verified.json`) пережил и этот запуск: развёрнутая уборка считает архивы,
но не отличает подтверждённые от неподтверждённых. `backup-retention.sh` из
ветки это и добавляет — но выигрыш разовый, один архив, а не спасение от роста.
