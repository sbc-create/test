...
## Что удалось обойти и чем это закрыто

Проверки писались вместе с попытками их обмануть. Найденные обходы и правки
(набор — `tests/seo_content/test_adversarial.py`):

| Обход | Чем закрыт |
| --- | --- |
| Числительные словами: «две тысячи четырнадцатого года», «сто шестая серия» — мимо всех числовых ворот | разбор `numerals.py`; числовые проверки работают по тексту с подставленными цифрами |
| «106 серия» считалось количеством, а не номером | различение по форме существительного при данном числительном: при 106 количество пишется «серий» |
| Обещание просмотра иными словами: «плеер доступен», «серии выложены», «лежит на витрине» | список оборотов расширен наблюдаемыми формами |
| Перефразированные штампы: «сюжет захватывает», «на одном дыхании» | то же; полнота списка не заявляется |
| Мягкие призывы: «начните просмотр сегодня», «убедитесь сами» | добавлены в рекламные обороты |
| Разбавление текста лишним словом через каждые девять: цепочка рвётся, пятиграммы садятся на 0,84 при пороге 0,85 | вложение словесных четвёрок — мера, не наказывающая за добавленный объём. На девяти тысячах пар принятых текстов максимум 0,375; разбавленная копия даёт 0,70 |

Обратная сторона ужесточения — ложные срабатывания, которые тоже пришлось
закрыть:

| Ложное срабатывание | Почему оно было неверным |
| --- | --- |
| «Продолжение первого сезона» на странице четвёртого объявлялось отставшим номером | это ссылка на другой сезон, а не личность страницы. Номер прощается, если стоит в значении факта; в заголовке и крошках — никогда |
| Произведение «2012» опровергало собственный год, «Драма» — жанр, «Россия» — страну | название — личность, а не утверждение. Собственное имя убирается до разбора |
| Общее длинное название засчитывалось как совпадающая цепочка слов | задание прямо выводит названия из-под этого правила |
