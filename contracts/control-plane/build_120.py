...
from factory.site_engine.audit import quarantine as qr  # noqa: E402
...
записать(
    НОВ / "schemas" / "QuarantineDecision.v1.json",
    {
...
записать(
    НОВ / "schemas" / "ProjectionState.v1.json",
    {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
...
фильтры = list(oa["paths"]["/api/v1/audit/events"]["get"]["parameters"])
...
oa["paths"]["/api/v1/audit/operational/events"] = {
    "get": {
...
oa["paths"]["/api/v1/audit/projection"] = {
    "get": {
        ...
        "responses": {
            "200": {
                ...
                "content": {"application/json": {"schema": ссылка("ProjectionState.v1")}},
            }
        },
    }
}

for путь in ("/api/v1/audit/actions/{action_id}", "/api/v1/audit/correlations/{correlation_id}"):
    ...
    узел.setdefault("parameters", []).append(
        {"name": "include_quarantined", "in": "query", "schema": {"type": "boolean"}}
    )
...
НОВЫЕ = {
    ...
    "audit.projection.rebuilt.v1": "Операционная проекция пересобрана и переключена.",
}
for имя, описание in НОВЫЕ.items():
    aa["channels"][f"fleet/{имя}"] = {
        "subscribe": {
...
cc["capabilities"].extend(
    [
        {
            "id": "audit.operational_projection",
            "status": "AVAILABLE",
            "description": (
                "Рабочая проекция журнала без карантинных записей; "
        ...
        },
        {
            ...
            "status": "AVAILABLE",
            "description": (
                "Логическое изъятие множества записей новым событием "
        ...
        {
            "id": "audit.token_revocation",
            "status": "AVAILABLE",
            "description": (
                "Отзыв токена по отпечатку без изменения исходного "
...
om["resources"].append(
    {
        ...
        "fields": ["ledger_seq", "excluded", "exclusion_reason", "quarantine_event_id"],
        ...
        "note": (
            "Проекция не является источником истины: при любом расхождении "
