...
ОТЧЁТ = Path(os.environ.get("CONTRACT_EVIDENCE_DIR", "/srv/site-factory/audit-ledger/evidence"))
провалы: list[str] = []
...
шаг(
    "схемы валидны по Draft 2020-12",
    есть_валидатор and not битые,
    ", ".join(битые) if битые else ("нет jsonschema" if not есть_валидатор else ""),
)
шаг("$id уникальны и заполнены", len(ids) == len(set(ids)) and all(ids), f"{len(set(ids))}")
...
if есть_валидатор:
    ...
    шаг("ObservedValue: UNKNOWN без value допустим", not list(v.iter_errors(неизвестно)))
    плохо = {"state": "PRESENT", "value": 1}
    шаг("ObservedValue: PRESENT без observed_at отвергается", bool(list(v.iter_errors(плохо))))
    cap = Draft202012Validator(json.loads((КОРЕНЬ / "schemas" / "Capability.v1.json").read_text()))
    планируемая = {
        "capability_id": "x",
        "owner_service": "y",
        "version": "1.0.0",
    ...
    шаг("Capability: PLANNED без адреса допустим", not list(cap.iter_errors(планируемая)))
    ложная = dict(планируемая, status="PLANNED", read_endpoint="/api/v1/x")
    шаг("Capability: PLANNED с адресом отвергается", bool(list(cap.iter_errors(ложная))))

# --- OpenAPI / AsyncAPI ------------------------------------------------------
oa = json.loads((КОРЕНЬ / "openapi.json").read_text(encoding="utf-8"))
шаг(
    "OpenAPI 3.x с путями",
    oa.get("openapi", "").startswith("3.") and len(oa.get("paths", {})) >= 20,
    f"{len(oa.get('paths', {}))} путей",
)
aa = json.loads((КОРЕНЬ / "asyncapi.json").read_text(encoding="utf-8"))
шаг(
    "AsyncAPI 2.x с каналами",
    aa.get("asyncapi", "").startswith("2.") and len(aa.get("channels", {})) >= 28,
    f"{len(aa.get('channels', {}))} каналов",
)
...
_стало = {k for k, v in aa["channels"].items() if v["subscribe"].get("x-status") == "AVAILABLE"}
if ПРЕДЫДУЩАЯ and (БАЗА / ПРЕДЫДУЩАЯ / "asyncapi.json").exists():
    ...
    _было = {
        k for k, v in _база["channels"].items() if v["subscribe"].get("x-status") == "AVAILABLE"
    }
    шаг(
        f"каналы {ПРЕДЫДУЩАЯ} остались AVAILABLE",
        _было <= _стало,
        f"потеряно: {sorted(_было - _стало)}"
        if _было - _стало
...
шаг(
    "каждый ресурс имеет владельца и политику",
...
инвентарь = {
    ...
    "entries": [
        ...
        {
            "name": "registry.events.outbox",
            "owner": "architect",
            "kind": "events",
            "classification": "CANONICAL",
            "detail": "site.*.v1 через durable outbox",
        },
        {
        ...
        {
            "name": "config/portfolio.fixture.json",
            "owner": "seo",
            "kind": "files",
            "classification": "COMPATIBLE_LEGACY",
        ...
        {
            "name": "inventory/portfolios.yaml",
            "owner": "seo",
            "kind": "files",
        ...
        {
            "name": "site-factory-seo-dryrun.service",
            "owner": "seo",
            "kind": "unit",
            "classification": "COMPATIBLE_LEGACY",
        ...
        {
            "name": "COMPATIBILITY_MATRIX.yaml",
            "owner": "architect",
            "kind": "files",
        ...
        {
            "name": "templates/seo/content/monitoring/backup APIs",
            "owner": "various",
            "kind": "http",
...
шаг(
    "инвентарь текущих контрактов составлен",
    len(инвентарь["entries"]) >= 10,
    f"{len(инвентарь['entries'])} записей",
)
