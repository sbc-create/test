...
def собрать() -> dict:
    ...
    if "template.release" not in уже:
        ...
        матрица["generated_at"] = (
            _d.datetime.now(_d.timezone.utc).isoformat().replace("+00:00", "Z")
        )
    ...
    свойства["audience"] = {"type": "string", "enum": ["changeset-worker", "templates-executor"]}
    свойства["resource_kind"] = {
        "type": "string",
        "enum": ["fake.resource", "template.release", "template.build"],
    }
    свойства["action"] = {
        "type": "string",
        "enum": ["publish", "rollback", "create", "update", "patch"],
    }
    свойства["environment"] = {"type": "string", "enum": ["test", "non-production", "production"]}
    ...
    записать(
        цель / "schemas/ExecutionGrant.v1.json",
        {
    ...
    # --- схема erratum ----------------------------------------------------
    записать(
        цель / "schemas/ClaimErratum.v1.json",
        {
            "$schema": "https://json-schema.org/draft/2020-12/schema",
                ...
                "claim_corrections": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            ...
                            "disposition": {
                                "type": "string",
                                "enum": ["CORRECTED", "WITHDRAWN_UNVERIFIED"],
                            },
                            "replacement_value": {},
                        },
                        "allOf": [
                            {
                                "if": {"properties": {"disposition": {"const": "CORRECTED"}}},
                                "then": {"required": ["replacement_value"]},
                            },
                            {
                                "if": {
                                    "properties": {"disposition": {"const": "WITHDRAWN_UNVERIFIED"}}
                                },
    ...
    существующие = {c.get("capability_id") or c.get("id") for c in каталог["capabilities"]}
    ...
    каталог["available"] = sum(1 for c in каталог["capabilities"] if c.get("status") == "AVAILABLE")
    каталог["planned"] = sum(1 for c in каталог["capabilities"] if c.get("status") == "PLANNED")
    ...
    openapi.setdefault("paths", {})["/grant"] = {
        "post": {
                ...
                "content": {
                    "application/json": {
                        "schema": {
                            "type": "object",
                            "additionalProperties": False,
                            "required": ["changeset_id", "audience", "fencing_token"],
                            "properties": {
            ...
            "responses": {
                    ...
                    "content": {
                        "application/json": {
                            "schema": {
                                "type": "object",
                                "properties": {
                                    "grant": {"$ref": "#/components/schemas/ExecutionGrant"},
                ...
                "403": {"description": "аудитория, запросчик или окружение " "не допущены"},
                "409": {"description": "состояние набора или маркер не " "допускают выдачи"},
            },
        }
    }
    ...
    манифест["version"] = В
    манифест["generated_at"] = _d.datetime.now(_d.timezone.utc).isoformat().replace("+00:00", "Z")
