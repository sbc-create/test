...
НОВЫЕ_ОШИБКИ = [
    {"error_code": "VERSION_CONFLICT", "status": 409, "retryable": True,
     "owner": "control-plane",
     "detail": "версия ресурса разошлась с ожидаемой"},
    {"error_code": "EXPECTED_VERSION_INVALID", "status": 422,
     "retryable": False, "owner": "control-plane",
     "detail": "expected_resource_version или If-Match разобрать нельзя, "
               "либо они расходятся между собой"},
    {"error_code": "FILTER_VALUE_UNKNOWN", "status": 422, "retryable": False,
     "owner": "control-plane",
     "detail": "значение фильтра не принадлежит перечислению"},
    {"error_code": "EVENT_VERSION_UNKNOWN", "status": 422, "retryable": False,
     "owner": "control-plane",
     "detail": "версия события неизвестна этому выпуску; запись задержана"},
    {"error_code": "LEDGER_UNAVAILABLE_EXHAUSTED", "status": 503,
     "retryable": False, "owner": "control-plane",
...
РАНЕЕ_НЕ_ОБЪЯВЛЕННЫЕ = [
    ...
    ("FILTER_VALUE_INVALID", 422, False,
     "значение параметра не разбирается: after и limit — целые"),
    ("IRREVERSIBLE_PLAN", 422, False,
    ...
    ("LIFECYCLE_FORBIDDEN", 409, False,
     "состояние жизненного цикла цели не допускает изменения"),
    ...
    ("OPERATION_UNSUPPORTED", 422, False,
     "род ресурса такой операции не допускает"),
    ("OWNERSHIP_MISMATCH", 409, False,
     "ресурс принадлежит другой службе: чужую запись не меняют"),
...
НОВЫЕ_ОШИБКИ += [
    {"error_code": к, "status": с, "retryable": п, "owner": "control-plane",
     "detail": д}
...
def собрать() -> dict:
    ...
    for с in НОВЫЕ_СОСТОЯНИЯ:
        if с not in статусы:
            ...
            якорь = {"APPLIED": "APPLYING", "VERIFIED": "VERIFYING",
                     "ROLLBACK_REQUESTED": "APPLY_FAILED"}[с]
    ...
    св["risk_class"]["description"] = (
        ...
        "безобидным.")
    св["impact_level"] = {
        "type": ["string", "null"], "enum": [*УРОВНИ_ВЛИЯНИЯ, None],
    ...
    образец_имя = next(
        (и for и in каналы if и.endswith("/changeset.applied.v1")), None)
    ...
    if "impact_level" not in по_имени:
        параметры.insert(
            ...
            if "risk_class" in по_имени else len(параметры),
            {"name": "impact_level", "in": "query", "required": False,
    ...
    заголовок_версии = {
        ...
        "schema": {"type": "string"}}
    для_действий = [p for p in o["paths"]
                    if p.startswith("/api/v1/changesets/{changeset_id}")]
    for путь in для_действий:
        for метод, оп in o["paths"][путь].items():
            if метод == "get":
                ...
                if isinstance(отв, dict):
                    отв.setdefault("headers", {})["ETag"] = {
                        "description": "Версия ресурса; возвращается обратно "
                                       "в If-Match.",
                        "schema": {"type": "string"}}
                continue
            ...
            тело = оп.setdefault("requestBody", {}).setdefault(
                "content", {}).setdefault("application/json", {}).setdefault(
                "schema", {"type": "object", "properties": {}})
            св_тела = тело.setdefault("properties", {})
            св_тела.setdefault("expected_resource_version", {
                "type": "integer", "minimum": 1,
                "description": "То же, что If-Match, полем тела. Заданы оба "
                               "и расходятся — запрос отклоняется."})
            св_тела.setdefault("request_id", {
                "type": "string", "maxLength": 200,
                "description": "Идентификатор запроса для истории."})
            оп.setdefault("responses", {}).setdefault("409", {
...
""" + чл, encoding="utf-8")
    ...
    (цель / "MIGRATION.md").write_text(f"""# Переход {ИЗ} → {В}
...
""", encoding="utf-8")
    ...
    м["version"] = В
    м["generated_at"] = _d.datetime.now(
    ...
    return {"version": В, "files": len(суммы["files"]),
            "statuses": len(статусы), "errors": len(ош["errors"])}
