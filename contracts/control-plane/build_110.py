...
from factory.site_engine.audit import ledger_store as _store  # noqa: E402
...
записать(
    НОВ / "schemas" / "AuditEvent.v1.json",
    {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "$id": f"{С}/schemas/AuditEvent.v1.json",
        "title": "AuditEvent.v1",
        "description": (
            "Неизменяемая запись журнала: состоявшийся факт, а не "
        ...
        "properties": {
            ...
            "stored_at": dict(
                ВРЕМЯ, description="Когда запись зафиксирована. " "Не подменяет occurred_at."
            ),
            ...
            "approval_ref": СТРОКА,
            "rollback_ref": СТРОКА,
            "evidence_refs": {"type": "array", "items": {"$ref": f"{С}/schemas/EvidenceRef.json"}},
            "corrects_event_id": dict(
                СТРОКА,
                description=(
...
записать(
    НОВ / "schemas" / "AuditCheckpoint.v1.json",
    {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "$id": f"{С}/schemas/AuditCheckpoint.v1.json",
        "title": "AuditCheckpoint.v1",
        "description": (
            "Подписанная хешем отметка состояния цепи. Уровень защиты "
...
записать(
    НОВ / "schemas" / "AuditIntegrityReport.v1.json",
    {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
...
def ссылка(имя):
    return {"$ref": f"{С}/schemas/{имя}.json"}
...
oa["paths"]["/api/v1/audit/events"] = {
    "get": {
        ...
        "description": (
            "Фильтрация выполняется на сервере. Неизвестный "
    ...
    "post": {
        ...
        "description": (
            "Идемпотентно по (producer_service, idempotency_key). "
            "Повтор того же содержимого — 200 с тем же event_id; "
            "другое содержимое под тем же ключом — 409."
        ),
        "requestBody": {
            "required": True,
            "content": {"application/json": {"schema": ссылка("AuditEvent.v1")}},
        },
        "responses": {
            ...
            "403": dict(
                проблема, description="IDENTITY_SPOOF, AUTHORITY_DENIED " "или MODEL_PHASE_DENIED"
            ),
            "409": dict(проблема, description="IDEMPOTENCY_CONFLICT"),
            "422": dict(
                проблема, description="SITE_ID_UNKNOWN, SECRET_IN_PAYLOAD " "или нарушение схемы"
            ),
        },
    },
    "put": {
        "operationId": "auditEventsPutDenied",
        "summary": "Запрещено",
        "description": "Журнал только на добавление.",
        "responses": {"405": dict(проблема, description="APPEND_ONLY")},
    },
    "patch": {
        "operationId": "auditEventsPatchDenied",
        "summary": "Запрещено",
        "description": "Журнал только на добавление.",
        "responses": {"405": dict(проблема, description="APPEND_ONLY")},
    },
    "delete": {
        "operationId": "auditEventsDeleteDenied",
        "summary": "Запрещено",
        "description": "Журнал только на добавление.",
        "responses": {"405": dict(проблема, description="APPEND_ONLY")},
    },
}
oa["paths"]["/api/v1/audit/events/{event_id}"] = {
    "get": {
        ...
        "responses": {
            "200": ответ("Событие", ссылка("AuditEvent.v1")),
            "404": dict(проблема, description="Не найдено"),
        },
    }
}
oa["paths"]["/api/v1/audit/correlations/{correlation_id}"] = {
    "get": {
        "operationId": "getAuditCorrelation",
        "summary": "Полная нить действия в порядке фаз",
        "parameters": [
            {"name": "correlation_id", "in": "path", "required": True, "schema": {"type": "string"}}
        ],
        "responses": {"200": ответ("Нить событий")},
    }
}
oa["paths"]["/api/v1/audit/actions/{action_id}"] = {
    "get": {
...
oa["paths"]["/api/v1/audit/integrity"] = {
    "get": {
...
КАНАЛЫ = {
    ...
    "audit.integrity.failed.v1": "Цепь хешей не сошлась; указана первая расходящаяся позиция.",
    ...
    "audit.reconciliation.failed.v1": "Сверка с производителем нашла пропуски или дубли.",
    "audit.backup.stale.v1": "Последняя проверенная копия старше допустимого возраста.",
}
...
for имя, описание in КАНАЛЫ.items():
    aa.setdefault("channels", {})[f"fleet/{имя}"] = {
        "subscribe": {
            ...
            "x-evidence-required": имя
            in ("audit.integrity.failed.v1", "audit.reconciliation.failed.v1"),
...
aa.setdefault(
    "x-self-event-policy",
    {
        ...
        "rule": (
            "События, которые журнал порождает о собственной работе, "
            "публикуются в ленту, но не принимаются обратно на запись."
        ),
        "delivery": "at-least-once",
        "consumption": "idempotent",
        "note": (
            "Ровно-однократная доставка не заявляется: она недостижима, и "
...
om.setdefault("resources", []).append(
    {
        ...
        "secret_class": "NONE",
        "mutation_policy": (
            "append-only; UPDATE и DELETE запрещены триггерами БД " "и отвечают 405 APPEND_ONLY"
        ),
        "note": (
            "Реестр остаётся источником истины о сайтах. Журнал хранит факты "
...
cc.setdefault("capabilities", []).extend(
    [
        ...
        {
            "id": "audit.backup.local",
            "status": "AVAILABLE",
            "description": "Копия и проверенное восстановление в изоляции на этом хосте.",
        },
        {
            "id": "audit.backup.off_host",
            "status": "PLANNED",
            "description": (
                "Копия вне хоста. Останется PLANNED, пока восстановление "
                "на другом хосте не выполнено и не измерено."
            ),
        },
        {
            "id": "audit.autonomous_apply",
            "status": "PLANNED",
...
ec.setdefault("errors", []).extend(
    [
        ...
        {
            "code": "EVIDENCE_PATH_DENIED",
            "http": 403,
            "meaning": "Путь доказательства выходит за разрешённые корни.",
        },
        {"code": "FILTER_UNKNOWN", "http": 422, "meaning": "Неизвестный параметр фильтра."},
    ]
)
...
записать(
    НОВ / "observability.json",
    {
        ...
        "metrics": [
            ...
            {"name": "audit_append_rejected_total", "type": "counter", "labels": ["error_code"]},
            ...
            {
                "name": "audit_chain_verified_ok",
                "type": "gauge",
                "note": (
                    "1 — цепь сошлась; 0 — нет. Отдельное 'не проверяли' = "
                    "отсутствие метрики, не 0."
                ),
            },
            {
                "name": "audit_reconciliation_missing",
                "type": "gauge",
                "labels": ["producer_service"],
            },
            {
                "name": "audit_backup_age_seconds",
                "type": "gauge",
        ...
        "alerts": [
            {
                ...
                "expr": "audit_chain_verified_ok == 0",
                "severity": "critical",
                "channel": "audit.integrity.failed.v1",
            },
            {
                "name": "AuditReconciliationGap",
                "expr": "audit_reconciliation_missing > 0",
                "severity": "critical",
                "channel": "audit.reconciliation.failed.v1",
            },
            {
                "name": "AuditOutboxBacklog",
                "expr": "audit_outbox_backlog > 100",
                "for": "15m",
                "severity": "warning",
            },
            {
                "name": "AuditDlqNonEmpty",
                "expr": "audit_dlq_size > 0",
                "severity": "warning",
                "channel": "audit.ingest.failed.v1",
            },
            {
                "name": "AuditBackupStale",
                "expr": "audit_backup_age_seconds > 172800",
                "severity": "warning",
                "channel": "audit.backup.stale.v1",
            },
        ],
        "scope": "internal",
        "note": (
            "Контур внутренний. Счётчики Метрики и проекты Topvisor не "
...
записать(НОВ / "checksums.json", {"version": ВЕРСИЯ, "algorithm": "sha256", "files": суммы})
