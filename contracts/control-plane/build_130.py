...
from factory.site_engine.changeset import model as M  # noqa: E402
from factory.site_engine.changeset import policy as POL  # noqa: E402
...
записать(
    НОВ / "schemas" / "ChangeSet.v1.json",
    {
        ...
        "properties": {
            ...
            "approval": {"anyOf": [{"$ref": f"{С}/schemas/Approval.v1.json"}, {"type": "null"}]},
            "evidence_refs": {"type": "array", "items": {"$ref": f"{С}/schemas/EvidenceRef.json"}},
            "failure_reason": {"type": ["string", "null"]},
            "targets": {"type": "array", "items": {"$ref": f"{С}/schemas/ChangeSetTarget.v1.json"}},
            "transitions": {
                "type": "array",
                "items": {"$ref": f"{С}/schemas/ChangeSetTransition.v1.json"},
            },
...
записать(
    НОВ / "schemas" / "ChangeSetTarget.v1.json",
    {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
...
записать(
    НОВ / "schemas" / "ChangeSetTransition.v1.json",
    {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        ...
        "properties": {
            ...
            "actor_id": {"type": "string"},
            "actor_role": {"enum": [M.PROPOSER, M.VALIDATOR, M.APPROVER, M.EXECUTOR, M.OPERATOR]},
...
записать(
    НОВ / "schemas" / "Approval.v1.json",
    {
        ...
        "properties": {
            ...
            "approver_type": {
                "enum": ["SERVICE", "HUMAN"],
                "description": "MODEL отсутствует намеренно: " "модель не одобряет изменения.",
            },
...
записать(
    НОВ / "schemas" / "AdapterCapabilities.v1.json",
    {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
...
oa["paths"]["/api/v1/changesets"] = {
    "get": {
        ...
        "parameters": [{"name": n, "in": "query", "schema": {"type": "string"}} for n in ФИЛЬТРЫ]
        + [
    ...
    "post": {
        ...
        "requestBody": {
            "required": True,
            "content": {"application/json": {"schema": ссылка("ChangeSet.v1")}},
        },
        "responses": {
            ...
            "422": dict(
                проблема, description="STATUS_NOT_WRITABLE, " "FIELD_REQUIRED или PAYLOAD_REJECTED"
            ),
        },
    },
}
for метод, код in (("put", "ACTION_ONLY"), ("patch", "ACTION_ONLY"), ("delete", "ACTION_ONLY")):
...
oa["paths"]["/api/v1/changesets/{changeset_id}"] = {
    "get": {
        "operationId": "getChangeSet",
        "x-status": "AVAILABLE",
        "parameters": [
            {"name": "changeset_id", "in": "path", "required": True, "schema": {"type": "string"}}
        ],
        "responses": {
            "200": {
                "description": "Набор",
...
oa["paths"]["/api/v1/changesets/{changeset_id}/transitions"] = {
    "get": {
        "operationId": "listChangeSetTransitions",
        "x-status": "AVAILABLE",
...
ДЕЙСТВИЯ = {
    ...
    "rollback": (
        "Откатить",
        "executor",
        "Идемпотентная компенсация с проверкой восстановленного " "состояния.",
    ),
    "cancel": ("Отменить предложение", "proposer", "Доступно до применения."),
}
for действие, (сводка, роль, описание) in ДЕЙСТВИЯ.items():
    oa["paths"][f"/api/v1/changesets/{{changeset_id}}/{действие}"] = {
        "post": {
            "operationId": "changeSet" + действие.title().replace("-", ""),
            "summary": сводка,
            "description": описание,
            "x-required-role": роль,
            "x-status": "AVAILABLE",
            "parameters": [
                {
                    "name": "changeset_id",
            ...
            "responses": {
                ...
                "409": dict(
                    проблема,
                    description="TRANSITION_NOT_ALLOWED, "
                    "LOCK_CONFLICT, FENCED_OUT, "
                    "PLAN_STALE или VERSION_CONFLICT",
                ),
                "422": dict(проблема, description="Нарушение валидации"),
                "503": dict(
                    проблема, description="AUDIT_LEDGER_UNAVAILABLE или " "REGISTRY_UNAVAILABLE"
                ),
            },
        }
    }

oa["paths"]["/api/v1/workflows"] = {
    "get": {
...
СОБЫТИЯ_ПЕРЕХОДОВ = sorted({п.событие for п in M.ПЕРЕХОДЫ if п.событие} | {"changeset.proposed.v1"})
ПОТРЕБИТЕЛИ = ["architect", "templates", "seo", "content", "monitoring", "backup", "qwen"]
...
for имя in ("workflow.started.v1", "workflow.completed.v1", "workflow.blocked.v1"):
...
cc["capabilities"].extend(
    [
        {
            "capability_id": "changeset.propose",
            ...
            "dependencies": ["registry.sites.filter"],
        },
        {
            "capability_id": "changeset.validate",
            "owner_service": "control-plane",
            "version": ВЕРСИЯ,
            "status": "AVAILABLE",
            ...
            "required_scopes": ["changeset:validate"],
            "dependencies": [],
        },
        {
            "capability_id": "changeset.approve",
            "owner_service": "control-plane",
            "version": ВЕРСИЯ,
            "status": "AVAILABLE",
            "read_endpoint": "/api/v1/changesets/{changeset_id}",
            ...
            "required_scopes": ["changeset:approve"],
            "dependencies": [],
        },
        {
            "capability_id": "changeset.apply",
            "owner_service": "control-plane",
            "version": ВЕРСИЯ,
            "status": "AVAILABLE",
        ...
        {
            "capability_id": "changeset.rollback",
            "owner_service": "control-plane",
            "version": ВЕРСИЯ,
            "status": "AVAILABLE",
            "read_endpoint": "/api/v1/changesets/{changeset_id}",
        ...
        {
            "capability_id": "changeset.production_apply",
            "owner_service": "control-plane",
            "version": ВЕРСИЯ,
            "status": "PLANNED",
            "read_endpoint": None,
        ...
        {
            "capability_id": "changeset.adapter.templates",
        ...
        {
            "capability_id": "changeset.adapter.content",
            "owner_service": "content",
            "version": ВЕРСИЯ,
            "status": "PLANNED",
            "read_endpoint": None,
            "command_endpoint": None,
...
ec["errors"].extend(
    [
        ...
        {"code": "LOCK_CONFLICT", "http": 409, "meaning": "Цель уже изменяется другим набором."},
        {"code": "FENCED_OUT", "http": 409, "meaning": "Предъявлен устаревший маркер ограждения."},
        ...
        {
            "code": "AUTONOMOUS_PRODUCTION_APPLY_DISABLED",
            "http": 403,
...
om["resources"].append(
    {
        ...
        "readers": ["architect", "templates", "seo", "content", "monitoring", "backup", "qwen"],
